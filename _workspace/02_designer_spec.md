...
- 가이드 시트 규정 준수: 흰 배경에서는 기본형 사용. 푸터 배경이 `--color-surface`(#f9fafb)이므로 로고 행 전체를 **흰색 칩(배경은 `--color-bg` 토큰으로 지정 — 하드코딩 #fff 금지, radius `8px`, 패딩 `8px 12px`, 보더 `1px solid --color-border-soft` 장식)** 안에 배치해 기본형 규정을 충족한다. 그레이/검정/오렌지 배경 변형은 본 사이트 범위에서 사용하지 않는다.
