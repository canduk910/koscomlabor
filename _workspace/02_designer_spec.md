...
| 요소 | 폰트 | 웨이트 | 자간 |
|------|------|--------|------|
| 히어로 제목 (모드 1 urgent 제목 / 모드 2 "코스콤지부", 40/56px) | Gmarket Sans | Bold 700 | `-0.03em` |
| 헤더 로고타입 "코스콤지부" (24/32px — 브랜드 아이덴티티) | Gmarket Sans | Bold 700 | `-0.02em` |
...
- 변경 파일: `globals.css`(@font-face 2 + `--font-display` 1) / `HeroPanel.tsx`(제목·CTA 클래스) / `SiteHeader.tsx`("코스콤지부" span) / `DateBadge.tsx`(M/D·D-n span) / `PostList.tsx`(모바일 D-n span) / 폰트 변환 스크립트 1식(`scripts/` — pretendard 동기화 스크립트와 동일 패턴 가능). 그 외 컴포넌트 무변경.
...
| 대상 | 변경 |
|------|------|
| `SiteHeader.tsx` | 배경 `bg-primary`·보더 제거 / 텍스트 2줄 흰색·명칭 "코스콤(한국증권전산)지부"로 변경 / 2줄 크기 20px→md 32px·Gmarket Bold(§12 `font-display`) / 마크 `rounded-lg` / focus-visible 흰 링 |
| 그 외 | 변경 없음 — 신규 폰트·토큰·자산 0건 |
