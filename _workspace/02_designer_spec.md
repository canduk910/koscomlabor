...
| 19 | `#1a1a1a` | `#eff6ff` | 15.99 | AAA(본문) | 가이드 링크 카드(§9) 설명 텍스트 |
| 20 | `#1d4ed8` | `#eff6ff` | 6.16 | UI 3:1 통과 | 가이드 링크 카드(§9) 좌측 보더 (텍스트 금지) |

- §9 가이드 링크 카드의 제목·아이콘 색은 기존 채택 조합 #8(`#1e40af` on `#eff6ff`, 8.01:1 AAA)을 재사용한다. #19·#20은 §9 추가 시(2026-08-16) 스크립트 실측 후 등재.
...
- 디지털온누리 가이드 링크(§9): 성격상 4순위(상시 정보)이나 접근 빈도가 높은 실용 가이드이므로 **긴급 배너 아래·탭리스트 위**에 상시 카드로 배치해 가시성을 확보한다. 단 1순위(긴급·적색)를 침범하지 않도록 주조색 계열 + 배너보다 낮은 시각 강도(배지 없음, 전폭 아님, 컨테이너 폭 내)로 제한한다.
...
- 근거: 첫 화면(모바일 375px 포함)에서 스크롤 없이 항상 보이는 위치이면서, 1순위인 긴급 배너(전폭·적색·배지)의 시각 우위를 유지한다 — 이 카드는 컨테이너 폭·주조색 계열·배지 없음으로 강도를 한 단계 낮춘다. 탭 목록(2순위 콘텐츠)의 진입도 가리지 않는다(§8 위계 노트 참조).

### 9.2 컴포넌트 스펙

- 구조: 단일 `<a>` 블록 카드. `href="https://onnuri.koscomlabor.cloud/"`, `target="_blank"`, `rel="noopener noreferrer"`. 카드 전체가 터치 대상.
- 컨테이너: 배경 `--color-primary-tint`(#eff6ff), radius `12px`, 좌측 보더 `4px solid --color-primary`(6.16:1 UI 통과 — 검증 #20), 패딩 모바일 `1rem` / md+ `1rem 1.5rem`. `min-height: var(--spacing-touch)`(44px — 실제 2행 구성으로 ≈80px).
- 내부 가로 배치(모바일·md+ 동일, 단일 행 flex): 좌측 가이드 아이콘 — 중앙 텍스트 블록(flex:1) — 우측 외부 링크 아이콘.
  - 가이드 아이콘: 책/문서 SVG `24px`, `currentColor`, 색 `--color-primary-strong`(8.01:1 — 채택 #8 재사용), `aria-hidden="true"`
  - 외부 링크 아이콘: ↗(화살표+사각형) SVG `20px`, `currentColor`, 색 `--color-primary-strong`, `aria-hidden="true"`
- 텍스트 블록:
  - 제목: "디지털온누리 사용 가이드" — `--text-body`(18px) / 700 / `--color-primary-strong` (8.01:1 AAA — 채택 #8 재사용)
  - 설명(제목 아래 `0.25rem`): "코스콤 조합원 대상 안내 · 외부 페이지가 새 창에서 열립니다" — `--text-caption`(15px) / 400 / `--color-ink` (15.99:1 — 검증 #19). tint 배경 위이므로 `--color-ink-muted` 금지(§2 탈락표 참조)
...
|------|--------|------|
| 기본 | 위 스펙 | #8, #19, #20 |
| hover | 제목에 밑줄 + 카드 `outline: 2px solid --color-primary-strong`(페이지 배경 #ffffff 위 8.72:1 — 채택 #6 재사용). 배경색 변화 없음 | #6 |
| focus-visible | `outline: 3px solid --color-primary; outline-offset: 2px` (흰 배경 위 6.70:1 ≥ 3:1 — 채택 #13 재사용) | #13 |
