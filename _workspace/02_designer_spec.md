...
- `<header>`: 배경 `--color-bg`(#ffffff), **상단 보더 `6px solid --color-primary` + 하단 보더 `6px solid --color-primary`**(얇은 띠 2개, 풀폭). 하단 띠가 히어로(네이비 패널)와의 경계를 담당 — v3의 색 중복 이슈 자연 해소.
- 내부: 좌측 KFIU 마크(높이 모바일 40px / md+ 48px, `alt=""`+`aria-hidden`, `priority` — 흰 배경 위 직접 배치라 radius 불요, 원본 흰 바탕이 배경에 동화) + 우측 명칭 2줄 등폭 록업. gap `0.75rem`, 전체 홈 링크 1개, `min-height: var(--spacing-touch)`, 세로 패딩 모바일 `0.75rem` / md+ `1rem`.
...
- 확정 크기 (2줄 기준값 → 1줄 계산값):
  ...
  |------|--------------|------------------|--------------|
  | 모바일(~767px) | `20px` | `23.7px` | 11.14×20 ≈ 223px + 마크·gap·패딩 84px = 307px < 375px 가용 343px |
  | md+(768px~) | `32px` (`--text-h1`) | `37.9px` | 11.14×32 ≈ 357px + 84px = 441px < 콘텐츠 폭 720px |
