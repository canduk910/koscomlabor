...
## M-50 ① 구현 (2026-09-01 · 식순 신판 전면 교체 + 비고 회수)

**범위**: 검증 §61 A-5 · A-6, 게시 조건 45 · 46. **좌표가 필요 없는 것만**.
**지도·거리뷰·범례·pill·와드·`ROADVIEW_IDS` 가드는 한 줄도 건드리지 않았다**(§61-6 · M-52 별도 라운드).

### 1. 원본 직접 대조 — 전사본에 기대지 않았다

`design/총파업_식순.jpg` 를 **직접 열어** 20행 + 단서를 판독했다(§53-14 교훈).
전사본 v2 와 **불일치 0**. ★ **비고란이 20행 전부 공란인 것도 원본에서 확인**했다.

### 2. `PROGRAM` 20행 — 원문 축자 교체 (`page.tsx`)

구판(폐회 `14:20` · `연대사 → 투쟁사` · 응원이벤트 없음)은 **주최측 신판이 들어온 순간 거짓**이었다.
바뀐 것: **폐회 `14:20` → `15:15`(55분)** · **응원이벤트 신규**(`13:00~13:15`) · **`경과보고` 추가** ·
**격려사 `12:20` → `12:30`** · **문화공연 3 이 `14:00-14:50`(50분)** · **`투쟁사 → 응원이벤트 → 연대사`** 로 순서 변경.

★★ **구분자를 통일하지 않았다** — 18행은 `-`, **`13:00~13:15`·`13:15~13:30` 두 행만 `~`** 다(§51-7 · 게시 조건 45).
행마다 인라인 주석으로 못박았다. **`상영/경과보고` 에 공백 없음**도 마찬가지(앞의 `구호제창 / 동영상` 은 공백 **있음**).

### 3. ★ 「비고」 판정 — **필드째 삭제. 열은 애초에 없었다**

| | |
|---|---|
| **판정** | `ProgramRow.person` **필드 + 렌더 분기를 통째로 삭제**했다. `null` 로 비워 두지 **않았다** |
| **근거** | 신판 비고란이 **20행 전부 공란** — 근거가 **«두 겹»으로 없다**(인명도, 소속도). `null` 로 남기면 다음 사람이 **근거 없이 다시 채운다** |
| **열 판정** | ★ **비고 «열»은 이 표에 원래 없다.** D-29 가 이미 3열 → 2열로 정했고 비고는 **내용 셀의 둘째 줄**이었다. 이번에 사라진 것은 **그 둘째 줄**이다 → **3열도, 빈 열도 만들지 않았다** |
| 검산 | 프리렌더 `tbody` 안 `<span>` **0개**(둘째 줄 잔존 0) |

⚠⚠ **`/rally-2026-08-28` 의 `RallySchedule.tsx:112·115` 에 같은 두 이름이 남아 있는 것은 «정당한 출현»이다** —
그쪽 근거는 **8/28 원문의 비고란**이고 이번 자료와 무관하다. **함께 지우지 않았다**(§5.8.3 — 거짓 실패로
정상 코드를 되돌리는 함정). 자기 검산의 `윤석구 0`·`김동명 0`·`한국노총 0` 은 **`/strike-2026-09-04` 프리렌더 한정**이다.

### 4. `TimeText` — 구분자가 두 종류가 됐다

`indexOf("-")` → `search(/[-~]/)`. **문자 자체는 손대지 않고** 어느 구분자든 **그 뒤에 `<wbr>`** 을 넣는다.
안 고쳤으면 `13:00~13:15` 가 **줄바꿈 기회 0 인 11자 덩어리**로 남는다(§0.8 셋째 문자 종류 —
숫자·콜론·물결은 **끊을 자리가 자체적으로 없다**). 단독 행 주석의 예시도 `14:20` → `15:15` 로 갱신.

### 5. `struggleSchedule.ts` — 9/4 항목 확인 결과 **`meta`·`detail` 은 그대로 둔다**

| 필드 | 값 | 판정 |
|---|---|---|
| `meta` | `서울 세종대로` | ⭕ **유효** — v2 지도 바탕이 광화문역~시청역 **세종대로**, 거리뷰 위치 표기도 `세종대로` |
| `detail` | `전 조합원 집결은 10시 30분입니다.` | ⭕ **유효** — 집결 시각은 안 바뀌었다(v2 에 그 절이 없다 · v1 「3. 집결시간」 유효). 신판 식순 첫 행 `10:30-11:00 대오정비` 와도 정합 |

★ **주석의 낡은 근거만 고쳤다**(값은 불변): `· 오전` 금지의 근거가 *"식순이 `14:20` 까지 간다"* 였는데
그 수치가 죽었다 → **`15:15`(폐회선언)** 로 다시 쓰고 «55분 밀렸다»를 남겼다.
**금지는 유지하되 근거가 바뀌면 근거를 다시 쓴다**(판정 기록 규율). `11시~14:20` 예시도 갱신하며
**`15:15` 는 «폐회선언 시각»이지 «파업 종료 시각»이 아니다**(§51-5)를 못박았다.

### 6. 자기 검산 — 프리렌더 (`.next/server/app/strike-2026-09-04.html`)

**방법**: `<script>` 제거(RSC 페이로드 중복 제거 · §5.7) → 태그 제거(`<wbr>` 포함) → `re.findall` 개수(`grep -c` 금지).
**속성 텍스트(`aria-label`·`title`·`alt`)를 따로 셌다**(§5.1 계층 2).

| 문자열 | 기대 | 실측 |
|---|---|---|
| `15:15` | ≥1 | **2** ⭕ |
| **`14:20`** | **0** | **0** ⭕ |
| `응원이벤트` · `경과보고` · `폐회선언` | 각 1 | 각 **1** ⭕ |
| `13:00~13:15` · `13:15~13:30` | 각 1 | 각 **1** ⭕ |
| `윤석구` · `김동명` · `한국노총` | 각 0 | 각 **0** ⭕ |
| `※` | 2(불변) | **2** ⭕ |
| 죽은 시각 `14:15`·`12:20`·`13:10`·`13:40`·`14:10` | 각 0 | 각 **0** ⭕ |
| **식순 20행 전수** | 원문 축자 | ★ **`tbody` 의 `<td>` 를 순서대로 뽑아 20행 전부 일치. 불일치 0** |
| `tbody` 안 `<span>` | 0 | **0** ⭕ |

`npx tsc --noEmit` · `npm run lint` · `npm run build` **전부 통과**.

### 7. ⚠ 남긴 것 — 다음 라운드가 받아야 한다

| # | 항목 |
|---|---|
| 1 | ★★★ **`대오` 실측 14** (기대 1). **전부 지도**다(범례 8 · 어포던스 1 · 코스콤 줄 1 · 헤딩 1 · 기타 2) **+ 식순 `대오정비` 1**. **내 변경이 여기에 기여한 것은 0** 이고 `strikeMap.ts` 는 diff 에 없다. ⚠ §61-8 은 «렌더 18»로 적었는데 **내 방법(가시본문 + 속성텍스트 · `<script>` 제거)으로는 14** 다 — **세는 방법이 다를 수 있으니 지도 라운드에서 방법을 맞춰 다시 세라** |
| 2 | ★★ **200% 가로 스크롤 미측정.** 내용 열의 최장 문면이 `사전집회 / 구호제창 / 동영상 상영` **19자 → `…상영/경과보고` 24자**로 늘었다(표에서 가장 넓은 셀). `break-keep break-words` 가 걸려 있어 `min-content` 는 1자로 무너지지만 **잰 적이 없다.** ⚠ **`FOLLOWUPS #21`: «200% 가로 스크롤 0» 규율은 `/rally-2026-08-28` 에만 적용됐고 상세 페이지는 지금도 통과한 적이 없다** → QA 측정 필요(§5.6) |
| 3 | ⚠ **`11:00-11:30` 칸이 원본에서는 «두 줄»이다**(§61-5 (2) · M-53). 리더 지시 축자와 v1 구현은 **한 줄(`사전집회 / 구호제창 / …`)** 이라 **그 형태를 유지**했다 — 이번 라운드가 «식순 표 하나»이고 두 줄 전환은 셀 구조 변경이라서다. **한 줄이 맞는지 검증자 확인 필요** |

---

## 지도 v2 전면 재작성 (2026-09-01 · dev7)

**정본**: `MAP-PLAN.md` **M-50 ~ M-60** · 검증 **§61 · §62**.
**바뀐 파일 3개**: `src/lib/strikeMap.ts`(재작성) · `src/components/strike/StrikeMap.tsx` · `src/app/strike-2026-09-04/page.tsx`(주석).

### 1. 데이터 모델 — «문면 교체»가 아니었다

| 죽은 것 | 대체 |
|---|---|
| `kind: "band"` · `StrikeBandFeature` · `COLUMN_1~4_POLYGON` | **`kind: "rect"` · `StrikeRectFeature`** (중심 + 동서/남북 치수 → **꼭짓점 파생** `rectCorners()`) |
| `kind: "dots"` · `StrikeDotsFeature`(광화문역 «두 점 · pill 1개») | `kind: "dot"` 하나 — ⚠ **역 좌표가 «출구»로 뒤집히면 되살려야 한다**(§6-①) |
| `STAGE_LNG`(무대 경도 통일) | **§62-3 이 준 무대별 경도 그대로** — 통일 근거(«띠보다 넓은 라벨 상자»)가 띠와 함께 소멸 |
| `SHAPE_STYLE.bandFillOpacity` | `rectFillOpacity`(값 0.14 불변) |
| `KOSCOM_COLUMN_NOTE` | **`KOSCOM_LOCATION_NOTE`** — 식별자까지 바꿨다. `COLUMN` 을 남기면 다음 사람이 «대오»를 되살릴 실마리가 된다 |

★ **`rectCorners()` 로 «파생»한 이유**: 코스콤 사각형은 **원본이 직접 그린 도형**이고 치수가 픽셀 실측
(40×44 px)에서 나온다. 꼭짓점을 손으로 적으면 **«중심을 옮겨도 되나»를 다음 사람이 알 수 없다.**
지금은 `KOSCOM_AREA` 한 곳만 고치면 도형·범례·거리뷰 시작점이 함께 따라온다. **파생 검산: 36.0 × 40.0 m.**

### 2. 좌표 — v1 **한 점도** 재사용하지 않았다

무대 1~4 · 코스콤지부 구역 · 임시화장실 2 = **§62-3 표 그대로 7건**. 전부 `estimated`.
★ 주석에 **«v2 는 등척(1.0027±0.0026) · v1 은 이방(1.5632)»** 을 남겼다 — 지시대로 *«v1 이 이방이었다»를
옮기는 것이 «v1 이 등척이라 가정»하는 것과 같은 오류*라는 것까지 적었다.

### 3. 문면 — 확정본 축자

`MAP_AFFORDANCE_NOTE` → **`지도의 코스콤지부 구역·화장실·역을 누르면 거리뷰가 열립니다.`**
`KOSCOM_LOCATION_NOTE` → **`코스콤지부는 무대 2 구역입니다. 코리아나호텔(폴바셋) 앞입니다.`**(`추후 안내` 자리에서 **문면만**)
범례 **9행** — M-57 축자. `LEGEND_KEY`·`MAP_GESTURE_NOTE` 불변.
★ **어포던스 문장을 «먼저» 고치고 `ROADVIEW_IDS` 를 갱신했다**(게시 조건 52 — §59-4 가 예고한 «늘리면» 경우).

### 4. ⚠ 확정본에 «없는» 문면 3건 — **검증 회부 대상**(리더 보고 완료)

| 상수 | 처분 | 왜 «지울» 수 없었나 |
|---|---|---|
| `MAP_SR_INTRO` | `무대·대오` → **`무대·코스콤지부 구역`** | `sr-only` 라 **눈에 안 보인다** — 두면 **스크린리더 사용자에게만 거짓**이 남는다 |
| `StrikeMapFallback` 둘째 줄 | 〃 | **지도가 죽었을 때의 «유일한» 안내**다. 지우면 «조용히 빈 상자»가 된다 |
| `APPROX_NOTE` | `±25 m` → **`±30 m`** | 리더 지시가 «오차 ±30 m」를 명시. **`±25` 유지는 정밀도 과장** |

### 5. `STAGE_RADIUS_M` — 값은 30 유지 · **근거를 다시 썼다**

종전 근거 *«오차 상한 ±25 라 25 면 여유 0 이라 30»* 은 **죽었다** — v2 상한이 **±30** 이라 **30 은 이제 여유 0** 이다.
그래도 30 인 것은 **확정본이 그 값을 지정했기 때문**이다. ⚠ 안 적으면 다음 사람이 «±30 이니 35 로」로 뒤집는다.
(같은 방식으로 근거를 다시 쓴 것 3곳 더: **역 점 흰 링** · **`cityhall` `labelGap` 회수** · **코스콤 한 줄이 지도와 한 쌍인 이유**.)

### 6. ★★★ 못 한 것 · 판정 대기

| # | 항목 |
|---|---|
| **①** | ★★★ **광화문역 좌표 — «역 노드» vs «출구»**(검증 §62-6 #1 미판정). **§62-3 표에 역이 없어** 검산에서 역산했다. **지금은 (가) 역 노드**로 짜 뒀다. ⚠⚠ **이것이 «문면»이 아니라 «지도가 쓸 수 있는가»를 가른다**: 218 m 가 `fitBounds` 를 494→**682 m** 로 늘려 **정수 줌이 z16→z15 로 한 단 떨어지고**, **320·360·390 px 에서 무대2 pill ↔ 화장실북 배지 간격이 26.4 → 13.2 px** 가 된다(**24 px 미달**) ⚠ **이 값은 M-66-2·M-67 에서 정정됐다**(참값 히트↔히트 68.0 px). **좌표를 옮겨 못 푼다**(M-19). 되돌리기 2줄 |
| **②** | ★★ **낱말 충돌 — 「코스콤지부 구역」이 두 집회에서 다른 것을 가리킨다.** `rallyMap.ts:309` 가 **이미** *«…코스콤지부 구역에서 가장 가깝습니다»*(**여의도 3구역**)를 쓴다. 9/4 는 **세종대로 36×40 m 사각형**이다. **M-1 이 가른 바로 그 위험인데 그 판정의 사정권 밖**이었다. **8/28 문면은 건드리지 않았다 — 판정 요청** |
| ③ | ⚠⚠ **이 줄은 M-66-0 에서 정정됐다 — 키는 «실키»(10자)이고 «localhost 도 등록돼 있다». 401 은 없다. 브라우저 실측이 «된다».** ~~**브라우저 실측 0.** 이 환경 `NEXT_PUBLIC_NAVER_MAP_CLIENT_ID=x` 라 **지도가 401 로 대체면만 나온다.**~~ 겹침·터치·200% 는 **전부 «계산 예보»이고 실측이 아니다**(§5.7). ⚠ 소스에 남긴 v1 실측값(2.7px·26.7px·7px²·8.6px)은 **전부 v1 기하**라 «지금 참»이 아니다 — 각 자리에 ⚠ 로 표시해 뒀다 |
| ④ | **코스콤 사각형 북단 ↔ 무대 2 원 남단이 «1.3 m»** 다. 흰 casing 7px 끼리 **반드시 겹친다.** z 순서로 처분했다(사각형 25 > 원 20 — «어디로 가는가»가 «무엇이 있는가»를 덮는다). **시각 판정은 디자이너·QA** |
| ⑤ | ⚠ **§62-9 #54**: 범례 8행 「무대 1 북쪽입니다」가 **«218 m»를 감춘다.** ①이 풀리기 전까지 유효한 경고다 |

### 7. 자체 점검 — 실행한 것

- `npx tsc --noEmit` ⭕ · `npm run lint` ⭕ · `npm run build` ⭕
- ★ **`ROADVIEW_IDS` 가드 «양방향» 실측**: 무대 4 에 켜기 / 임시화장실(남) 끄기 → **둘 다 build 종료코드 1**
  (메시지에 기대·실제 집합 출력). **원복 후 종료코드 0.**
- ★ **프리렌더 기대 개수 32항목 — 불일치 0.** §5.7 대로 **`<script>` 를 먼저 제거**하고 셌다
  (45,484 → 19,412 바이트). §5.2 대로 `grep -c` 를 쓰지 않았다(파이썬 `str.count`).
- ★ **범례 9행의 «사실 주장»을 구현 좌표로 역산 — 8행 전부 §62-4 와 일치**
  (51.2 / 119.5 / 58.3·49.9 / 84.7·56.0 / 217.9 / 95.6 m).
- ★ **클라이언트 번들 문자열 집합 차분**: `로드뷰` 13 · `메인무대` 1 이 남지만 **지문으로 갈라 8/28 청크임을
  확인**했다(`여의도` 5 · `의사당` 2 · `세종대로` **0** · `임시화장실` **0**). **정당한 출현**(§5.8.3).

### 8. §63 판정 반영 (2026-09-01 · 같은 라운드 · dev7)

| 판정 | 반영 |
|---|---|
| **정정 1** 코스콤 한 줄 | **`코스콤지부는 무대 2 구역입니다. 천막과 집회 모두 코리아나호텔(폴바셋) 앞입니다.`** + **근거 3**(좌표 같음 / 원문이 `천막 및 집회:` 로 **한 줄에 함께** 적었다 / §5.3 «따로 있나»를 닫는다)을 주석에. ★ **«위험은 사라진 것이 아니라 이동했다 — «어딘지 모른다» → «거기서 무엇을 하는지 모른다». 그 자리를 채우지 마라»** 를 축자로 넣었다. **새 마커·새 범례 행 만들지 않음**(범례 9행 불변) |
| **판정 1** 역 좌표 | **(가) 역 노드 확정.** ★ **등급이 갈렸다**: 시청역 `verified` · **광화문역 `calculated`**(OSM 단독 · v2 원본 대조 못 함). 기각된 두 안(«출구 두 점» = v1 «줄기» 렌더 특징이라 §5.8.2 / «출구 한 점» = 배지 매칭 실패 → «가장 가까워 보이는 것 고르기»는 날조)을 **이름으로 지목해** 주석에 |
| **판정 1-보** | `fitBounds` 에 광화문역 **포함.** 근거(**368 → 464 → 682 m** · *«빼면 범례 8행이 «지도에 없는 것»을 말한다»* · **§53-15 거리 문구 금지의 «전제»가 «지도가 원근을 보여 준다»** · 대가는 확대가 진다 · **둘을 따로 결정하지 마라**)를 주석에 |
| **판정 2** 문면 2건 | `MAP_SR_INTRO` = **`아래는 세종대로 집회 장소를 표시한 지도입니다. …`** · 대체면 = **`집회 장소는 아래 목록에 있습니다.`** ★ 근거(**나열하면 반드시 하나가 빠진다** · `세종대로`=원문 축자+헤딩 동일어 · `집회 장소`=9개를 전부 덮는다)를 주석에. **«검증 회부 대상» 표시 3곳 전부 «§63 확정»으로 교체** |
| **판정 3** | `±30 m` 확정 + **회귀 감시 대상** 명기 |

#### ★★ 개발자가 «없는 결함»을 보고했다가 스스로 잡았다 — 그 오류를 기록한다

**보고했던 것**: *«z15 에서 무대 2 pill ↔ 임시화장실(북) 배지가 **13.2 px** 라 24 px 미달»* → **틀렸다. 두 겹으로:**
1. **1차원으로 쟀다.** 13.2 px 는 **세로 성분만**이고, 그 쌍은 **가로로 4.7 px 떨어져 애초에 «안 겹친다».**
2. ★ **«장식»을 «대상»으로 셌다.** 무대 pill 은 `roadview` 가 없어 **히트가 아니다** — **WCAG 2.5.8 의 대상 쌍이 아니다.**

**참값**(히트 ↔ 히트 · 상자 2차원 최소 간격): **z15 31.8 px · z16 68.2 px — 겹침 0. 두 줌 다 통과.**
★★ **그리고 z15 가 «더 낫다»** — pill 은 «화면 고정 px», 매달린 지점은 «지리 고정»이라 축척이 거칠어지면
**고정 28 px 간격이 상대적으로 더 벌어진다**(§54.18-1 «두 좌표계»가 여기서도 그대로 작동한다).
→ **줌 한 단의 대가는 «규격»이 아니라 «작아 보인다» 하나이고, 그것은 «확대»가 진다**(판정 1-보와 일치).

⚠ **§5.8.4 ③(«프록시로 판정하지 말고 참값을 재라»)의 새 변종이다** — 거기는 «행 높이로 줄바꿈을 추정»이었고
여기는 **«한 축 성분으로 2차원 간격을 추정»** 이다. ⚠ 그리고 **«대상이 무엇인가»를 먼저 안 갈랐다.**

남는 임계는 **히트 ↔ «장식»**(규격 아님 · 시각): 임시화장실(북) 배지 ↔ 무대 2 pill **z16 2.4 px · z15 4.7 px**.
★ **이 값은 pill 폭 개산과 «무관»하다**(세로 26.4 − pill 높이 24 로 정해진다). **z16 이 이 쌍의 최악이다.**

#### ⚠ 새로 생긴 «못 한 것» — `calculated` 가 «시각 상태»를 못 가진다

9/4 가 이제 **세 등급**(`verified` 시청역 · `calculated` 광화문역 · `estimated` 나머지)을 쓰는데
**시각 상태는 둘**(선종 `solid` ↔ `dashed`)이다. → **`calculated` 가 `verified` 와 «똑같이» 그려지고
「근사 ±30 m」 줄도 안 붙는다.** 조합원에게 **두 역이 같은 확신도로 보인다.**
⚠ **임의로 풀지 않았다**(셋째 선종 신설도, `estimated` 강등도). **디자이너·검증 회부 사항.**
★ `estimated` 로 내리는 것은 **틀린 처방**이다 — 「근사 ±30 m」는 **픽셀 환산 오차 예산**인데
이 좌표는 **환산으로 나온 값이 아니다.**

#### 자체 점검 재실행 (§63 반영 후)

- `tsc` · `lint` · `build` **전부 통과**
- **`ROADVIEW_IDS` 가드 양방향 재확인** — 늘리기/줄이기 **둘 다 종료코드 1** · 원복 후 **0**
- **프리렌더 33항목 불일치 0**(`<script>` 제거 후 · 19,425 B):
  `천막` **1** ⭕ · `코리아나호텔(폴바셋)` **2** ⭕ · `참석명단`·`물품 수령`·`명단 작성`·`지부천막` **각 0** ⭕ ·
  `±25 m` **0** ⭕ · `대오` **1** · `메인무대` **0** · `무대 1` 3 · `추후 안내` 1 · `임시화장실` 2 · `※` 2 · `로드뷰` 0
- ★ **`±30 m` 는 프리렌더에 «없는 것이 정상»이다** — `APPROX_NOTE` 는 **팝업·시트에서만** 그려진다.
  **§5.1 3계층 중 «소스 문자열 집합»에서 따로 셌다**: 클라이언트 번들 **`±30 m` 1 · `±25 m` 0** ⭕
  (함께 확인: `천막과 집회 모두` 1 · `세종대로 집회 장소` 1 · `집회 장소는 아래 목록` 1 · `참석명단`·`지부천막`·`물품 수령` 0)

### 9. §63-2 ④ 반영 — 광화문역을 `fitBounds` 에서만 뺀다 (2026-09-01 · dev7)

```ts
inFitBounds?: false            // StrikeFeatureBase 신규 필드 (기본: 담는다)
const FITTED_FEATURES = STRIKE_MAP_FEATURES.filter((f) => f.inFitBounds !== false);
```
⚠ **`STRIKE_MAP_FIT_BOUNDS` 의 «항목 0건» 방어도 함께 고쳤다** — 판정 기준이 «전체 0건»에서
**«담기는 것 0건»** 으로 바뀐다(안 고치면 전부 `false` 일 때 배열이 비지 않았는데 범위가 없어진다).

| 담는 것 | 남북 | 초기 줌(320~1280) | 코스콤 사각형 |
|---|---|---|---|
| (가) + 광화문역 | 682 m | 320·360·390 은 **z15** | 9.5 × 10.6 px |
| ★ **④ 지금** | **494 m** | **전 뷰포트 z16** | ★ **19.0 × 21.1 px**(면적 4배) |

**마커·pill·범례 8행은 그대로다**(프리렌더 `광화문역(5호선)` 1회 유지 확인).

#### ⚠⚠ 채택 근거에서 «죽은 것» 하나 — 리더가 인용한 검증 문장이 내 오보였다

리더가 ④의 계기로 인용한 검증자 진술 — *«z15 로 떨어지면 지도는 «원근»을 보여 주는 대신
«아무것도 못 누르게» 만든다»* — 은 **내가 §8 에서 이미 철회한 주장**이다(1차원 측정 + 장식을 대상으로 셈).
**참값: 히트↔히트 최소 z15 31.8 px · z16 68.2 px — 두 줌 다 통과이고 그 축에서는 z15 가 오히려 낫다.**

★★ **그런데도 ④는 «그대로 선다»** — 오보와 무관한 근거가 둘 있다:
1. ★ **리더가 준 것**: ④는 bounds 를 **좁히는** 방향이라 **축척이 «같거나 올라간다»** —
   **계산 예보가 틀려도 (가)보다 나빠질 수 없다.** (그래서 QA 실측을 기다리지 않고 넣었다.)
2. ★★ **사용자 요구**: 이 지도의 목적이 «코스콤이 무대 2»를 보여 주는 것인데
   **가장 중요한 도형이 두 배(면적 4배)가 된다.**
→ **판정은 유지하고 «근거만» 다시 썼다**(소스 주석에 «인용하지 마라»로 못박음).

⚠ **대가도 적는다**: ④가 전 뷰포트를 z16 으로 만들었으므로 **히트 ↔ «장식» 최악값 2.4 px 가 «상시 조건»**
이 된다(z15 이면 4.7 px 이었다 · 임시화장실(북) 배지 ↔ 무대 2 pill · 규격 아님 · 시각). **QA 실측 대상.**

#### ★ 근거를 다시 쓴 것 2곳 (값은 유지)
| 대상 | 죽은 근거 | 지금 |
|---|---|---|
| `FIT_PADDING.top = 48` | *«광화문역 pill 이 그 점 위에 서야»* — ④로 광화문역이 bounds 밖이 되어 **북쪽 극점은 무대 1 원**이고 그 pill 은 `left` 라 위 공간을 안 쓴다 | **건드리지 마라**(리더) — 줄이면 **§54.16-3 «서쪽 여유 27 px»가 처음부터 다시 계산돼야 한다** |
| 광화문역 `placement: "top"` | 근거가 **두 겹으로** 죽었다(«두 점 위» → 점이 하나 / «최북단» → bounds 밖) | **값 유지 + 디자이너 회부**(아래) |

#### ★★ 새 «못 한 것» — 광화문역 pill 이 초기 화면 밖이다 (디자이너 회부)

pill 은 `placement:"top"` · gap 14 라 **점보다 38 px 더 위**다. 계산 예보:

| 뷰포트 | 점(장식) | **pill = 이 항목의 «유일한» 히트** |
|---|---|---|
| 320 / 360 / 390 px | 위로 47.7 / 22.7 / 4.0 px 밖 | **완전히 밖** |
| 412 px | 안(9.8 px) | ★ **완전히 밖**(4.2 px 모자람) |
| 430 px | 안(21.0 px) | 아래 **7 px 만** |
| 768 px 이상 | 안(34.8 px) | 24 중 **20.8 px** |

⚠⚠ **휴대폰에서는 광화문역 거리뷰에 «드래그 없이» 닿지 못한다.**
★ **«정보»가 가려진 것은 아니다** — 위치는 **범례 8행**이 그대로 말한다(§0.4 판정 충족).
가려진 것은 **«거리뷰 진입점»** 이고 남는 경로는 **드래그·축소**다(M-14 · `MAP_GESTURE_NOTE` 가 안내).
★ **`placement` 를 `"bottom"` 으로 바꾸면 pill 이 38 px 안으로 들어온다**(z16 에서 무대 1 은 115 px
아래라 안 덮는다). **실익이 있으나 개발자가 임의로 정할 값이 아니다 — 디자이너 판정 요청.**

#### 낱말 충돌 — **리더 판정 «고치지 마라. 양쪽 다». 접수. 손대지 않았다**

#### 자체 점검 (④ 반영 후)
- `tsc`·`lint`·`build` ⭕ · **가드 양방향 재확인**(늘리기·줄이기 **각 종료코드 1** · 원복 **0**)
- **프리렌더 33항목 불일치 0** — bounds 변경이 **문면을 하나도 건드리지 않았음**을 확인
  (`광화문역(5호선)` 범례 행 **1회 유지** · `천막` 1 · `대오` 1 · `메인무대` 0 · `±25 m` 0)
- 번들 층: `±30 m` **1** · `±25 m` **0**

#### §65 반영 (2026-09-01 · dev7) — 판정 3건

**1. ④ 유지 · 근거 통째로 교체.** 코드 무변경, 주석만.
⛔ **회수**: `13.2 px` · 「(가)는 24 px 규격 위반」 · 「저울이 뒤집힌다」 → 소스에 **«인용 금지»** 로 못박음.
★ **새 근거는 «판독» 축**(회수된 «규격» 축과 **다른 축**이라 결론이 살아남는다):
```
z16 ≈ 1.893 m/px → 코스콤지부 구역 36 m ≈ 19.0 px
z15 ≈ 3.787 m/px →                       9.5 px
```
*그 사각형이 이 지도에서 유일하게 「조합원이 서는 자리」다. 맥락 지물 하나를 첫 화면에 담으려고
행동 대상을 반으로 줄이지 않는다.*
★ §63 판정 1-보의 「빼면 범례 8행이 지도에 없는 것을 말한다」를 **조건부**로 적었다:
① 범례 8행이 텍스트로 위치를 말한다 **참** / ② 마커가 드래그로 닿는다 **미확인 — QA 실측** ·
**②가 거짓이면 ④를 다시 연다** · *조건 없이 「은폐 아님」만 적지 마라.*

**2. 「남는 임계」 재측정 — 축 혼동 정정.**
종전 보고(`z16 2.4 px · z15 4.7 px` 병기)는 **두 수를 서로 다른 축으로 «서술»해** 비교 불가로 읽혔다.
→ **히트↔히트에 쓴 그 함수(축정렬 상자 2차원 최소 간격) 하나로 전 쌍 55개를 다시 쟀다.**
★ **④ 확정이므로 z16 이 「최악」이 아니라 「첫 화면」이다:**

| | 값 |
|---|---|
| **전 쌍 최소** | **2.4 px — 겹침 «없음»**(무대 2 pill ↔ 임시화장실(북) 배지 · 히트↔장식) |
| 히트↔히트 최소 | 68.2 px (코스콤 pill ↔ 화장실북 배지) |
| 다음 3쌍 | 6.9 / 8.0 / 9.7 px (전부 히트↔장식) |

★ **양수 = 무대 2 pill 은 가려지지 않는다.** 음수였다면 시각이 아니라 **기능** 문제였다.
⚠ pill 폭 개산 기반 **계산 예보** — QA 실측 대상. (2.4 px 자체는 폭과 무관: 세로 26.4 − pill 높이 24.)

**3. `calculated` — 셋째 상태 없음 · 강등 없음 · 판정을 «코드 형태»로.**
```ts
export const CONFIDENCE_VISUAL: Readonly<Record<StrikeGeoConfidence, StrikeConfidenceVisual>> = {
  ...
  calculated: { stroke: "solid",  approxNote: false },  // ★ 누락이 아니라 판정(§65-3)
  estimated:  { stroke: "dashed", approxNote: true  },
};
```
★ **주장을 실측으로 확인했다** — `StrikeGeoConfidence` 에 등급 하나(`surveyed`)를 더하니
**`tsc` 가 `TS2741`(`Property 'surveyed' is missing`)로 죽는다.** 「등급이 늘면 타입이 빌드를 죽인다」가 참이다.
**우연한 2분기 4곳을 전부 매핑 경유로 배선했다:**
`StrikeMapPopup`(「근사」 줄) · `toiletBadgeHtml`(**하드코딩 `dashed` 제거**) · `openRoadviewHere`(`approximate` →
`visual`) · `wardHtml`(선종). `StrikeRoadviewSheet` 의 `approximate` prop 주석도 매핑 파생임을 명시.
주석에 **두 축 표**(확신도=우리가 출처를 몇 개 봤나 → **§5.3 우리 작업 사정** / 점선·「근사」=지물이 실제로
거기 있나 → **행동이 갈린다**)와 *「임시화장실은 정반대다 — 그 자리에 가서 못 찾을 수 있다」* 를 넣었다.

**4·5** 낱말 충돌 «닫힘» 접수(무변경) · 401 은 리더가 본다(더 파지 않음).

**자체 점검**: `tsc`·`lint`·`build` ⭕ · 가드 양방향(**각 1** · 원복 **0**) ·
프리렌더 **33항목 불일치 0**(`광화문역(5호선)` 범례 행 1회 유지) · 번들 `±30 m` 1 · `±25 m` 0.

#### §66 반영 (2026-09-01 · dev7) — **실측 라운드. 「계산 예보」는 끝났다**

측정 환경(§5.5): `360×640` · **스크롤바 0** · `dpr 1` · `visibilityState visible` · 루트 `12px` ·
`documentElement.scrollWidth 360` · 지도 상자 **336 × 420** · **초기 줌 z16** · Playwright(리더가 띄운 3000 사용 · **죽이지 않았다**)

**4. WCAG 2.4.7 결함 — 고쳤다.** `StrikeMap.tsx` 에 `focusin` 위임 effect 신설.
히트가 포커스를 받았을 때 그 rect 가 상자 안에 **완전히** 들어있지 않으면 `panTo`(reduced-motion 이면 `setCenter`)로 옮기고 `setMoved(true)`.
`naverMaps.ts` 에 **`panTo`·`setCenter` 선언 추가**(둘 다 공식 문서 API — §54.17-4 「문서 없는 API 0개」 유지).

| 실측 | 결과 |
|---|---|
| 포커스 «전» 광화문역 히트 | `y 796.2–820.2` · 상자 top `852.2` → **32 px 위 · 가시 면적 0 px²** · `fullyInside: false` |
| 포커스 «후» | `y 1024.2–1048.2` ∈ 상자 → ★ **`gwNowInsideBox: true`** · 가시 면적 **2,161 / 2,161 px²** |
| 실제 탭 경로(`preventScroll` 없이) | 동일하게 `true` — **브라우저 자동 스크롤에 가려지지 않는다** |
| **가드** | 이미 안에 있는 히트 4개(`koscom-area`·`toilet-north`·`toilet-south`·`cityhall`) 포커스 → **지도 이동 0.0 px** |
| 복귀 경로 | 「처음 위치로」 버튼 **나타남** · 포커스 유지(`activeIsGw: true`) |

⛔ `aria-hidden`·`tabindex="-1"` 로 숨기지 **않았다**(그러면 키보드 사용자만 광화문역 거리뷰를 못 쓴다).

**1·3. 예보가 실측으로 확증됐다** — 초기 줌 **z16** · 히트 **5개** · 히트↔히트 최소 **68.0 px**(예보 68.2 · **오차 0.2**) ·
코스콤 사각형 **19 × 21 px**. §65-1 판독 축 근거는 안 흔들린다.

**2. ⚠ 「남는 임계」 — 리더 §66-2 의 참값 지정을 정정한다(실측)**

| | 값 |
|---|---|
| **전 쌍 최소** | ★ **2.0 px** — `toilet-north` [히트] ↔ **무대 2 pill** [장식] · **겹침 없음** |
| 2~4위 | 7.0 / 10.0 / 20.9 px (전부 히트↔장식) — 예보 6.9 / 9.7 / 21.5 |
| 히트↔히트 최소 | **68.0 px** |

⚠⚠ **리더 §66-2 의 *「무대 2 에 pill 이 있다고 가정한 것이다. 없다」* 는 틀렸다 — 실측으로 «있다»:**
**무대 pill 4개가 렌더된다**(`45×24`·`46×24`·`47×24`·`47×24` · `hit=false` 장식).
★ **원인은 `[data-strike-hit]` 로 세면 «5개»가 나오는 것** — **그 쿼리는 «히트»만 센다.** 장식 pill 은 안 잡힌다.
→ 그래서 §66-2 가 제시한 참값 **3.0 px(무대 2 «원»)** 은 **더 «느슨한» 쌍**이다. **최근접은 2.0 px(pill).**
★ **결론은 양쪽 다 같다** — 빠듯하지만 **겹침 0** · 시각이고 **규격 아님**.
★ 그리고 내 예보 **2.4 px** 는 «폐기»가 아니라 **실측 2.0 px 에 0.4 px 근접**했다 — 계열도 맞았다.

**5.** 조건 ② — 포인터는 원래 닿았고(드래그 120 px · 리더 실측) **키보드는 못 닿았다.** 4번으로 **양쪽 다 참**이 됐다. 소스에 그렇게 적었다.
**6.** 종횡비·`FIT_PADDING` **안 건드렸다.** 실측값만 주석에: **상자 높이 ≥ 453 px 이면 z16 에서 9개 전부 담긴다**
(682.4 / 1.8934 = 360.4 + 92 = 452.4) · 현재 **33 px 부족** · **이번에 고치지 마라**(축척 → 서쪽 27 px 사슬).
**0·7.** 401 없음 접수(「잴 수 없다」 전제 판단 전부 재개) · `layout.tsx` `<script>` 경고 **손대지 않았다**.

**자체 점검**: `tsc`·`lint`·`build` ⭕ · 가드 양방향(**각 1** · 원복 **0**) · 프리렌더 **33항목 불일치 0** ·
측정 위생: `localStorage` 글자크기 **미설정(오염 0)** · 인라인 스타일·프로브 **미주입** · 측정 후 **reload 로 원복**.

#### §67 반영 (2026-09-01 · dev7)

**1. 리더 §66-2 철회 접수 — 주석을 「누가 틀렸나」가 아니라 「셈법 표」로 적었다.**
이 한 쌍에서 **「세는 방법이 질문에 안 맞는」 사고가 «두 번»** 났고 **둘 다 「무대 pill 이 없다」로 끝났다:**

| 셈법 | 왜 놓쳤나 |
|---|---|
| `querySelectorAll('[data-strike-hit]')` | **히트만** 센다 → 5개. 장식 pill 은 애초에 안 잡힌다 |
| 라벨을 **`span` 으로만** 훑기 | ★ **무대 pill 은 `<div>` 다**(`pillHtml` 장식 분기). 히트 pill 만 `<span role="button">` |

★ 참값은 **태그 무가정 전수**로만 나온다 — 무대 pill **4개 렌더**(45~47 × 24 px · 장식) · 최근접 **2.0 px**.

**3. `placement: "top"` 유지 — 근거를 「바꿀 시간이 없다」로 못박았다.**
*「근거가 죽었으니 바꾸자」가 아니다*: 접근성 결함은 `focusin` effect 가 **이미 닫았으므로** 남은 것은
**「초기 화면 미관」**이고, `"bottom"` 은 **충돌 지형을 통째로 바꿔** `320·360·390·412·768·1280 × 기본·200%` 를
전부 다시 재야 한다(지금 히트↔장식 여유 **2.0 px**). → **후속 #30 합류**(상자 높이 33 px 건과 **같은 사슬**).

**5.** `calculated` — §65-3 그대로 이미 반영돼 있다(3항 명시 매핑 + 「누락이 아니라 판정」 한 줄 + 등급 추가 시 `TS2741`).

#### ★★ 측정 사고 1건 — 내가 냈고 내가 잡았다 (§5.8.4 ③ 새 변종)

가드 확인에서 **「지도 이동 400 px」** 이 나와 「가드가 깨졌다」로 보고할 뻔했다.
**원인: 지도 이동을 «뷰포트 절대좌표»로 쟀다.** 그런데 직전 포커스가 **페이지를 488 px 스크롤**시켜
**상자 자체가 움직여 있었다** — 잰 것은 **지도 이동이 아니라 페이지 스크롤**이다.
★ **참값은 «상자 기준 상대좌표»** 다(상자도 페이지와 함께 움직이므로 차분에서 스크롤이 소거된다).

| 방법 | 이미 안쪽 히트 4개 포커스 시 |
|---|---|
| 뷰포트 절대좌표(오염) | **400 px** ← 거짓 |
| ★ **상자 기준 상대좌표(참값)** | ★ **0.0 px — 가드 정상** |

⚠ **§5.8.4 ③(「프록시로 판정하지 말고 참값을 재라」)의 새 변종이다** — 거기는 «행 높이로 줄바꿈 추정»이었고
여기는 **«스크롤하는 문서 안에서, 자기도 스크롤되는 상자 «안» 요소의 이동을 절대좌표로 잼»** 이다.
**«무엇이 움직였나»를 물을 때는 «무엇을 기준으로»를 먼저 정한다.**

#### 최종 실측 (`360×640` · 스크롤바 0 · dpr 1 · 루트 12px · docScrollWidth 360 · z16 · 아래 해시 고정)

| | 결과 |
|---|---|
| 광화문역 포커스 «전» | `y 796.2–820.2` · 상자 `y 852.2–1272.2` · **가시 0 px²** · `fullyInside: false` |
| 포커스 «후»(실제 탭 경로 · `preventScroll` 없이) | ★ **`gwNowInsideBox: true`** · 가시 **2,161 / 2,161 px²** · 지도 이동 **228 px** |
| **가드**(이미 안쪽 히트 4개) | ★ **지도 이동 0.0 px** — 탭 이동마다 흔들리지 않는다 |

```
f3e4d48fac77…  src/lib/strikeMap.ts
0d517ea83820…  src/lib/naverMaps.ts
ddd393b4b602…  src/components/strike/StrikeMap.tsx
544b652aeae1…  src/components/strike/StrikeRoadviewSheet.tsx
70ec1cb45459…  src/app/strike-2026-09-04/page.tsx
```
`tsc`·`lint`·`build` ⭕ · 가드 양방향(각 **1** · 원복 **0**) · 프리렌더 **33항목 불일치 0**.

#### §68 반영 (2026-09-01 · dev7) — 주석만. 렌더 산출물 **완전 일치**로 확인

**낡은 근거 1건**: `page.tsx` 「빠져 있는 것」 목록의 `천막` 항목이 *「위치를 알려 주지 않았다」·「한 줄도 없다」·
기대 `천막` **0*** 을 말하고 있었다. **셋 다 사실과 어긋났다**(사용자 축자 「천막도 코리아나호텔 앞」 ·
`KOSCOM_LOCATION_NOTE` 에 있다 · **렌더 1**). ★ **같은 라운드에 같은 문자열의 기대값이 두 개**였다 —
내 프리렌더 점검표는 **1**, 이 주석은 **0**.

**처분**(숫자만 바꾸지 않았다):
1. `천막` 을 「빠져 있는 것」 목록에서 **뺐다** — 그 목록의 계약이 *「자리도 만들지 않았다」* 인데 **자리가 생겼다.**
2. 바로 위 「코스콤지부 위치」 항목에 붙이고 **세 처분 표**를 만들었다 —
   **위치 = 생겼다 / 천막 = 위치가 확인돼 생겼다 / 대오 = 사라졌다.** 종전 두 처분에 **셋째**가 붙었다.
3. **계층과 함께** 적었다(§5.1): `천막` **렌더 1**(`KOSCOM_LOCATION_NOTE` 안) · `참석명단` **0** · `지부천막` **0**.
   ⚠ **`지부천막` 0 은 유지** — 8/28 낱말이고 9/4 근거는 여전히 0. **`천막` 과 한 덩어리로 읽지 마라.**
   ★ **«천막이 어디인가»는 답이 나왔고 «천막에서 무엇을 하는가»는 근거 0** — **위험은 «이동»했다.**
4. **M-58 · §62-7 인용을 남기고** *「이 근거는 사용자 확인으로 소멸했다」* 를 명시 — 지우면
   *「왜 천막을 오래 안 썼지」* 가 다음 사람의 일이 된다.

⚠ ★ **`추후 안내` 사고의 재판이다** — 기대 0 으로 적힌 문자열을 **나중에 확정된 문면이 포함**해
**정상 문면이 실패로 올라갈 뻔했던** 그 건(§5.9.3). **이번엔 지우는 대상이 «사용자 축자»라 한 걸음 더 나쁘다.**
⛔ `KOSCOM_LOCATION_NOTE` 문면 **무변경**(1회 그대로) · 본문에 **둘째 자리 안 만들었다**(요구 88).

**검증**: `tsc`·`lint`·`build` ⭕. ★ **주석만 바뀌었음을 «값으로» 보였다** —
프리렌더 가시 마크업(`<script>` 제거) sha256 **`7fe7130407ee…` 17,911자** 가 §68 **전후 완전 일치**.
계수: `천막` **1** · `참석명단` 0 · `지부천막` 0 · `대오` 1 · `추후 안내` 1.
`page.tsx` 해시 `70ec1cb45459…` → **`b9697ec90994…`**(QA 재고정 필요 · 통보함).

#### M-69 최종 게이트 반영 (2026-09-01 · dev7) — 3건

**1. ★ 식순 2행 «묶음» 복원.** ★ **원본(`design/총파업_식순.jpg`)을 직접 열어 재확인했다** —
`사전집회` 아래가 **줄바꿈**이고 그 사이에 ` / ` 가 **없다.** 원문은 `사전집회` 를 **그 시간대의 「이름」**으로
세우고 둘째 줄이 내용을 편다. 한 줄로 이으면 셋이 **대등한 3항 나열**이 되어 위계가 평평해진다.
```
content: "사전집회\n구호제창 / 동영상 상영/경과보고"   +  <td class="… whitespace-pre-line …">
```
★★ **오염 경로를 주석에 적었다** — M-53·§61-5 의 *「**두 줄**: `사전집회` / `구호제창 / …`」* 에서 그 `/` 는
**「1줄 / 2줄」을 가르는 «우리 표기»** 였는데 세 단계를 거치며 **원문의 일부로 굳었다.**
⚠ **여기서 배운 것**: **「한 글자도 바꾸지 마라」는 지시도 «원문»이 아니라 «지시 문면»일 수 있다 —
원본을 열어야 갈린다.** 이 프로젝트가 §5.8.1 로 세운 「도착지를 직접 본다」의 **문면 판**이다.
⛔ `상영/경과보고` 공백 없음 유지 · 3열 복귀 금지 · 「비고」 복원 금지. `\n` 은 **이 한 행뿐**이다.

**실측(360)**: 렌더 문자열 `"사전집회\n구호제창 / 동영상 상영/경과보고"` · `white-space: pre-line` ·
★ **줄 상자 «2개»**(39px + 142px · `Range.getClientRects()` 로 «줄 상자»를 직접 셌다 — 행 높이 휴리스틱 아님) ·
셀 높이 **53px**(다른 행 36px) · **가로 스크롤 0**.
★ **프리렌더 식순 20행 전수 대조 — 불일치 0**(`<td>` 경계로만 자르고 태그 무가정).

**2. `±25 m` → `±30 m`** (`StrikeMap.tsx` 와드 주석 · §63-4 · `APPROX_NOTE` 와 같은 값). 판정 유지 · 수치만.
★ 남은 `±25` **6곳은 전부 「죽었다·인용하지 마라」 회수 기록**이라 정당한 출현이다(§5.9 계보).

**3. ★★ 실재하지 않는 절 번호 — 소스 인용 23곳을 실재 자리로 고쳤다**
| 소스 | → | 곳 |
|---|---|---|
| `§65-1` | `M-66-2` | 2 |
| **`§65-3`** | **`M-70`** | **9** |
| `§66-1` | `M-66-1` | 1 |
| **`§66-4`** | **`M-66-5`**(WCAG 2.4.7) | 4 |
| **`§66-5`** | **`M-66-4`**(드래그 도달) | 2 |
| `§66-6` | `M-66-6` | 2 |
| `§67-3` | `M-67-4` | 2 |
| `§68` | `M-69` | 1 |
⚠ **뒤바뀐 두 항목 때문에 «단일 패스» 정규식 치환**을 썼다(순차 치환이면 4↔5 가 서로 틀린 자리를 가리킨다).
★ **치환 «전»에 6곳의 문맥을 전수 확인**해 방향이 맞는지 봤다 — `§66-4` 4곳이 전부 «WCAG 결함», `§66-5` 2곳이
전부 «드래그 도달»이었다.
★ **`M-70` 을 읽고 내 `CONFIDENCE_VISUAL` 과 대조했다 — 어긋난 곳 0.**
(3항 명시 매핑 · `calculated` = 실선·「근사」 없음 · 한 줄 주석 · 등급 추가 시 `TS2741`. M-70-5 가 요구한 형태 그대로.)

**★ 함께 잡은 것 — 내가 낸 «귀속 오류» 8곳**(리더 지적 밖):
`§63-2 ④` 로 6곳에서 인용했는데 **④를 채택한 절은 §64-1**(리더 **M-63**)이고 **§63-2 는 그것이 «개정한» 죽은 절**
(*「광화문역을 `fitBounds` 에 넣는다」*)이다. → `M-63 ④` 로 고치고, **§63-2 를 ④의 근거로 인용하지 마라**를 명시.
함께: *「§0.4 판정(§63-2 개정)」* → **`M-67-5`** · *「리더 §63-2」*(종횡비 금지) → **`M-66-6`** ·
*「(QA 실측 · M-66-5)」* → **「(리더 실측 · M-66-5)」**(M-66-5 는 리더 실측이다).
⚠⚠ **그리고 §64-1 이 적은 ④ 채택 근거 하나가 죽었다** — *「z15 면 13.2 px 로 24 px 미달」* 은 **내 오보**였다
(M-66-2·M-67 정정 · 참값 68.0 px). **소스에 「그 근거는 죽었고 ④는 «판독» 근거로 선다」를 못박았다.**

**4. M-71 — 「구판」의 «범위»를 적었다**(`strikeMap.ts` 머리): **v2 자료 4장은 «식순·지도»만 대체했다.**
그 4장에 「2. 개요」·「3. 집결시간」이 없어 **v1 전사본의 그 부분은 살아 있는 출처**다. 문자 그대로 적용하면
**출처가 사라진다.** ⚠ 다만 v1 전사본 6행의 「식순 20행 전부 일치」는 **구판 식순에 대한 확인**이라 인용하면 거짓.

**검증**: `tsc`·`lint`·`build` ⭕ · 가드 양방향(**각 1** · 원복 **0**) · 프리렌더 식순 20행 + 기대 개수 14항목 **불일치 0**.
```
78eddeb08808…  src/lib/strikeMap.ts            e18d0f140a18…  src/lib/naverMaps.ts
...
#### QA §67 회차 종료 (2026-09-01) — **실패 0 · 수정 요청 0.** 개발 측 종료

QA 동결 기준이 내 최종 해시와 **일치**: `strikeMap.ts 78eddeb0` · `naverMaps.ts e18d0f14` ·
`StrikeMap.tsx 91db83c1` · `StrikeRoadviewSheet.tsx 01de8082` · `page.tsx e7466e11` · 프리렌더 `2c4be85b` 18,410자.

**D(포커스 표시) 독립 재현 — 6폭 × 5정지점 30/30.** rect 도 링(`rect+4px`)도 **100% 상자 안**(최소 여백 14.8 px).
히트↔장식 **2.0 px** · 히트↔히트 **67.98** · 다음 3쌍 **7.0 / 10.0 / 20.88** — 내 값과 전부 일치.

##### ★★ 내가 넘긴 함정은 피했는데 QA 가 «다른» 함정에 걸렸다가 잡았다 — 같은 계보 3건째

가드 검증에서 **「가드가 228 px 깨졌다」**가 나왔다. 원인: **`cityhall` 로 가는 탭 경로가 `gwanghwamun` 을
반드시 통과**한다. **탭 «한 칸마다» 차분**을 내니 갈렸다:
```
tab 4·5·6 (koscom-area·toilet-north·toilet-south) →  0.00 · 0.00      ← 가드 정상
tab 7     (gwanghwamun)                           → +2 · +226         ← 의도된 panTo
tab 8     (cityhall)                              →  0 · −352         ← 7번 팬으로 cityhall 이 «진짜로» 밖이 돼서 옮긴 것
```
★ **「누적 차분」이 아니라 「단계 차분」이어야 했다.** 내 함정(뷰포트 절대좌표 = **기준계**가 틀림)과
**축이 다르다** — 이쪽은 **«구간»이 틀렸다**(여러 전이를 한 차분에 뭉쳤다).
→ 이 라운드 「세는 방법이 질문에 안 맞는」 계보에 **하나 더**: **기준계 · 대상 · 태그 · «구간»** 네 축이 됐다.

##### ★ QA 가 낸 «주석에 없는» 새 사실 3건 (리더 판단 대기 — 반영은 안 했다)

`320` 폭 **+ 텍스트 200%** 에서만 **z15**(상자 256×320 · 3.79 m/px):
| | 값 | 내 소스 주석 |
|---|---|---|
| 히트↔히트 | **32.25 px ≥ 24** 통과 · 겹침 0 | 없음(z16 만 적혀 있다) |
| 히트↔장식 | **4.0 px** — z16(2.0)보다 **오히려 넓다** | 없음 |
| ★★ **갈리는 «축»이 바뀐다** | **z16 = 세로 · z15 = 가로** | 없음 |
| ★ 히트 5개가 **전부 상자 안**(광화문역 포함) | | `inFitBounds` 표는 **기본 배율**만 다룬다 |

⚠ **내 소스가 «틀린» 것은 아니다**(그 표는 기본 배율 기준이라고 적혀 있다) — **비어 있는 것**이다.
★ 특히 **「광화문역이 320+200% 에서는 초기 화면 안」**이면 그 배율에서는 `focusin` panTo 가 **발동하지 않는다.**
결함이 아니라 **정상 거동**이지만, 다음 사람이 「왜 안 움직이지」로 볼 자리다.
→ **주석 3~4줄이면 채워진다.** ⚠ **QA 가 회차를 닫은 직후라 임의로 저장하지 않았다 — 리더 판단으로 넘겼다.**
★ 비용은 낮다: **주석은 번들에 도달하지 않음을 실증**했으므로(지도 청크 `79b75364`·`b7c91f5a` 바이트 동일)
**QA 의 지도 측정 A·C·D·E 는 다시 잴 필요가 없다.** 소스 해시만 갱신하면 된다.

##### 커밋
**하지 않았다.** 지시 대기(소스 5개 + `03_developer_impl.md` 가 미커밋 상태).

#### 대기 배치 적용 (2026-09-01 · QA §69 «런타임 문면 계층» 종료 뒤)

QA 가 그 항목을 닫아(불일치 0 · 미검증 1건은 «통과로 적지 않음») 대기하던 **주석 배치 4건을 한 번에** 넣었다.
**앵커 5개는 적용 «전»에 드라이런으로 검증**해 뒀다(실패 회차를 또 만들지 않으려고).

| # | 대상 | 분량 |
|---|---|---|
| 1 | `inFitBounds` 주석에 **배율 축** — `320`+200% z15 · **히트 5개 전부 상자 안** · **그 배율에서 panTo 가 «안 도는 것이 정상»**(⚠ 가드를 뜯지 마라) · 히트↔장식 4.0 > z16 2.0 · **갈리는 축이 z16 세로 → z15 가로** | 순증 4줄 |
| 2 | `코스콤 천막` 항목 — M-61 반영. **초안 13줄 → 5줄**(리더 §71-3: 「하지 마라 + 포인터」) | 5↔5 교체 |
| 3 | `03_developer_impl.md` **2셀 회차 표식** — ① `13.2 px`(M-66-2·M-67 정정) · ③ **`401` 주장**(M-66-0) | 2셀 |
| 4 | `StrikeRoadviewSheet.tsx` `범례 13행` → **`9행`** | 한 낱말 |

★ **③ 은 리더 지정 밖이라 «근거를 대고 물어» 승인받았다** — 같은 표·같은 결함류에 표식을 하나만 달면
**「절반만」**이 된다(이 라운드에 세 번 본 형태). 리더 판정: *「③이 ①보다 «더» 중요하다 — ①은 낡은 수치이고
③은 «측정 자체가 불가능하다»는 «거짓 주장»이다. 그 한 줄이 이 라운드를 통째로 계산 예보로 굴렸다.»*
→ 문면도 리더 것으로 바꿨다: **「없는 것」이 아니라 「할 수 있는 것」**(*「401 은 없다. 브라우저 실측이 «된다».」*).

★ 4번에서 `(v1 은 13행이었다)` 를 **뺐다** — `StrikeMap.tsx` 에 이미 있어 **두 벌**이 된다(요구 88).
★ 행 번호 대신 **심볼 이름**(`KOSCOM_LOCATION_NOTE`) — 승인. *「썩는 쪽을 아예 안 적는 것이 «한 출처»다.」*

**검증**: `tsc`·`lint`·`build` ⭕ ·
★★ **지도 번들 «바이트 동일»** — `79b75364b6da…`(52,734 B) · `b7c91f5a0b05…`(30,427 B) = **QA §69 기준과 일치.**
**A·C·D·E 재측정 불필요.** 소스 해시만 2개 이동: `strikeMap.ts ced9d729…` · `StrikeRoadviewSheet.tsx a539f449…`
(나머지 3개 불변 — `page.tsx e7466e11` 이 그대로라 **프리렌더도 불변**).

##### O7 주석 (리더 판정 · 후속 #32) — ★ 그 자리의 «종전 사유»가 «거짓»이었다

리더가 「현행 유지 + 근거를 주석에 한 줄」로 판정했다. **넣기 «전»에 그 자리의 기존 문장을 코드와 대조했더니
`closeStreetView` 의 종전 사유 *「길만 남으면 눌러도 열 것이 없다」* 가 «거짓»이었다:**
모드가 켜진 채 시트만 닫히면 `streetAt === null` · `panoRef.current === null` 이라 길 클릭이
`streetMode` 리스너의 **`else` 분기 → `setStreetAt(...)`** 로 가고 **시트가 «열린다».**

**리더가 «독립으로» 같은 진단에 도달했고**(`streetMode` 효과의 `else` 분기를 직접 읽었다) 처분을
**「근거만 교체 · 한 줄 · 순증 금지」**로 지시했다. → **최종은 1줄 교체다:**
```
/** 시트를 닫는다. **거리뷰 모드도 함께 끈다** — 남으면 «팝업을 열려던 다음 탭»이 거리뷰를 연다(1차 목적은 위치 안내 · 후속 #32) */
```
⚠ **초안에서 «죽은 절을 이름으로 지목»했다가 뺐다.** 내 근거는 *「거짓 문장 옆에 참 문장을 두면 어느 쪽을
믿을지 판정해야 한다」* 였으나, **이 문장은 «게시된 판정»이 아니라 «내부 사유»이고 «교체»되므로 재인용 대상이
남지 않는다.** 그리고 **거짓 문장을 파일에 남기는 것 자체가 위험**이다 — 지목 표식이 있어도 훑는 사람은 본문을 읽는다.
★ **«죽은 절 표식»은 «뒤집힌 판정»에 쓰는 것이지 «처음부터 틀린 내부 사유»에 쓰는 것이 아니다** — 오늘 배운 구분이다.
★ 리더 사유가 코드와 맞는다: 모드가 남으면 **길 클릭이 거리뷰를 열므로**, 마커를 노린 탭이 빗나가면
**팝업 대신 거리뷰가 열린다.** 이 지도의 1차 목적은 위치 안내다.

**최종 해시**(`tsc`·`lint`·`build` ⭕ · **지도 번들 `79b75364b6da`·`b7c91f5a0b05` 바이트 동일 = QA §69 기준**):
```
ced9d729bd27…  src/lib/strikeMap.ts                          e18d0f140a18…  src/lib/naverMaps.ts (불변)
857aaff004ad…  src/components/strike/StrikeMap.tsx           a539f4490b6d…  src/components/strike/StrikeRoadviewSheet.tsx
e7466e110ec4…  src/app/strike-2026-09-04/page.tsx (불변 → 프리렌더 불변)
```

---

## V3 라운드 — 조합원 상세일정 · 섹션 개명 · 원본 사진 (2026-09-01 · dev7)

**정본**: `V3-PLAN.md`(자족적 확정본) · `V3-IMAGES.md` · v3 전사본.

### 1. 「총파업 일정」 — 한 섹션 · 두 패널 (§5-1)
`MEMBER_SCHEDULE` 6행 + `ATTENDANCE_NOTE`(※) 신설 · **1열 스택 `<dl>`**(표 아님) · 패널 B 는 기존 식순 표 **무변경**.
★ **축자를 «전사본에서 바이트 추출»했다** — 손으로 옮기지 않았다. 대조 결과 **셀 18개 + ※ 전부 일치**.
`~` 는 6행 전부 **ASCII U+007E** 로 확인(전각이면 `TimeText` 가 `<wbr>` 을 못 넣는다 · §8-2 함정).

### 2. ⚠ 내가 잡은 것 2건
| | |
|---|---|
| ★★ **`text-h3` 는 «존재하지 않는 토큰»이었다** | 내가 처음 그렇게 썼는데 `globals.css` 에 없다 — **Tailwind 가 조용히 무시해 크기가 상속된다.** 이 저장소의 `<h3>` 선례는 **`text-lead`**(`bargaining-2026` ×2)라 그것으로 바꿨다. **토큰을 임의로 만들지 않는다** |
| ★★★ **§3-A 의 `15:15` 기대값 «4» 가 틀렸다 — 참값 «5»** | 프리렌더 전수로 문맥을 갈랐다: 식순 2(`15:10-15:15`·`15:15`) + v3 **3**(`11:30~15:15` · `15:00~15:15` · `15:15~17:40`). **§3-A 가 `11:30~15:15` 행을 안 셌다**(§4-A A-6 이 그 행을 게시 대상으로 열거하는데 계수에서 빠졌다). ⚠ **다음 게이트가 그 표로 세면 «거짓 실패»다** |

### 3. 게시 조건 이행
- **28 발동** — QR 한 줄 **둘째 문장만** 삭제. 첫 문장 유지(2회로 강해진다) · 새 자리·`<br>` 분할 없음 · 시각 재기재 0.
- **27 개정** — `※` 종류 2 · 출현 3. 둘 다 **원문 축자** · 우리가 짓는 `※` 는 여전히 0.
- **66** — A-5 **쌍둥이 3벌**(`page.tsx` · `strikeMap.ts` ×2)을 **한 번에** 고쳤다.
- **65(부분)** — 코드 쪽 회수만 했다: **A-1**(★ 내 §1-1 수정을 되돌릴 잠금장치였다) · A-3/B-6 · A-4(세 줄로 갈랐다) ·
  A-5 3벌 · A-14 · A-15 · **B-7**(정본 범위가 파일 내용보다 좁았다) · **B-13**(계층 병기 — 없으면 거짓 실패) · C-11.
  ⚠ **`_workspace` 문서 회수(FOLLOWUPS #26 · LEADER-DECISIONS 5곳 · 검증/최종 산출물)는 안 했다 — 리더·검증자 영역이다.**

### 4. 섹션 개명 (§6) — 6곳, 렌더 1
`page.tsx:518`(★ 유일 렌더) · `:516` 주석(괄호 안 **원문 절 제목은 보존**) · `:444`·`:445` · `StrikeMap.tsx:128`·`:1580`.
★ **§6-3 「두어야 할 자리」 10곳 보존 확인** · ⛔ **`/rally-2026-08-28` 5곳(렌더 3) 무변경 확인.**
경계 사례 `page.tsx:63` 은 한정어를 붙였다 — 「**원문 식순 20행**의 `대오정비` 만」.

### 5. 자산 3장 (`public/images/strike-2026-09-04/`)
| 파일 | 크기 | 판정 |
|---|---|---|
| `branch-layout.png` 733×2083 | **46KB** | `unoptimized` — WebP(98KB)보다 **작고 무손실** |
| `location-map.png` 1300×1179 | **898KB** | `quality={90}` |
| `location-roadview.jpg` 1600×926 | **456KB** | `quality={90}` |
★ **PNG 2장은 raw 버퍼 비교로 «픽셀 동일» 확인**(`compressionLevel:9, palette:false` · ⛔ `effort:10` 금지).
★ 로드뷰는 처음 mozjpeg 로 399KB 가 나왔으나 **리더 실측 기준선(456KB)에 맞춰 재생성**했다 —
그 기준선에서 **Next 산출물 `w=1080` WebP 판독 확인이 이미 끝나 있어서**다. `design/` 원본은 **그대로 뒀다**(감사 추적).
⚠ **아직 페이지에 안 붙였다 — 자리가 «판정 대기»다**(아래).

### 6. ★ 물어야 하는 것
| # | |
|---|---|
| **①** | ★★ **사진 3장의 «자리». 강한 제약 하나를 찾았다** — 지도 섹션은 `NAVER_MAP_CLIENT_ID` 조건부라 **키가 없으면 통째로 사라진다.** 그런데 사진의 가치 중 하나가 **«네이버 API 가 죽어도 남는다»**(V3-IMAGES) 다. → **사진을 그 섹션 «안»에 두면 그 가치가 0 이 된다.** 밖에 두려면 **새 `<section>` + 헤딩 문면**이 필요한데 그것은 **우리 저작**이다 |
| **②** | **캡션 문면 3개.** 8/28 선례는 극단적으로 짧다(`배치도`). ⛔ 출처 배지·꼬리표·`※` 금지는 알지만 **문면 자체는 내가 지을 값이 아니다** |
| **③** | **V-a · V-b**(Q3 「한 번만」 · Q4 「같은 행위」) — 검증 회부가 열려 있어 **관계를 잇는 문장을 안 썼다.** 지금 페이지는 원문 배치만으로 서 있다 |
| **④** | **화장실 새 사실**(`인근화장실(시청역, 스벅 등)`) — V3-IMAGES 판정 4번. **문장을 짓지 않았다.** ⚠ **사진을 게시하면 그 문장이 이미지에서 판독된다** → 보이는 사람은 읽고 **스크린리더 사용자는 못 읽는다** |
| **⑤** | **게시 조건 63** — v3 원본 이미지가 `design/` 에 없어 **축자 대조를 «못 했다»**(«통과»가 아니다) |

**검증**: `tsc`·`lint`·`build` ⭕ · **프리렌더 계수 29항목 불일치 0**(`15:15` 는 **참값 5**로 판정).

### 7. 검증 §72 확정 문면 2건 반영 (2026-09-01)

**V-a·V-b** — 표 «밖» · 표 «아래» · **원문 묶음(`<dl>` → ※) 뒤** 한 문단 두 문장(축자).
★ **핵심이 「한 번만」이 아니라 「같은 것」**이라는 리더 정정을 주석에 남겼다 — 「한 번만」은 **10:40 실패를 못 막는다**.
⛔ 합치기·지시어(「위 두 곳」)·시각 기재·`지부천막` 전부 금지를 주석에 명시. **두 문장 순서 고정.**

**화장실** — `TOILET_NOTE` 신설. ★ **«새 사실»이 아니라 M-53 확정 문면의 «이행»**(M-57 범례 9행이 안 담아 유실).
`화장실` 과 `:` 사이 **공백 1칸 원문 그대로** · `스벅` 안 폄.
★★ **자리에 «구조 제약»이 하나 걸렸다** — 리더 지시는 「범례 «아래» 문장」인데 `<figcaption>` 은
**`<figure>` 의 마지막 직계 자식이어야 한다**(HTML 스펙 · 소스에 이미 명시된 계약).
→ **`figcaption` «안» · `<ul>` 아래**에 뒀다. 밖으로 빼면 그 계약이 깨진다.
⛔ 범례 `<ul>` «행»으로는 안 넣었다 — 범례 행은 **지도 도형의 텍스트 등가**이고 이 문장에는 **도형이 없다**.
⚠ **세로 증분·배치는 QA·디자이너 회부** — 내가 정하지 않았다.

**`~` 구분자** — 리더 §11 판정대로 **ASCII U+007E 고정**(이미 그 상태였다 · 전사본 6행 전부 ASCII).
⚠ **§8-2 를 «해소»로 적지 않는다 — 여전히 못 쟀다.** 「안전한 방향으로 고정」이 참값이다.
⛔ 정규식을 `～`(U+FF5E)로 **확장하지 않았다.**

#### 프리렌더 실측 (반영 후)
```
천막 4   참석명단 2   출석부 2   서명 2   명단 작성 3   물품 수령 1
지부천막 0 ⭕   추후 안내 0 ⭕   대오 1 ⭕   ※ 3 ⭕(종류 2 · 출현 3)
인근화장실 1   스벅 1   화장실 : 1   임시화장실 3(범례 2 + TOILET_NOTE 1)
```
⚠ 리더 예고 `천막 1→2` 는 **v3 «이전» 기준선**이다 — v3 블록이 들어간 뒤 참값은 **4**
(`KOSCOM_LOCATION_NOTE` 1 + v3 2 + V-b 1). **기대표를 «회차 기준선과 함께» 적어야 한다.**

`tsc`·`lint`·`build` ⭕.

### 8. 사진 3장 게시 — 「원본 자료」 섹션 (2026-09-01)

**자리**: 새 `<section aria-labelledby="source-heading">` · **지도 섹션 «바로 뒤»** · 헤딩 **`원본 자료`**(사용자 축자).
★★ **자리 근거가 내 발견에서 나왔다** — 지도 섹션은 `NAVER_MAP_CLIENT_ID` **조건부**라 키가 없으면 사라지는데
사진의 가치 하나가 **«API 가 죽어도 남는다»** 다. **밖에 두었기에 «지도가 죽었을 때의 위치 안내»가 된다.**
⛔ **조건부 «안»으로 옮기면 그 근거가 통째로 죽는다** — 주석에 그렇게 적었다.
⚠ 헤딩을 「주최측 자료」류로 바꾸면 **거짓**이다(배치도는 금융노조 것, 지도·로드뷰는 **지부가 표시를 얹은** 것).

| 파일 | 압축 | `sizes` | 캡션(사용자 축자) |
|---|---|---|---|
| `branch-layout.png` 733×2083 · 46KB | **`unoptimized`** | — (무시된다) | `지부배치도` |
| `location-map.png` 1300×1179 · 898KB | `quality={90}` | `(min-width: 768px) 624px, 100vw` | `집회 장소` |
| `location-roadview.jpg` 1600×926 · 456KB | `quality={90}` | 〃 | `코스콤지부 구역` |
⚠ **세 판정을 통일하지 마라**(8/28 이 «일부러» 갈라 놓은 계보) · `quality` 는 **`[75,90]` 만** 허용(밖이면 400 → 조용히 75) ·
`sizes` **624px 은 실측**(패널 672 − `p-8` 24×2) — ⛔ 8/28 의 832px 은 **33% 과대 요청**이다.

**`alt` 3개는 «임시»다** — 검증 회부 중이고 주석에 그렇게 명시했다. ⛔ 비워 두지 않았다(내용 이미지의 빈 `alt` 는 접근성 실패).

#### ★★ 계층 검사 — 리더가 경고한 «배치도 금지어 함정»을 값으로 통과했다
`alt` 는 **태그 제거 계수에 «안 잡힌다»**(§5.1 «속성 텍스트» 계층). 그래서 **따로** 셌다:
```
속성 계층(alt·aria-label·title) 금지어 23종 검사 → 0건 ⭕
  메인무대 0 · 6,000명 0 · 천막 8 0 · 지부천막 0 · 상품권 0 · 위치서비스 0 · 1차 0 · 여의도 0 …
```
★ **이것이 이 라운드에서 «계층»이 실제로 갈린 첫 자리다** — `임시화장실` **가시 3 · 속성 1 · 합 4.**

#### 내 기대값이 틀린 3건 (렌더는 정상 · 참 출처를 갈랐다)
| 낱말 | 내 기대 | 참값 | 출처 |
|---|---|---|---|
| `집회 장소` | 1 | **2** | `MAP_SR_INTRO`(sr-only) + 캡션 |
| `금노` | 2 | **3** | v3 비고 2 + **V-a 문장 1**(내 기대가 V-a «이전» 값이었다) |
| `임시화장실` | 4 | **가시 3**(+속성 1) | 범례 2 + `TOILET_NOTE` 1 |
★ 리더 확정값은 둘 다 맞았다 — **`15:15` 5 ⭕ · `코스콤지부 구역` 3 ⭕.**

`tsc`·`lint`·`build` ⭕ · 프리렌더 34항목(가시) + 속성 계층 23항목 검사 완료.

### 9. `alt` 3개 — 검증 §73 확정본 반영 (2026-09-01)

축자로 갈아 끼우고 **판정 근거를 주석에 남겼다**(안 적으면 «불완전해 보여» 되돌린다):

| # | 판정 | 주석에 남긴 근거 |
|---|---|---|
| ① | **북단 무대를 «이름으로 부르지 않는다»** | `메인무대` → 기대 0 + ★ **비시각 사용자«만» «현장 표지판과 다른 이름»을 얻는다**(시각 사용자는 범례·pill·본문이 전부 `무대 1` 이라 정정되는데 **`alt` 만 듣는 사용자는 정정 경로가 없다**) / `무대 1` → **이미지에 없는 글자** → **셋째 길**. ⚠ **지도 이미지는 처분이 갈린다**(원본 라벨이 `무대 1` 이라 쓴다) — **통일 금지** |
| ② | **`오른쪽`·`왼쪽` 제거** | **비시각 사용자에게 «화면의 오른쪽»이라는 개념이 없어** 낭독 순간 **지상 방위로 읽힌다** → 기대 0 위반. ⚠ **「남쪽」은 쓴다**(지도 방위는 확정 · 실측 51 m) · ⛔ **「아래」로 바꾸지 마라(화면 참조)** |
| ③ | **상자 두 줄을 `alt` 에 «안» 넣는다** | ⚠ 근거가 **«두 벌 금지»가 아니다** — 그러면 «문장을 빼고 `alt` 에 넣자»가 성립한다. **참 근거: 그 경로가 «문장»이라 비시각 사용자에게 «직접» 닿는다.** ⚠⚠ **조건부** — **`TOILET_NOTE` 가 게시되어야 이 판정이 선다. 빼면 다시 열린다** |
| ④ | **좌측 세로열 미언급** | 약칭뿐 · 위치 정보 없음. ★ **정직하게: 시각 사용자는 그 열을 본다 — «완전 등가»가 아니다** |
| ⛔ | **로드뷰 세로 라벨 축자 인용 금지** | 원본이 **세로쓰기**(`코스콤 / 지부구 / 역`)라 **공백을 결정할 수 없다** |
| ⛔ | **`코스콤지역` 을 지도 이미지 출처로 적지 마라** | 그 상자에 **없다**(리더 오기 정정) — v3 비고와 8/28 `RallySchedule` 에 있다 |
...
★ **`무대 2` 참값은 7 이다** — 전수로 갈랐다: 가시 5(코스콤 한 줄 · 범례 2·3·4·6행) + 속성 2(alt ①②).
**리더 기대 8 이 하나 많다.** ⚠ 클라이언트 pill(`무대 2`)은 **프리렌더에 없다**(마커는 effect 생성) —
**계층이 하나 더 있고 그것을 세면 8 이 된다.** → **기대를 «계층과 함께» 적으면 둘 다 맞다.**

**0 유지 22종 — 두 계층 각각 0 ⭕**(`메인무대`·`6,000명`·`천막 8`·`동화면세점`·`서울시의회 본관`·`덕수궁`·
`2026년 04월`·`오른쪽 차로`·`동측`·`서측`·**`오른쪽`·`왼쪽`**·`지부천막`·`상품권`·`위치서비스`·`14:20`·`추후 안내` …).
⚠ `코스콤지역` 은 **0 이 아니라 1 이 맞다**(v3 비고 축자 · §3-A) — 내가 0 유지 목록에 잘못 넣었다가 잡았다.

`tsc`·`lint`·`build` ⭕.

### 10. 「원본 자료」 자리 확정 — 「총파업 일정」 «뒤»로 이동 (검증 §74 · 2026-09-01)

섹션을 **통째로** 옮겼다(세 장 **묶음 유지** · 내용·`alt`·캡션 **무변경**). 랜드마크는 `aria-labelledby` 유지
(이 페이지 5/5 전부 그 규칙 — ⛔ `aria-label` 을 붙이면 **하나만 규칙이 달라진다**).

**주석에 남긴 «되돌림 방지» 4건:**
1. ⛔ **암묵 근거를 미리 죽였다** — *「지도와 사진이 같은 대상이니 붙어 있어야 대조가 된다」*.
   **대조는 «붙어 있어야» 성립하지 않는다** — 그 행동은 **가정이지 관측이 아니고** 해도 **행동을 안 가른다.**
   반면 **«일정을 못 읽는 것»은 행동을 가른다.**
2. **세 장을 묶는 근거 = «판별기»**(8/28 「출처 증거」 판정의 숨은 전제 — *「같은 정보를 지는 «다른 도구»가 있다」*).
   배치도 ↔ `KOSCOM_LOCATION_NOTE`+범례 2·3행 / 지도 ↔ 네이버 지도+범례 9행 / 로드뷰 ↔ 거리뷰 기능+범례 3행 — **셋 다 선다.**
3. ⚠⚠ **로드뷰 조건 «두 문장 한 벌»** — ① 키가 없으면 거리뷰 기능이 죽어 **사진이 유일한 시각 경로**가 된다
   ② **그래도 자리를 가르지 않는다**(스크롤로 닿는다 · **§0.4 는 «숨김»을 막지 «아래»를 막지 않는다**).
   ⛔ **한 문장만 읽고 되돌리지 마라.**
4. ⛔ **`TOILET_NOTE` 를 함께 옮기지 마라** — 자리 근거가 **«범례 6·7행을 읽은 뒤»** 이지 «사진 옆»이 아니다.

#### 실측 (`360×640` · 스크롤바 0 · dpr 1 · 루트 12px · **가로 스크롤 0**)
| 섹션 | top | height |
|---|---|---|
| `gather` | 228 | 182 |
| `overview` | 464 | 230 |
| `map` | 748 | 808 |
| ★ **`program`(총파업 일정)** | **1,610** | **1,689** |
| ★ **`source`(원본 자료)** | **3,353** | **1,506** |

문서 높이 **5,402px**.
★ **리더 예측(일정 시작 1,862)보다 «더» 좋다 — 실측 1,610.** 종전 3,498 대비 **−1,888px(≈ 3.0 화면)**.
⚠ 리더 실측 `1,630~3,266` 은 **«지도 뒤» 배치 전제 값이라 죽었다** — 위 표가 참값이다.
⚠ 이미지 3장 로드 확인: `branch-layout.png` 는 **원본 733px 그대로**(`unoptimized` ⭕) · 나머지 둘은
**Next 옵티마이저 경유**(dpr 1 · 360 뷰포트라 작은 변형을 받는다 — 실기기 dpr 3 은 `w=1080`).

`tsc`·`lint`·`build` ⭕.

### 11. QA §72·§73 — 실패 1건(이미지 전송량). **원인을 값으로 갈랐다. 처방은 회부**

QA 실측: 모바일이 **`w=1080` 이 아니라 `w=1200`** 을 받아 첫 방문 **451.1KB**(기준선 390 대비 **+61.1KB**).
QA 가 *「구현 이탈이 아니라 «스펙의 모바일 항» 문제」* 로 분류하고 **처방을 디자이너·리더로 넘겼다.** 나도 안 고쳤다.

#### ★★ 원인 — **루트 폰트 75% 가 패딩 사슬을 0.75배로 줄인다**
```
루트 12px(html{font-size:75%})
  컨테이너 px-4 = 1rem    → 16px 이 아니라 12px
  패널     p-5  = 1.25rem → 20px 이 아니라 15px
  사슬 = (12 + 15) × 2 = 54px          ← 16px 가정이면 72px 로 «틀리게» 나온다
슬롯 = 100vw − 54  →  390 에서 336 ⭕ (QA 모델 `100vw − 54px` 와 일치)
```
★★★ **그래서 `sizes` 의 데스크톱 항은 «맞고» 모바일 항만 «틀렸다»** —
데스크톱 **624** 는 리더가 **패딩을 실제로 재서**(672 − 24×2) 냈고, 모바일 항은 **관용 기본값 `100vw` 를 그대로 뒀다.**
**이 사이트의 특수 조건(루트 75%)이 데스크톱 계산에는 들어갔는데 모바일 계산에는 안 들어갔다.**
★ 데스크톱 항은 내가 실측으로 재확인했다 — 컨테이너 720 · 패딩 24×2 · 패널 672 · 패딩 24×2 · **슬롯 624 ⭕**

#### 처방 후보 2개 (판정 회부 · 내가 안 골랐다)
| | `sizes` | 비고 |
|---|---|---|
| A | `(min-width: 768px) 624px, calc(100vw - 54px)` | QA 모델 그대로. **`54px` 이 «루트 12px» 전제에 매달린다** |
| ★ B | `(min-width: 768px) 624px, calc(100vw - 4.5rem)` | **패딩 사슬을 그대로 쓴 표현**(px-4 1rem + p-5 1.25rem)×2 = **4.5rem**. 루트가 바뀌어도 따라간다 |
**예상 효과(둘 다 같다)**: 390·dpr3 → 336×3 = 1008 → **`w=1080`** · 첫 방문 **451.1 → 391.4KB**(−59.7KB).
⚠ **한 줄 변경이고 3장 중 2장에만 적용된다**(`branch-layout.png` 는 `unoptimized` 라 `sizes` 를 무시한다).

#### QA 가 회신한 것 — 전부 통과
200% 가로 **6조합 0**(★ **스크롤바 15px 조건**이라 내 §72 측정보다 **가혹한데도 0**) ·
첫 화면 `10시 30분` **여유 305.5px** · 헤딩 차례 **건너뜀 0**(런타임 `h5` 「지도 데이터」는 **네이버 위젯 주입** — 손대지 마라) ·
`unoptimized` 를 «통일 안 됨»으로 **안 올림** · **속성 계층 금지어 33종 0건**(내 23종을 독립 재현 + 10종 추가) ·
`임시화장실` **가시 3 · 속성 1** 내 값과 완전 일치 · 식순 회귀 **기준값 3개 전부 일치**.

★★ **§72 계수 불일치 8건은 «렌더 문제가 아니다»** — 전부 §7-A 사용자 답이 만든 두 문장(7건)과 캡션(1건)이고
**검증 확정 문면과 바이트 일치**한다. **`V3-PLAN §3` 기대표가 그 승인분을 안 셌을 뿐이다.**
⚠ **그대로 세면 정상 문면 8건이 실패로 올라간다 — 코드가 아니라 기대표를 고쳐야 한다.**

### 12. `sizes` 모바일 항 교체 (리더 확정 · 2026-09-01)

```
- sizes="(min-width: 768px) 624px, 100vw"
+ sizes="(min-width: 768px) 624px, calc(100vw - 54px)"
```
**최적화 경유 2장에만** 적용(`location-map.png` · `location-roadview.jpg`).
★ **`branch-layout.png` 는 원래부터 `sizes` 가 «없다»** — `unoptimized` 가 무시하므로 8/28 선례대로 **일부러 비워 뒀다.**
**그래서 이번에 손댈 것이 없었다**(리더가 «네 판단»으로 남긴 자리).

★★ **주석에 «54 의 유래»를 적었다 — 임의 숫자로 보이면 다음 사람이 지운다:**
```
루트 12px (html{font-size:75%})
  컨테이너 px-4 = 1rem    → 12px      패널 p-5 = 1.25rem → 15px
  사슬 = (12+15)×2 = 54px             ← 16px 가정이면 72 로 «틀리게» 나온다
```
실측 검산 **320→266 · 360→306 · 390→336** 세 폭 일치 · `100vw` 였을 때 대가 **+61.1KB(+15.7%)**.
★ 대안 `calc(100vw - 4.5rem)`(사슬이 정확히 4.5rem · 루트를 따라간다)도 주석에 남기고
**«지금은 리더 확정값 54px 을 쓴다 · 루트 폰트가 바뀌면 다시 재라»** 를 못박았다.

**예상 선택**(deviceSizes 640·750·828·1080·1200·1920): 390·dpr3 → 336×3=1008 → **`w=1080`** ⭕ ·
360·dpr3 → 918 → `w=1080` · 320·dpr3 → 798 → **`w=828`**.

⚠ **리더 전제 정정 접수** — *「지도 청크 무변경이니 재측정 불필요」* 는 **틀렸다**(QA 실측 `93339a3c1cde · 30,598 B`).
★ 내가 그 문장을 **두 번** 근거로 썼다. **앞으로 청크 해시는 «그때그때» 재고 «전제로» 쓰지 않는다.**

### 13. `sizes` — **B(`4.5rem`) 확정**. `54px` 은 «고정»이고 `4.5rem` 은 «따라간다»

```
sizes="(min-width: 768px) 624px, calc(100vw - 4.5rem)"     ← 최적화 경유 2장
```
★★★ **결정적 사실은 리더가 냈다 — 이 사이트에는 «루트를 바꾸는 장치»가 있다:**
`FontScaleControl.tsx:72` 가 **`document.documentElement.style.fontSize` 를 직접 바꾼다**(`localStorage` 지속 · 내가 확인).
→ **조합원이 글자를 키우면 패딩이 함께 커지고 슬롯이 줄어드는데 `54px` 은 그대로 있는다.**
**`4.5rem` 은 패딩 사슬 그 자체**(`px-4` 1rem + `p-5` 1.25rem, 양쪽)라 **루트가 무엇이든 따라간다.**
⚠ **QA 모델이 `100vw − 54px` 인 것은 «그 측정이 기본 배율에서 이뤄졌기 때문»**이지 상수라는 뜻이 아니다 — 주석에 못박았다.

**첫 방문 합계 실측 391.4 KB** = 지도 178.7 + 로드뷰 166.4 + 배치도 46.3(`unoptimized`).
**기준선과 정확히 일치**(종전 451.1 → **−59.7 KB · −13.2%**).

#### ⚠ 측정 방법을 정직하게 적는다 — **내가 띄운 서버가 아니다**
`npm run start` 를 백그라운드로 띄웠는데 **`EADDRINUSE`(errno −48)로 죽었다.**
★ **이 저장소가 §5.7 에 기록한 바로 그 함정**(*「앞 서버를 안 죽이고 재기동해 EADDRINUSE 로 죽었는데
종료코드가 0 이라 실패 신호가 없었고 `curl` 은 낡은 서버가 응답해 성공했다」*)이다.
→ 위 바이트는 **리더가 띄워 둔 기존 서버**의 `/_next/image` 산출물이다. **값 자체는 유효하다**
(옵티마이저는 `public/` 소스에서 만들고, 리더·QA 실측 178.7·166.4 와 **완전 일치**한다).
⚠ **그러나 «내가 띄운 prod 서버에서 쟀다»고 적었으면 거짓이었다.**
★ **리더 서버(pid 69793)는 무사하다 — 내 프로세스가 포트를 못 잡고 죽었기 때문이다.**

#### ★ B 확정 근거 보강 — **리더가 «쟀다»**
말로 다투지 않고 실측했다(360 뷰포트 · 루트 12px → 18px): **슬롯이 «실제로» 306 → 279 로 줄었다.**
| 표현 | 루트 12px | 루트 18px | 참값 |
|---|---|---|---|
| `100vw − 54px` | 306 ⭕ | **306** | 279 → ⛔ **27px(9.7%) 과대** |
| **`100vw − 4.5rem`** | 306 ⭕ | **279** ⭕ | 279 → ⭕ 일치 |
★ 그 표를 주석에 넣었다. ⚠ **못 잰 것도 함께 적었다** — 이미 로드된 이미지라 **«재요청»은 관측하지 못했다.**
★ 그러나 **`sizes` 의 «일»은 슬롯을 «표현»하는 것**이고 **`px` 은 확대 상태에서 그 일을 못 한다** —
**우리가 방금 고친 결함이 정확히 그것**이고 `px` 은 그것을 **«확대 상태에» 남긴다.**

#### ⚠ 리더가 잡은 내 주석 결함 — **«다시 재라»는 할 사람이 없다**
내가 적었던 *「지금은 확정값 54px · **루트 폰트가 바뀌면 다시 재라**」* 는
**루트 폰트 변경이 «코드 변경»이 아니라 «조합원의 런타임 조작»**(슬라이더 + `localStorage`)이라
**그 문장이 전제하는 «개입할 사람»이 존재하지 않는다.**
★ **«사람이 개입할 수 있는 사건»을 전제한 지시를 «개입할 수 없는 사건»에 붙인 것**이다 — B 적용 때 이미 지웠다.

### 14. ⛔ `4.5rem` 회귀 — **되돌렸다. 나와 리더가 «같은 오류»를 함께 했다**(QA §75)

```
sizes="(min-width: 768px) 624px, calc(100vw - 54px)"     ← 최종
```
★★★ **기전**: **`sizes` 의 폰트 상대 단위는 «초기» `font-size`(16px)로 푼다** —
**프리로드 스캐너가 «스타일 적용 전»에 평가**하므로 이 페이지의 `html{font-size:75%}` 를 **못 본다.**
→ **`4.5rem` 은 54px 이 아니라 «72px»** 이 되어 **슬롯을 과소 표현**한다.

| 조건 | 슬롯 | `54px` | `4.5rem`(=72px) |
|---|---|---|---|
| 320/dpr3 | 266 | **828** ⭕ | 750 ❌ **48px 부족(6.0%)** |
| 390/dpr2 | 336 | **750** ⭕ | 640 ❌ **32px 부족(4.8%)** |
| 나머지 4조건 | — | 동일 | 동일 |
★ **내가 독립 검산했다 — 두 모델 다 6조건과 일치**하므로 «rem 이 72px 로 평가된다»가 참이다.

#### ★★ 이번 오류의 «형태» — 관측은 맞았고 «어디에 적용되는가»가 틀렸다
리더가 잰 것(**루트 18px → 슬롯 306 → 279**)은 **참이다.** 그러나 그것은 **레이아웃** 관측이고,
`sizes` 는 **프리로드 시점**에 평가된다. **두 값이 «다른 시점»에 산다.**
→ *«rem 이면 슬라이더를 따라간다»* 는 전제가 **애초에 성립하지 않았다** — `sizes` 는 **초기값으로 얼어 있어
둘 다 슬라이더를 못 따라간다.** `rem` 은 **따라가지도 않으면서 «기본 배율에서» 틀린** 쪽이다.
⚠ **나도 그 전제를 «구조 논증»으로 먼저 제안했고**(A/B 비교에서 B 를 권고), 리더가 **레이아웃 실측으로 보강**했다.
**둘 다 «측정한 것»은 맞았는데 «그 측정이 이 결정에 닿는가»를 아무도 안 물었다.**
★ 이 라운드 「세는 방법」 계보에 **일곱째 축: «어느 «시점»에 평가되는 값인가»** 가 붙는다.

⚠ **판독 대상이 걸려 있어 «전송량이 더 줄어 좋다»가 아니다** — 두 장 다 판독 대상이 있고
(`화장실 : … 스벅 등) 이용` · `PAUL BASSETT`) **판독 생존 확인이 `w=1080` 기준**이었다.
**과소 요청은 그 확인을 무효로 만든다.** 최악 조건 전송량은 **두 판본이 391.4KB 로 같다** — 되돌려도 목표 유지.

★ 주석에 **«`4.5rem` 으로 바꾸지 마라 + 기전 + 관측표»** 를 넣었다 — 안 적으면 다음 사람이
*「루트가 12px 인데 왜 54px 하드코딩?」* 으로 **또 `rem` 으로 고친다**(QA 경고 그대로).

#### QA 자기 정정 1건 — 기록해 둔다
첫 측정에서 `w=1920` 요청이 조건마다 하나씩 더 보여 **«같은 이미지를 두 번 받는다»로 보고 직전까지 갔다가**
**CDP `setDeviceMetricsOverride` 가 `goto` 보다 늦게 걸린 계측 아티팩트**임을 스스로 잡았다.
★ **«조건을 처음부터 가진 새 컨텍스트»로 다시 잰 것**이 정답이었다 — 측정 위생 계보(§5.8.4)의 새 사례다.

#### 조건 63 이름 충돌 — 소스 1곳을 `V3-63` 으로
`page.tsx` 의 `MEMBER_SCHEDULE` 머리 주석 1곳이 «게시 조건 63» 을 인용하고 있었다.
**그 번호가 두 계약을 가리켰다** — ① 식순 묶음(**해소**) ② v3 원본 축자 대조(**열림**).
→ 열린 쪽 이름 **`V3-63`** 으로 바꾸고 **«두 계약이었다는 사실 + 번호 재배정 금지»** 를 함께 적었다.
★ 소스 인용은 **그 1곳뿐**이었다(전수 확인).

### 15. QA §76 — **복원 확인 · 실패 0 · 라운드 종결**

동결 기준 **4개 해시 전부 내 현재 상태와 일치**(`page.tsx b9dba3c7eccf` 포함 — **V3-63 이름 편집이 이미 그 안에 들어 있었다**).
6조건 전부 슬롯 충족(부족 0) · **320/dpr3 828 복구 · 390/dpr2 750 복구** · 최악 **391.4 KB**(§72 451.1 대비 −59.7 · −13.2%) ·
요청 조건마다 정확히 3건 · 중복 0 · 지도 청크 불변 · 200% 6조합 0 · 첫 화면 여유 305.5px(**네 회차 연속 동일**).
★ 이번은 **스크롤바 0**, §73·§74 는 **15** — **양쪽 조건 모두에서 통과**했다.

#### ⚠ 선언 한 줄에 «계층»이 빠졌다 — 내가 잡아 QA 에 넘겼다
QA 선언: *「소스 `calc(100vw - 54px)` 3건 · **`4.5rem` 0**」*.
**소스 리터럴 `grep` 은 `4.5rem` 4건**이다 — **전부 «⛔ 바꾸지 마라» 금지 기록 주석**이고
**`sizes` 속성 0 · 프리렌더 0** 이다(내가 계층별로 갈랐다).
★★ **`V3-PLAN §0` 이 경고한 바로 그 형태다** — *「소스 `grep` 으로 금지어를 세지 마라 — 이 저장소는
**주석에 금지어를 적는 것이 설계**다」*. 실측 선례: `손피켓` 소스 9 ↔ 렌더 0.
⚠ **그 선언을 계층 없이 회귀 기준으로 쓰면** 다음 게이트가 소스에서 4건을 찾아
**«금지 기록 주석을 지우는 것»으로 처방한다** — **그 주석이 바로 이 회귀를 막는 장치**다.
→ **「`4.5rem` = `sizes` 속성 0 · 프리렌더 0 · 소스 주석 4(금지 기록 · 정당)」로 계층을 병기해야 한다.**
★ 이 라운드 **여덟 번째** «축이 없는 값»이고, **처음으로 «QA 선언»에서 났다.**

**최종 상태**: `tsc`·`lint`·`build` ⭕ · 남은 게시 조건 **2건**(62 · **V3-63**) — 둘 다 개발자 손 밖 · **커밋 안 함.**

#### QA §76-6 — 선언 정정. **QA 가 두 번째 누락까지 스스로 찾았다**
내 지적(`4.5rem` 계층)을 독립 재계수해 **내 값과 일치**시키고 확정 선언을 계층별로 다시 썼다:
```
sizes 속성              54px 2 · 4.5rem 0
프리렌더(<script> 제거)  54px 2 · 4.5rem 0
소스 주석               54px 1 · 4.5rem 4   ← 전부 «금지 기록». 정당한 출현. 지우지 마라
```
★★ **그 4건은 «QA 가 §75 에서 요청해 내가 쓴 바로 그 주석»**이고 **QA 실측표를 인용**한다 —
*「제가 요청해 만든 방어 장치를, 제가 축을 빠뜨린 선언 때문에, 다음 사람이 지울 뻔했다」*(QA 자평).

★ **정정하면서 «두 번째 누락»도 찾았다** — 자기가 적은 «프리렌더 `54px` 4건» 에도 계층이 없었다.
`<script>` 를 지우면 **2건**(= `sizes` 2)이고 나머지 2건은 **RSC flight 페이로드** 안이다.
**이 저장소가 이미 세운 «Next 산출물을 셀 때는 `<script>` 를 먼저 지운다»를 «마감 확인»에서 안 지킨 것.**

★★★ **이 건의 형태**(QA 자평 · 그대로 옮긴다):
> **측정 블록에는 환경표를 꼬박 붙였는데 «확인 완료» 한 줄에는 아무 축도 안 붙었다.**
> **마감 문장이 측정만큼 검증되지 않는다.**

**최종**: 실패 0 · 동결 `page.tsx b9dba3c7eccf` · 지도 청크 `93339a3c1cde` · `tsc`·`lint`·`build` ⭕ ·
남은 게시 조건 **62 · V3-63**(둘 다 개발자 손 밖) · **커밋 안 함.**

---

## 한 표 3열 병합 (2026-09-01 · dev7) — `MERGED-TABLE.md` 확정본 구현

**사용자 축자**: *「결의대회 페이지 일정처럼 금노일정표와 조합원일정표를 한표에 함께 표시했으면 해.」*
⛔ **두 패널은 죽었다**(`V3-PLAN §5`). 재론·대안 제안 금지.

### 구현
- **패널 1개 · 표 21행 3열**. 열 머리 `시간` / `금융노조 공식 순서` / **`코스콤 참가 조합원 일정`**(사용자 확정).
- **데이터 무변경** — `PROGRAM`·`MEMBER_SCHEDULE` 그대로. 병합 정보는 **별도 상수**에만:
  `KOSCOM_COL_HEADING` · `[MS_MOVE…MS_FREE]` 구조분해 · `KOSCOM_BY_PROGRAM_TIME` · `LEAD_ROW` · `KoscomCell`.
- `<caption>` → **표 위 `<p>`**(caption 은 표의 «접근성 이름»이라 식순 단서가 코스콤 열까지 덮는다).
- 가로 여백 **px 고정**(`pr-[4.5px]`·`pr-[9px]`·`px-[6px]`·`ml-[4.5px]`) · **세로는 rem 유지**.
- `inline-block` + `max-w-[calc(100%-4.5px)]` + ★ **`align-top`**(`detail` 이 최대 4줄 통짜라 baseline 이면 시각이 마지막 줄에 붙는다).
- 테두리는 **`<td>` 에**(⛔ `<tr>` 에 주면 15행 병합 칸에 가로줄 14개).

### 검산 3종 (코드 + 렌더 둘 다)
```
표 행 수 21 ⭕ · rowSpan 선언 [1,1,1,15,2,1] · 합 21 ⭕ · 코스콤 칸 6 ↔ 원문 6행 1:1 ⭕
KOSCOM_BY_PROGRAM_TIME 합 20 (= PROGRAM.length) ⭕
<th scope="col"> 3개 ⭕ · <caption> 0 ⭕ · <tr> 테두리 0 ⭕ · 표행 1 금노 칸 «빈 셀» ⭕
```

### 브라우저 실측 — **§8 회귀 기준과 대조**
| 항목 | 스펙 | 실측 |
|---|---|---|
| 문서 가로 밀림 | **0** | **0** — `320`·`345` × 기본·**루트 24px**·**루트 32px** **6조합 전부** ⭕ |
| 최대 셀 넘침 | **0** | **0** — 6조합 전부 ⭕ |
| 열 폭(345·루트 32px) | **78 / 48.2 / 74.8** | **78 / 48.2 / 74.8** ⭕ **정확히 일치** |
| 표 행 수 | 21 | **21** ⭕ |
| 표 높이(345·루트 12px) | 1,245.9 | **1,224** — ⚠ **−21.9px** |
| 표 높이(345·루트 32px) | ≈10,984 (U1 재측정 대상) | **11,063** |
| ★ **루트 24px 높이**(U8 «미측정») | — | ★ **345 → 5,060 · 320 → 5,547** ← **새로 쟀다** |

★ **U8 이 닫혔다** — 이 저장소의 「200%」가 두 값을 가리킨다는 것이 수치로 확인됐다:
**루트 32px 11,063 ↔ 루트 24px 5,060 — 2.19배.** ⚠ **회귀 기준값을 인용할 때 루트를 반드시 함께 적어라.**
⚠ **표 높이 −21.9px 는 U1 계열이다**(스펙 값이 «20행 + A·B 합본» 가정에서 나왔다). **결함이 아니라 재측정값이다.**

### 측정 위생
⚠ 리더 개발 서버(3000)가 **내려가 있었다**(내가 죽인 것이 아니다 — 내 앞 회차 `npm run start` 는 `EADDRINUSE` 로 죽었다).
→ **3000 을 비워 두고 `PORT=3100` 으로 내 것을 띄웠고**, ★ **«현행본인가»를 지문으로 확인**했다
(`코스콤 참가 조합원 일정` 2 · `총파업 참가 조합원 상세일정` **0**). **측정 후 종료하고 «값으로» 확인**했다.
★ 루트 폰트 인라인 주입도 **매 측정 후 제거하고 값으로 확인**(`루트 12px` · `style 없음`).

### 렌더 문자열 (회차 병기)
`코스콤 참가 조합원 일정` **1** · `총파업 참가 조합원 상세일정` **0** ← ★ **병합 «전»에는 각각 0·1 이었다** ·
`금융노조 공식 순서` 1 · `총파업 일정` 1 · `총파업 식순` 0 · `※` **3** · `대오정비` 1 · `폐회선언` 1 ·
`추후 안내` 0 · `천막` 4 · `참석명단` 2 · `여의도` 1 · `15:15` 5 — **불일치 0.**

#### U10 판정 — 자리 «그대로». ★ 내 우려가 «반대 방향»이었다
리더가 쟀다(`390×844` · 루트 12px): 병합이 페이지를 **짧게** 만들었다 —
**일정 1,689 → 1,391(−298)** · **원본 자료 top 3,353 → 3,075(★ 278px «위로»)** · 문서 5,402 → 5,255.
중복 헤딩 2개와 패널 여백이 사라졌다. **사진은 밀려난 것이 아니라 «당겨졌다».**

★★ **내 오류**: 근거로 든 `11,063px` 이 **«루트 32px» 값**이었다 — 그 배율에서는 **모든 것이 길다.**
**«루트를 안 적은 높이»로 자리를 판정하려 했고**, ⚠ **그 함정(「200%」가 두 값)을 바로 이 라운드에 «내가» U8 로
닫아 놓고 나 자신이 걸렸다.** 규율을 세운 것과 그것을 자기에게 적용하는 것은 다른 일이다.
★ 그리고 위계 판정은 **애초에 높이와 무관**했다(8/28 「출처 증거 = 위계 3순위」 · §3 「행동 필요 정보 1순위」).

**주석에 넣은 것**: 판정 + 실측표 + **«높이를 근거로 다시 열지 마라»** + **남는 사실을 «관찰»로**
(기본 배율에서 원본 자료 1,636 > 일정 1,391 · 원인은 배치도 733×2083 → 951px · ⛔ 줄이거나 접어서 고치지 마라 —
8/28 이 «게시 블로커 아님»으로 판정했고 그 근거는 **`unoptimized` 와 한 쌍**이며 §0.4 가 접기를 막는다).

#### ⚠ 「`※` 중복」 지적 — **코드 안 고쳤다. 지우면 게시 조건 2 가 깨진다**

리더가 *「표 앞뒤로 `※ 세부 프로그램은 조정될 수 있음.` 이 두 번 나온다 — 뒤엣것을 지워라」* 를 냈다.
**증거 3건이 «설계»임을 가리킨다:**

1. ★★ **지시서가 «두 번»을 명시한다** — `MERGED-TABLE.md` §4-2 배치도에 **「① 1회차(구 caption)」 + 「① 2회차(금노 열 대응)」**,
   계약도 **「종류 2 · 출현 3(① **2회** + ② 1회)」**, §3-6 마크업에도 두 줄이 있고 **`{/* ① 2회차 */}`** 라 주석까지 붙어 있다.
2. ★★★ **게시 조건 2 가 «2곳»을 요구한다**(`page.tsx:267` · 검증 조건 2 · §51-1):
   *「표와 같은 화면에 없으면 게시 불가 — **표가 길어 표 위 + 표 아래 2곳**에 둔다」*
   ⚠ **병합으로 표가 «더» 길어졌다**(루트 32px 11,063px) → **필요가 «줄지» 않고 «늘었다».**
3. **병합 «전»에도 2회였다**(`git show HEAD` — `<caption>` 571 + `<p>` 618).
   **내가 한 것은 `<caption>` → `<p>` 교체뿐**이고 **개수는 안 바뀌었다.** 병합이 만든 중복이 아니다.

→ **뒤엣것을 지우면 «게시 조건 2 위반»이다.** 그래서 **지우지 않고 충돌을 보고했다.**

#### 중복 전수 검사 (리더 요청 · **렌더** 문장 기준 · 주석 제외)
렌더 문장 **80개**(6자 이상) → 중복 **2건**, **둘 다 정당**:
| 문장 | ×2 인 이유 |
|---|---|
| `※ 세부 프로그램은 조정될 수 있음.` | **게시 조건 2**(표 위·아래) — 위 참조 |
| `금융노조 QR코드` | **원문 축자** — `MEMBER_SCHEDULE` 3행·5행(시작/종료 출석체크)이 **같은 비고**를 갖는다 |
→ ★ **병합이 만든 중복 «0건».**

#### ★ 다만 리더 지적의 «절반»은 값지다 — 계약에 «자리»가 없다
*「개수 계약은 «같은 문자열 두 번»과 «두 문자열 한 번씩»을 구별하지 않는다」* 는 **참이다.**
이번엔 그 3 이 **정당하게** 채워졌지만, **계약이 그것을 «보증하지» 못한다는 것도 참**이다.
→ **자리를 넣은 계약안**(내가 지우지 않고 제안만 한다):
```
...
#### 판정: **A 확정 — 현행 유지.** 리더가 «거짓 실패»였음을 확인하고 철회했다
리더 자평: *「중복을 보고 «실수»라고 단정했다. «왜 두 번인가»를 묻지 않았다. 근거는 «바로 그 상수 위 주석»에
있었다 — 한 줄만 위로 읽었으면 나왔다」* · *「내가 팀에 반복 경고한 그 형태(«사라져야 하는 것» 목록에
정당한 출현이 섞여 개발자가 정상 코드를 되돌린다)에 내가 걸렸다」*.

**반영**: `※` 계약을 **«개수»가 아니라 «자리»로** 다시 썼다(`CHANGE_NOTE` 머리 주석).
★★ 그 주석에 **«양방향으로 안 지켜진다»를 «둘 다 실제로 났다»로** 적었다 —
① 중복을 못 잡는다(3 이 채워져 정상 통과) ② ★ **정당한 중복을 «중복»으로 오인한다(2026-09-01 실제 발생 · 지웠으면 게시 조건 2 위반)**.
**②의 사례가 이 주석 안에 있다는 것이 이 기록의 값**이다 — 추상 경고가 아니라 **일어난 일**이다.

#### ★ 중복 전수 검사는 «헛되지 않았다» (리더 지시로 남긴다)
**렌더 문장 80개(6자 이상) 전수 → 병합이 만든 중복 «0건».**
검출된 2건은 `※ 세부 프로그램…`(게시 조건 2)·`금융노조 QR코드`(원문 축자 · 3행·5행 같은 비고).
★ **«결과가 0 인 것»과 «안 해 본 것»은 다르다.** 병합처럼 문장을 옮기는 작업에서는 **이 검사가 기본값이어야 한다.**

#### ★ 이 라운드에서 확인된 협업 규율
리더 축자: *「리더 지시가 계약과 충돌할 때 «따르지도 무시하지도 않고 충돌을 드러낸 것»이 정확하다.
앞으로도 그렇게 해라. 내 지시가 근거를 못 대면 되물어라」*
→ **증거를 대고 · 두 길을 제시하고 · 「조건 위반을 모르고 하지 않도록 먼저 알린다」고 적는 것**이 그 형태다.

---

## 2026-09-01 · 지도 «선택 전/후»(①) + «강조»(②) 구현 — `MAP-EMPHASIS.md` 확정본 §1·§2·§10

### 바꾼 파일 3개 · 신규 파일 0 · 신규 색 0 · 신규 토큰 0 · 신규 오버레이 0

| 파일 | 무엇 |
|---|---|
| `src/components/strike/StrikeMap.tsx` | `pillHtml` `box` 배경 2층화 + `color` 변수화 · `toiletBadgeHtml` 배지 배경 2층화 · 히트 span 2곳 `aria-pressed="false"` · **선택 표시 동기화 effect 신설** · ②-1 판 · `GO`/`REFERENCE` 정의 주석 |
| `src/app/globals.css` | `[data-strike-hit][aria-pressed="true"]` 규칙 + `@media (forced-colors: active)` 블록 (**기존 두 규칙은 한 글자도 안 고쳤다**) |
| `src/lib/strikeMap.ts` | `stage-2` `tone: "reference"` → **`"go"`** · `StrikeMapTone` 의 `go` 정의 갱신 · `koscom-area` 근거 갱신 |

### `tone: "go"` 인 지물 = **4개** (종전 3개)
`koscom-area` · **`stage-2`(신규)** · `gwanghwamun` · `cityhall`
→ 정의를 **`StrikeMapTone` 주석에 다시 썼다**: *「`go` = 조합원의 «경로»(내리는 역 → 우리 무대 → 우리 구역)」*.
종전 문면 *«조합원이 갈 곳(코스콤지부 구역·역)»* 은 **죽었다고 명시**했다(3곳: 타입 주석 · `StrikeMap.tsx` `GO` 상수 · `koscom-area` 근거).
⛔ 함께 박은 금지: *«갈 곳처럼 보인다»로 다른 지물을 `go` 로 옮기지 마라 — **원문이 «소속»을 말하는 지물만** `go` 다.*

### ★ `stage-2` 는 «hue 하나»만 옮겼다 — 원과 pill 이 함께 간다
`drawFeature` 가 `const color = toneColor(feature.tone)` **하나로 도형과 pill 을 둘 다 그린다**
→ 리더 판정(«원도 함께 `go`»)이 **코드 1줄로 이행된다.** 분기를 만들지 않았다.
`confidence`·`fillOpacity`·`strokeStyle`·`casingWeight`·`zIndex`·`placement`·`glyph`·문면 **전부 불변**(= 확신도 축 무변경).
범례 9행도 무변경 — 범례 glyph 는 `text-ink` 라 `tone` 을 안 쓴다(실측 확인).

### ★★ 내가 «재확인해야 한다»고 지시받은 것 — §9-1 `background-clip` 2층 기전 : **성립한다**

**측정 환경표**: `http://localhost:3100/strike-2026-09-04`(내 프로세스 PID 96694 · 단독 리스너) ·
뷰포트 `360×640` · `innerWidth − clientWidth = 0` · dpr 1 · **루트 폰트 12px** ·
`visibilityState "visible"` · 글자크기 `localStorage` = `null` · 빌드 = 방금 `npm run build` 산출물
(지문: 프리렌더에 `border-2 border-primary bg-primary-tint` **1** · 종전 `<p>` 형태 **0**).
**주입 노드에 쓴 스타일 문자열은 내가 다시 타이핑한 것이 아니라 `.next/static` 청크에서 뜯어낸 «프로덕션» 문자열**이다.

| 형태 | `getComputedStyle().backgroundClip` | 판정 |
|---|---|---|
| **2층**(우리 구현) | **`"content-box, border-box"`** | ★ **둘 다 산다** |
| **1층**(§7-2 가 금지한 형태 · 대조군) | **`"content-box"`** | ★★ **둘째 값이 «실제로» 버려진다** |

- `aria-pressed="true"` → `--strike-hit-plate: #1a1a1a` · `--strike-hit-ink: #fff` · `color` 가 `rgb(9,51,137)` → `rgb(255,255,255)`.
  `aria-pressed="false"` → 두 변수 **unset**(폴백이 산다).
- **`backgroundColor` 는 두 상태 모두 `rgb(255,255,255)`** — 판은 «이미지 층»이라 배경«색»을 안 건드린다.
  이것이 **border-box 를 흰색으로 유지**하는 기전이다.
- **`getBoundingClientRect` 가 네 형태 모두 동일**(`[18, 36.59]`) → 쉬는↔선택 **delta 0**(2.5.8 무영향의 방증).
- 프로브 철거를 **«값으로»** 확인했다(`getElementById(...) === null` → `true`) · 루트 인라인 폰트 복원도 값으로 확인.

⚠ **내가 «안» 잰 것을 밝힌다**: ⓐ 이것은 **주입 노드**다 — «네이버가 그린 마커»가 아니다.
  기전(CSS 층·클립·변수 캐스케이드)은 **실페이지 스타일시트 위에서** 쟀지만 **쉬는 상태 픽셀 대조(§9-1 후반)는 못 했다.**
ⓑ `backgroundImage` 층 수를 «콤마 분해»로 세려다 **4** 가 나왔다 — `rgb(a,b,c)` 안의 콤마까지 세는 **내 정규식 결함**이다.
  **이 수를 인용하지 마라.** 층 수의 참 신호는 **`backgroundClip` 이 몇 값을 살렸는가** 다(위 표).

### ⛔ 못 잰 것 — **`localhost:3100` 에서 지도가 안 뜬다(401)**

```
GET https://oapi.map.naver.com/v3/auth?ncpKeyId=x79smqla3u&url=http%3A%2F%2Flocalhost%3A3100%2F… → 401
```
키가 **`localhost:3000` 으로만 등록**돼 있다(포트가 인증 대상 URL 에 들어간다).
→ **네이버가 그린 마커·도형이 필요한 측정은 전부 3100 에서 «못 잰다»**: §9-1 후반(픽셀 대조) · §9-3(2.5.8 전후 delta) ·
§9-4·5·6·8·9·10 · §2-1 `stroke-dasharray` 재확인 · 새 항목(무대 2 원 ↔ 코스콤 사각형 «두 지물로 읽히는가»).
⛔ **3000 에서 재지 않았다** — 리더 프로세스이고 **구 빌드를 서빙**한다(§5.7 «낡은 «프로세스»»).
⚠ **이것을 «통과»로 적지 않았다.** 리더 회부(아래).

### ②-1 판 — 자체 위생 점검(전수 sweep 은 §9-7 로 QA 이관)
`320×640` · 스크롤바 0 · dpr 1 · 지도는 폴백 상태(위 401):

| 루트 | `scrollWidth − clientWidth` | 판 rect | 판 안쪽 `scrollWidth` / `clientWidth` |
|---|---|---|---|
| 12px | **0** | 296 × 55.75 | 274 / 292 |
| 24px(=200%) | **0** | 272 × 175 | 232 / 268 |

문면은 `KOSCOM_LOCATION_NOTE` **축자 그대로**(`innerText` 대조 일치). `mt-4`·`max-w-` 를 `<p>` → 래퍼로 옮겨 중복 0.
⚠ **가로만 봤다.** 6폭 × 2배율 전수와 세로 예산은 §9-7 그대로 QA 항목이다.

### ⚠ 이 라운드에 내가 «공유 자원»을 만진 것 — 밝혀 둔다
- **공유 브라우저가 §9 경고대로 오염돼 있었다** — 탭 4개 중 3개가 **포트 3000**(리더 서버)이고 1개가 리더의 `bgclip.html` 프로브였다.
  **매 `evaluate` 첫 줄에 `location.origin !== "http://localhost:3100"` 가드**를 넣었다. 뷰포트는 `360×640` 로 되돌렸다.
- **`git stash` 를 한 번 썼다**(프리티어 경고가 «내 회귀인가»를 가르려고 변경 전 상태를 재려 했다).
  ⚠ **공유 워킹트리에서 할 일이 아니었다.** 왕복 후 `git stash list` 빈 것 · `design/` 16개 파일 전수 · `git status` 를
  **값으로** 확인했다 — 손실 0. **다음엔 `git show HEAD:<path>` 로 읽어야 한다**(트리를 안 흔든다).
- 프리티어는 **변경 «전»에도 이 3파일에서 실패**했다(두꺼운 주석이 print width 를 넘는다) → **내 회귀가 아니다.**
  이 저장소의 게이트는 `npm run lint`(eslint)이고 그쪽은 통과다.

### 검증 결과
`npx tsc --noEmit` **exit 0** · `npm run lint` **출력 0** · `npm run build` **성공**(정적 10/10 · `/strike-2026-09-04` ○).

### QA 이관 목록 — §9 13건 + 신규 2건, **«잴 수 있는가»로 먼저 갈랐다**

⚠⚠ **선행 조건 하나가 모든 것을 가른다**: 아래 **12건이 «네이버가 그린 마커·도형»을 요구**하는데
**키가 `localhost:3000` 으로만 등록**돼 있다. → **QA 는 «신 빌드를 3000 에서» 서빙하거나, 콘솔에 측정용 포트를 등록해야 한다.**
⛔ **구 빌드가 도는 3000 에서 재고 «통과»로 적지 마라** — 그것이 §5.7 이 못박은 «낡은 프로세스» 함정이다.

| # | 항목 | 상태 | 측정 방법 / 판정선 |
|---|---|---|---|
| 9-1a | `background-clip` 2층 **계산값** | ★ **내가 닫았다 — 성립** | 위 표. `"content-box, border-box"` · 1층 대조군은 `"content-box"` |
| 9-1b | **쉬는 상태 픽셀 대조** | ⛔ **지도 필요** | 변경 전/후 같은 z16·같은 뷰포트에서 pill 7 + 배지 2 를 3배 확대 캡처 → 세로 단면 RGB 열 비교. **완전 일치**. padding 자리가 타일색이면 **즉시 반려** |
| 9-2 | **강제색 모드 선택 전/후 차이** | ⛔ **지도 필요** · ★ **미실측 블록** | DevTools › Rendering › `forced-colors: active`. 선택 전/후 `backgroundColor`·`color` 비교 + 캡처. **차이 0 이면 게시 조건 미달** → `border-width`·`text-decoration` 으로 재설계 |
| 9-3 | 2.5.8 **전후 delta 0** | ⛔ **지도 필요** | 환경표 세우고 변경 «전»/«후» 히트 5개 `getBoundingClientRect` + 히트↔히트 2차원 최소 + 히트↔장식 최소. **delta 0.** ⛔ `67.54`/`68.0`/`67.98` 을 기준으로 쓰지 마라 |
| 9-4 | 선택 + 포커스 **동시** | ⛔ **지도 필요** | Enter 로 선택 → **Shift+Tab 으로 히트 복귀** → `boxShadow`(이중 링)·`backgroundImage`(판) 동시 확인. 한쪽이 사라지면 반려 |
| 9-5 | 배지 점선의 «틈»이 흰색인가 | ⛔ **지도 필요** | 선택 전/후 배지 5배 확대, 테두리 고리를 x 스캔. 틈이 **`#ffffff`**. 타일색이면 M-70 표시가 죽는다 → 반려 |
| 9-6 | 무대 pill 4개가 **안 반전되는가** | ⛔ **지도 필요** | 무대 pill `getComputedStyle().backgroundImage` 두 층이 전부 투명/흰색인지 + 픽셀 대조. **오늘과 동일** |
| 9-7 | ②-1 판 세로 예산·가로 넘침 | ⭕ **지도 없이 가능** · 내가 320 만 봤다 | `320·360·390·412·768·1280` × 기본·200% 로 `scrollWidth − clientWidth`·`body.scrollHeight`·`figure` 높이. **가로 넘침 0.** ⚠ 끝나면 `documentElement.style.fontSize` 를 지우고 **«값으로»** 확인 |
| 9-8 | z15·z16·z19 에서 판이 읽히나 | ⛔ **지도 필요** | 각 배율에서 히트 5개 선택·캡처. 마커는 화면 고정 px 이라 배율 무관해야 한다 — **«무관하다»를 값으로 확인** |
| 9-9 | 대비 **실측**(계산이 아니라) | ⛔ **지도 필요** | 선택된 pill·배지 5배 확대 → 판/padding/테두리/픽토그램 픽셀 RGB → WCAG 식. 판↔흰 **≥ 17** · 픽토그램↔판 **≥ 3.0** |
| 9-10 | 마커 DOM 재생성 시 `aria-pressed` 잔존 | ⛔ **지도 필요** | 선택 → bounds «안»에서 큰 드래그 · 줌 1단 왕복 · 탭 전환 후 복귀 → 값 확인. **`true` 유지.** 사라지면 네이버가 노드를 갈아 끼운 것 → 별도 처방 |
| 9-11 | `aria-pressed` ↔ `aria-expanded` 낱말 | ⭕ **접근성 검증 회부** | VoiceOver·NVDA 2종 낭독. 같은 페이지 거리뷰 토글도 `aria-pressed` 라 **한 페이지에 두 뜻**인지 판정. **시각 처방과 독립 — 어느 쪽이든 이 구현은 안 흔들린다** |
| 9-12 | §5-1 타일색 가정(`#f8f9fa`) | ⛔ **지도 필요** | 무대 원 채움 자리 픽셀 **표본 20점** → `fillOpacity 0.10` 실효 대비 분포. 낙관이면 §5 판정이 더 강해지고 **비관이면 리더 회부** |
| 9-13 | `stroke-dasharray` 파생 규칙(원 `w,w` / 폴리곤 `1,2w`) | ⛔ **지도 필요** · ⚠ **리더가 나에게 재확인 지시 → 못 했다** | 원·폴리곤 SVG 의 `stroke-dasharray` 직접 판독. **이 안은 굵기를 안 바꾸므로 결론에 영향 0** — 다만 «굵기를 강조에 쓰자»가 다시 올라오면 **이 값부터 재라** |
| **N-1** | ★ **무대 2 원 ↔ 코스콤 사각형이 «두 지물»로 읽히는가** (리더 신설 · 1.32 m · 같은 `go` 색) | ⛔ **지도 필요** | `320·360·390·412·768·1280` × 기본·200% × z15·z16·z19 캡처. ⛔ **«읽힌다»를 가정으로 적지 마라 — 실측 항목이다.** 판정선을 QA·디자이너가 세워야 한다(개발자가 정하지 않았다) |
| **N-2** | ★ **`stage-2` pill 폭이 «정말» 안 변했는가** | ⛔ **지도 필요** | §10 이 *«색만 바뀌어도 «재측정 불필요»로 «단정»하지 마라»* 로 명시 요구. 무대 2 pill `getBoundingClientRect` 를 변경 전/후로. **delta 0** · 특히 **히트↔장식 최소 2.0 px 자리** |
