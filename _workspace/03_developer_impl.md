...
### E-9. ⚠ `FIT_MAX_ZOOM` 도입 — **적용했다가 철회했다** (QA 23회차 실패 1건 취소)

**결론부터: 원래 코드에 결함이 없었다. `RallyMap.tsx` 는 `637d4e9b7768d1c3…` 로 되돌렸다.**

#### 무엇이 실제로 일어났나

QA 가 보고한 *"1280px 축척 50m · ② 라벨 −24px 잘림"* 은 **측정 환경의 산물**이었다.
QA 가 `next start` 로 서빙했는데 **이 프로젝트는 `output: "standalone"`** 이라 정적 자산이 온전히 서빙되지 않아
**레이아웃이 축소**됐다.

| | `next start`(잘못된 환경) | **standalone**(올바른 환경) |
|---|---|---|
| 360 박스 | 313×391 | **328×410** |
| 768 박스 | 689×388 | **704×396** |
| 1280 축척 | **50m** | **100m** |

박스가 작아지자 `fitBounds` 가 1280 에서 다른 zoom 을 골랐고 ② pill 이 밖으로 나갔다.
**정상 환경에서는 3뷰포트 전부 100m · 잘림 0 이고, 360 좌표가 내 기준선(①82,131 ②68.5,30 ③276,293)과 일치한다.**

#### 왜 무해해 보이는 상수를 남기지 않았나

1. **주석이 거짓 근거를 담고 있었다** — *"⑤ 제거로 fitBounds 범위가 좁아져 1280px 에서 z17 이 걸렸다"* 는
   **사실이 아니었다.** 잘못된 근거가 코드에 남으면 **다음 사람이 그것을 믿는다.**
2. **근거 없는 상한이 된다.** 지점이 늘어 범위가 넓어지면 z16 상한이 오히려 제약으로 작동하는데
   **왜 있는지 아무도 모르게 된다.**
3. 원래 코드에 결함이 없었다.

#### 남길 가치가 있는 판단 1건 — **권고를 그대로 따르지 않은 이유**

QA 권고 문구는 **`MAX_ZOOM: 16`** 이었다. 그대로 따랐다면 우리 코드의 `MAP_MAX_ZOOM`(19)을 16으로 낮췄을 텐데,
그 상수는 **사용자 조작 상한**이라 **조합원이 지도를 확대할 수 없게 되고** §21.1.1 의
`minZoom 15 / maxZoom 19` 계약이 깨진다. 고쳐야 하는 것은 **초기 화면(`fitBounds`)의 상한**이라
별도 상수로 분리했다.

> **규칙: 권고가 지목한 "값"이 아니라 그 권고가 고치려는 "증상"을 보라.**
> 같은 이름의 상수가 두 가지 일을 하고 있으면, 권고를 문자 그대로 적용하는 순간 **엉뚱한 계약이 깨진다.**

(결과적으로 수정 자체가 불필요했지만, **상수를 분리한 판단은 유효**하다 — 실제로 z16 상한이 필요해지는 날이 오면
`MAP_MAX_ZOOM` 을 낮추는 것이 아니라 이 방식으로 분리해야 한다.)

#### 부수 기록 — 빌드 사고 1건

수정 후 첫 기동에서 **`ChunkLoadError`(404)** 가 났다. **이전 `next start` 가 살아 있는 채로 빌드**해
산출물이 섞인 것이다. QA 서버가 측정 중 4번 죽은 것과 **같은 계열**이다.
**서버를 내리고 빌드한다 — 예외 없다.**

#### 되돌린 뒤 확인

`RallyMap.tsx` **`637d4e9b7768d1c3…`**(QA 검증 해시와 일치) ·
`rallyMap.ts` `1d86029a…` · `page.tsx` `e7e2cbef…` — 셋 다 동결 시점과 동일.
`tsc` 0 · `eslint` 0 · `build` 성공. **서버는 내려 둔 상태**이고 재측정은 불요(QA 검증 완료 해시).
