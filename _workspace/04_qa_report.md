...
## §54 QA (2026-08-29 · 9/4 지도·거리뷰)

> 대상: 신규 `src/lib/strikeMap.ts` · `src/components/strike/StrikeMap.tsx` · `src/components/strike/StrikeRoadviewSheet.tsx` · 변경 `src/app/strike-2026-09-04/page.tsx`
> 체크리스트: 디자인 `§54.14`(492~507 · **498 폐기**) · `§54.16-8`(508~522) · `§54.16-13`(523~530)
> ⚠ **`§54.16-0` 의 죽은 절 6건(§54.0-3 · §54.3 · §54.4-2 · §54.5-3 · §54.13-7 · §54.13-12)은 채점하지 않았다.**

### §54-QA-0 판정 — **체크리스트 38항목: 통과 34 · 실패 4** + **체크리스트 밖 실패 1** | **미검증 3**

- 체크리스트(492~530 · **498 폐기 제외**) **38항목** 중 **실패 4**: `499`·`514`(→ F2) · `523`(→ F3) · `528`(→ F4) · `522`(→ F5). 나머지 **34 통과**.
- 체크리스트에 없는 접근성 요구에서 **실패 1**: **F1 — 지도 컨트롤 `:focus-visible` 링이 그려지지 않는다**(WCAG 2.4.7).
- **게시 차단 0건.** 다만 **F1 은 그 버튼이 «키보드 사용자의 유일한 확대 경로»**(§54.16-1 이 `keyboardShortcuts:false` 를 택한 대가)라 **게시 전 수정을 강하게 권고**한다.
- **F4·F5 는 산출물이 아니라 «문서»를 고치는 항목**이다(기대 개수의 계층 · 세로 예산 예측값).

---

### §54-QA-1 측정 환경표 (§5.5 · §5.6 · §5.8.4)

| 항목 | 값 |
|------|-----|
| 측정일 | **2026-08-29** |
| 도구 | **Playwright MCP (Chromium · dpr 1)** |
| 서버 | **프로덕션 `output: standalone`**(`node .next/standalone/server.js`) · `next start` 미사용 · `.next/static`·`public` 복사 후 기동 |
| 포트 확인 ① | 기동 전 `lsof -nP -iTCP:3000 -sTCP:LISTEN` **0건** → 기동 후 **PID 26050**(2회차 **28241**) · 로그 `EADDRINUSE` **0건** |
| 확인 ② | **서버 응답 sha256 == 디스크 프리렌더 sha256** — **같은 시각 3회** 일치. ⚠ 원본 해시는 ISR 캐시 수치 때문에 갈리므로 **`<script>` 제거 후** 비교했다(§5.7 오탐 모드 ①). 제거 후 `0dd3833d…`(19,611바이트) **3/3 일치** |
| 확인 ③ | 정적 자산 **전수 11개 200** (`grep -oE '/_next/static/[A-Za-z0-9._/-]+'`) |
| 빌드 동결 | `BUILD_ID` `pOjIwVviUFApFb4-20sYq` · `build-manifest.json` mtime 불변 · **빌드보다 새로운 소스 0건** |
| 뷰포트 | **375×640**(`clientWidth 360`) · **360×640**(`clientWidth 345`) · **1280×800**(`clientWidth 1265`) |
| **`innerWidth − clientWidth`** | **15**(클래식 스크롤바) — 세 뷰포트 전부 |
| 루트 폰트 | 기본 **12px**(`html{font-size:75%}`) · 확대 **32px**(인라인) |
| `visibilityState` / rAF | `visible` / **1.5초 안에 발화 ⭕** |
| 오리진 | ★ **`127.0.0.1:3000` 은 지도 키 화이트리스트 밖이라 401 이 난다.** 정상 측정은 **`localhost:3000`**, 401 방화벽 측정(QA-495)은 **`127.0.0.1:3000`** 에서 했다 — **키를 위조하지 않고 실제 401 을 얻었다** |
| **정리 확인**(§5.8.4) | 인라인 `font-size` 제거 후 computed **12px** · 크롭 프로브 노드 **잔존 0**(`getElementById` 로 값 확인) · `MapControlStack` 인라인 `overflow` 원복(**computed `hidden`** · 인라인 `""`) · **내가 쓴 `localStorage` 키 `koscomlabor:strike-streetview-height` 삭제 확인**(`null`) · **8/28 키 `koscomlabor:roadview-height` 는 끝까지 `null`** |
| ⚠ 잔존 상태 1건 | `localhost:3000` 오리진에 `koscomlabor:font-scale = "75"` 가 있다. **내가 쓴 것이 아니다**(슬라이더 미조작 · `apply()` 만이 쓴다) — 다만 **내 측정 «전» 상태를 기록하지 못했다.** 값이 `DEFAULT(75)` 와 같고 **루트가 매 측정에서 12px 로 확인**되므로 오염은 없다 |
| ⚠ 빌드 교체 1회 | **QA-496 은 «키 없는 빌드»가 필요해 측정 «후»에 `NEXT_PUBLIC_NAVER_MAP_CLIENT_ID=""` 로 재빌드**했다(BUILD_ID `6a3UuOaB1jPU8bM62_3Oc`). 그 뒤 정상 키로 복구했고(`zRn4KCC8Y9NFz9bvyB93m`), **복구 빌드의 `<script>` 제거 프리렌더 해시가 측정 회차와 완전히 같음**(`0dd3833d…`)을 확인해 **모든 수치가 현재 산출물에 그대로 유효**함을 증명했다 |

---

### §54-QA-2 실패 항목 (심각도순 · 5건)

| # | 분류 | 위치 | 실측 / 기대 | 수정 방법 |
|---|------|------|------------|----------|
| **F1** | **접근성 — WCAG 2.4.7 (Focus Visible)** | `StrikeMap.tsx:503`(`MapControlStack` 의 `overflow-hidden`) × `:485`(`focus-visible:outline-offset-2`) | **실측**: `확대` 버튼에 포커스가 갔을 때 computed 는 `outline: 3px solid rgb(9,51,137)` · `outline-offset: 2px` 인데 **화면에 아무 링도 그려지지 않는다.** **대조 실험으로 원인 확정** — 스택의 `overflow` 만 `visible` 로 바꾸니 링이 나타났다(픽셀 차분 **632px 변화 · bbox (8,8)-(62,101)**). 버튼 box 와 스택 box 가 **정확히 같은 rect**(x21 y861.2 w44)라 **바깥 2~5px 링이 통째로 클리핑**된다. **기대**: 포커스 표시가 보일 것. ⚠ 대상은 **확대·축소·처음 위치로·거리뷰 토글 4개 전부**이고, 이 버튼들이 **`keyboardShortcuts:false` 하에서 키보드 사용자의 유일한 확대 경로**다 | **두 안 중 하나. 디자이너 판정 필요**(§54.16-6 (3) *«포커스 링은 버튼 «바깥»»* 과 충돌한다). ⓐ `MapControlStack` 에서 `overflow-hidden` 을 빼고 모서리를 자식에 준다(`[&>*:first-child]:rounded-t-card [&>*:last-child]:rounded-b-card`) — 판정 유지. ⓑ **안쪽 링**으로 바꾼다: `MAP_CTRL_CLASS` 에 `focus-visible:-outline-offset-3 focus-visible:outline-white`(남색 면 위 흰 링 **11.37**), `MAP_CTRL_ON_CLASS` 는 `focus-visible:outline-primary` 유지(흰 면 위 남색 **11.37**) — **판정을 뒤집는 것이므로 근거를 다시 써야 한다** |
| **F2** | 겹침 (초기 뷰) | `strikeMap.ts:481`(`cityhall-toilet` 좌표) ↔ `:511`(`cityhall` 좌표) · 렌더는 `StrikeMap.tsx:250`·`:273` | **실측**(375×640 · 상자 336×420 · z16 · 상자 기준 좌표): 화장실 배지 span rect `(174,301)-(194,321)` + 흰 링 2px → **시각 중심 (184,311) 반지름 12** / 시청역 점 span rect `(171,316)-(183,328)` + 흰 링 3px → **시각 중심 (177,322) 반지름 9**. **중심 거리 13.04px · 반지름 합 21 → 겹침 깊이 7.96px.** 사각 교차만으로도 **9×5px**. 스크린샷(`_workspace/screenshots/2026-08-29-strike-map/crop-dot-band.png`)에서 **화장실 배지의 `dashed` 테두리가 점의 흰 링에 좌하단이 잘린다.** **기대**: QA-514 «항목이 서로를 덮지 않는다» | ★ **§54.16-11 이 이 조합을 안 봤다** — 그 절은 «밴드 «안»에 든 항목 둘»만 다루고 **그 둘끼리의 겹침**은 다루지 않는다. **좌표 이동은 금지**(둘 다 근거가 다르다)이므로 **크기로 푼다**: 화장실 배지 20 → **16px**(흰 링 2 → 1.5) + 점 12 → **10px**(흰 링 3 → 2.5)이면 반지름 합 21 → **16.5 < 13.04** … 은 아니고 **16.5 > 13.04 라 여전히 겹친다.** → **완전 해소는 불가**(두 지점이 실제로 26 m 밖에 안 떨어졌고 초기 축척이 0.54 px/m 다). **판정을 리더/디자이너에게 회부한다**: ⓐ 크기 축소로 겹침 깊이를 7.96 → 3.5px 로 줄이거나 ⓑ **확대 1단이 해소한다**(z17 에서 겹침 **0** — 실측)를 근거로 «남는 위험»에 편입 |
| **F3** | 겹침 (QA-523 ③) | 같은 두 마커 ↔ `strikeMap.ts:175`(`COLUMN_4_POLYGON`) | **실측**: 대오 4 밴드 SVG bbox `x 171..184`(13px) · `y 269..357`(88px). 두 마커의 **불투명 흰 원**이 밴드 테두리를 덮는 세로 구간 — **동쪽 테두리(x=184): 화장실 24px ∪ 점 11.3px = 28.66px = 밴드 길이의 32.6%** / 서쪽 테두리(x=171): 점만 **13.42px = 15.2%**. 스크린샷에서 **동쪽 점선이 그 구간에서 끊겨 보인다.** **기대**(§54.16-11 (3) 남는 위험): **18px · 18%** → **실측이 예측의 1.6배** | ⚠⚠ **명시된 대체 처방 «점 지름 12 → 10px» 이 이 문제를 못 고친다.** 계산: 점 시각 반지름 9 → 8 이면 동쪽 기여 11.3 → 7.75px, **합집합 28.66 → 26.9px(32.6% → 30.6%)** — 동쪽의 지배 기여자는 **점이 아니라 화장실 배지(24px)** 다. → **§54.16-11 (3) 의 처방 문면을 고쳐라**(근거가 바뀌면 근거를 다시 쓴다). 실효안은 **화장실 배지 축소**이거나, **«남는 위험»으로 확정하고 C·D(범례 문장 `대오 4 — 무대 4 남쪽 구간입니다` · `◉` 기호)가 오독을 막는다**로 닫는 것이다. **QA 판정: 게시 차단 아님**(확대 1단에서 소멸 · 범례가 «구간»을 문자로 확정) |
| **F4** | 문서 정정 — 기대 개수의 «계층» | `02_designer_spec.md` §54.16-12 · `01_verifier_factcheck.md` §54-6 | **실측**: `5호선` — 프리렌더 **1** / **라이브 DOM 2** · `1·2호선` — 프리렌더 **1** / **라이브 DOM 2**. 초과분은 **지도 pill `광화문역(5호선)`·`시청역(1·2호선)`**(네이버 마커 안 raw HTML · `aria-hidden` 이지만 **화면에 보이는 텍스트**)다. **기대 표는 «각 1»** 로 적혀 있다 | 표에 **«어느 계층을 세는가»를 명시**하라 — 프리렌더 기준이면 `1`, 라이브 DOM 기준이면 `2`. **문면 결함이 아니다**(pill 문면은 §54.16-10 (1) 이 지정한 값이다). 같은 이유로 `대오`·`광화문역`·`시청역` 도 라이브에서 **18 / 3 / 5** 로 프리렌더(14 / 2 / 4)보다 크다 — **`≥` 기대라 통과** |
| **F5** | 문서 정정 — 세로 예산 | `02_designer_spec.md` §54.16-7 · §54.16-10 (4) | **실측**(375×640 · `clientWidth 360` · 루트 12px · 문서 좌표): 개요 `</section>` **694.16** → 지도 `</section>` **1604.42** = **910.26px**(`mt-section` 54 포함). **예측 «약 900»** → **+10.26px**. **원인 2건**(§5.9.3 «왜 갈렸는가»): ① **완화 안내 줄이 1줄이 아니라 2줄이다** — 예측 `23`(17+`mb-2` 6) vs 실측 **33.75 + 6 = 39.75**(`+16.75`) ② 범례 예측 `309.38` 에 **실재하지 않는 `mt-2` 6px** 이 들어 있다 — 실측은 `mt-4` **12 + `<ul>` 291.38 = 303.38**(`−6`). 나머지는 전부 일치(`mt-section` 54 · `h2` 23.39 · `figure mt-6` 18 · 상자 **420** · 코스콤 `mt-4` 12 + 16.88) | 예측표를 **910.26** 으로 갱신하고 **완화 안내를 «2줄»로** 적어라. **첫 화면 판정선 영향은 여전히 0**(삽입 지점 B 가 판정선 640 아래) |

---

### §54-QA-3 통과 항목 (체크리스트 전수 · 방법을 같이 적는다)

#### (1) 자리 · 첫 화면 · 상자

| # | 결과 |
|---|------|
| **492 자리** | `main > div` 직계 자식 순서 실측: 복귀링크 → `h1` → **집결시간** → **개요** → **`map-heading` 섹션** → **QR 한 줄** → 식순 → 복귀링크. **개요 다음 · QR 앞** ⭕ |
| **493 ★ 첫 화면 불변** | `375×640`(`clientWidth 360` · 스크롤바 15 · 루트 12px)에서 `<time>10시 30분` 하단 = **`upcoming` 334.52** · **`today` 361.52** — 기준(334.52 / 361.52)과 **소수점까지 일치** ⭕ **여유 278.48**. `today` 는 **`<p class="mt-4"><span class="rounded-full bg-primary px-3 py-1 text-caption font-bold text-white">오늘</span></p>` 삽입 + `h1` `mt-4`→`mt-3`** 으로 재현했고, §5.9.3 대로 **주입한 클래스가 실제로 컴파일됐는지 computed 로 검산**했다(`display:inline` · `bg rgb(9,51,137)` · `padding 3px 9px` · `font-size 11.25px` · `line-height 16.875px`). **원복 후 334.52 재확인 · 프로브 잔존 0 · `h1` 클래스 원복 확인** |
| 부수 실측 | 집결시간 `</section>` **410.33** · 개요 `</section>` **694.16** — 설계 실측과 동일 ⭕ |
| **494 · 513 상자 · CLS** | `clientWidth 360` → **336×420** · `1265` → **420×525** ⭕. **로딩 336×420 · 성공 336×420**(같은 페이지 연속 측정) · **실패(401) 336×420**(1280 에서는 420×525) — **세 상태 높이 동일 → CLS 0** ⭕. `main` 높이도 성공/실패 **둘 다 2671.63** (범례가 정적 데이터 파생이라 지도 상태와 무관) |
| **522 세로 재측정** | → **F5**(실측 910.26 · 원인 2건 분해 완료) |

#### (2) ★ 401 방화벽 · 대체면 · 키 미설정

| # | 결과 |
|---|------|
| **495 ★★ 401 방화벽** | **프로덕션 빌드 · 실제 401**(`127.0.0.1:3000` 오리진이 키 화이트리스트 밖 → 콘솔 `Failed to load resource: 401 … /v3/auth?ncpKeyId=…`). 마운트 노드에 네이버가 `auth_fail.png` 배경을 박은 것으로 **`new maps.Map()` 은 성공했고 오버레이도 생성된 뒤 인증이 깨진** 상태를 확인했다(= 정리 경로가 실제로 돌 조건). **결과: `main` 요소 1개 · `h1` = `9/4(금) 총파업 참석안내` · 개요(`세종대로`) ⭕ · 집결시간(`10시 30분`) ⭕ · 식순(`대오정비`) ⭕ · QR 한 줄 ⭕ · 범례 13행 ⭕ · 대체면 `지도를 불러오지 못했습니다.`** ⭕. ★ **가장 위험한 «언마운트» 까지 밟았다** — 그 상태에서 복귀 링크로 **클라이언트 내비게이션**해 `StrikeMap` 을 언마운트시켰고 `/bargaining-2026` 이 **`main` 1개 · `"This page couldn't load"` 0** 로 정상 렌더 ⭕. **`safely()` 가 작동한다** |
| **496 키 미설정** | **`NEXT_PUBLIC_NAVER_MAP_CLIENT_ID=""` 로 재빌드**(BUILD_ID `6a3UuOaB1jPU8bM62_3Oc`)해 서버 렌더를 계수: `세종대로 안내지도` **0** · `map-heading` **0** · `메인무대`(범례) **0** · `코스콤지부가 어느 대오인지는` **0** · `oapi.map.naver.com` **0** ⭕ **섹션이 통째로 사라진다.** 나머지는 전부 남는다: `집결시간` 1 · `개요` 1 · `10시 30분` 1 · `세종대로` 1 · `당일 QR 출석체크` 1 · `총파업 식순` 1 · `대오정비` 1. ★ **`대오` 가 1**(식순 축자만)로 떨어지는 것이 «의도된 상태»의 값 증거다 — 지도가 없으면 «대오 4개» 위험도 없다 ⭕ **죽은 어포던스 0** |
| **497 대체면** | 실패 상태 렌더 문자열(본문+속성)에서 `여의도` **0** · `국회` **0** · `집회 3구역` **0** · `더샵` **0** · `구역` **0** · `KDB` **0** · `산업은행` **0** · `국회의사당` **0** ⭕ |
| **518 거리뷰 401** | ⚠ **«키를 틀리게 하고 거리뷰를 켠다»는 구조적으로 도달 불가**다 — 401 이면 `status` 가 `failed` 라 컨트롤이 렌더되지 않아(**버튼 0개 실측**) 토글할 수 없다. **위험이 존재하지 않는다.** 대신 **같은 정리 경로를 정상 키로 밟았다**: 거리뷰 시트를 연 채(파노라마 인스턴스 살아 있음) 복귀 링크로 이탈 → `/bargaining-2026` **`main` 1 · 시트 제거 · 에러 화면 0** ⭕ |

#### (3) ★ 조작 · 완화 문구

| # | 결과 |
|---|------|
| **508 조작(498 대체)** | **드래그** — 합성 pointer/mouse 8스텝(−80px)으로 `메인무대` pill 이 상자 기준 `y 73.9 → −6.1` 이동 ⭕ · **`처음 위치로` 가 그때 나타남** ⭕. **확대** — `+` 클릭 z16→z17→z18→z19(타일 URL `/{z}/{x}/{y}.png` 로 판정) ⭕, **z19 에서 `확대` `disabled: true`** ⭕(`maxZoom 19`). **휠** — 지도 중앙에서 `WheelEvent(deltaY −300)` 디스패치 → **줌 불변 · `defaultPrevented: false`**(지도가 휠을 안 먹으므로 페이지가 스크롤된다) ⭕. **`처음 위치로`** — 클릭 후 z16 복귀 + **버튼이 사라짐**(`moved=false`) ⭕ |
| **510 ★ 완화 문구가 참인가** | **판정선 44px · 설계 예측 106px → 실측 «훨씬 크다».** 방법: 헤딩 sticky 헤더(**position `sticky` · 68px · z200**) 아래 영역 `640−68=572` 중 상자(420)가 안 덮는 세로 px 을 **스크롤 전 구간 1px 격자로 스윕**해 최악값을 찾았다. **합계 152px(고정)** · **최악 지점에서도 «위·아래» 중 큰 쪽이 76.17px**(scroll 708 · 위 76.17 / 아래 75.83) ⭕ **판정선의 1.73배**. **그리고 «실제로 스크롤되는가»를 기전으로 확인**했다 — 그 지점에서 `elementFromPoint` 히트 체인의 `touch-action` 이 **위: `P.mb-2 → FIGURE → SECTION → DIV → MAIN → BODY` 전부 `auto`**, **아래: 전부 `auto`**, **지도 위: `DIV.size-full: none`**(알려진 제약) ⭕. → **`MAP_GESTURE_NOTE` 의 세 주장이 모두 참이다** |
| **520 키보드** | **방향키** — `확대` 버튼에 포커스를 두고 `ArrowDown` → **`scrollY 742 → 782`(페이지가 스크롤됨) · 지도 줌 불변 · `처음 위치로` 미출현** ⭕(`keyboardShortcuts:false` 확인). **마운트 노드 `tabindex` 제거 확인**(`null`) ⭕. **탭 도달** — 상자 안 포커스 가능 요소 8개(네이버 링크 5 + 우리 버튼 3) · 우리 버튼 전부 도달 ⭕. **엔터 실행** — `거리뷰` 버튼 포커스 후 `Enter` → `aria-pressed false → true` · 라벨 `거리뷰 모드 켜기 → 끄기` ⭕ |
| **505 ResizeObserver** | ★ **«부모에 걸렸는지»를 값으로 확인했다.** 상자(부모)의 `max-width` 를 인라인으로 336 → 280 으로 줄이자 **`--strike-label-max` 가 `235px → 196px`**(=280×0.7)로 따라왔고, 원복하니 **235px 복귀** ⭕. `syncLabelWidth()` 는 **RO 콜백에서만** 불리므로 이것이 «RO 가 실제로 울렸다»의 직접 증거다. (마운트 노드에 걸렸다면 네이버가 그 노드에 인라인 px 를 박으므로 영영 안 울린다.) 인라인 원복 확인 |

#### (4) ★ 겹침 · 배율 축 · 도형

| # | 결과 |
|---|------|
| **499 · 514 겹침(초기 뷰)** | pill **10개**(대오 4 · 무대 4 · 역 2 — 실측 개수 일치) · 배지 6개(화장실 3 · 역 점 3). **pill ↔ pill 교차 0** ⭕ · **pill ↔ 도형(자기 밴드/원) 교차 0** ⭕ · **`처방 B` 검증**: 시청역 pill `l 197` 이 밴드 동단 `184` 보다 **13px 밖** — «밴드를 통째로 덮는» 문제 해소 ⭕. ⚠ **배지 ↔ 배지 1건 = F2**. ★ **8/28 전례(«지도 저작권 표기가 시트 위»)를 사각 교차만으로 넘기지 않았다** — `.map_copyright`(`0,401`–`88.2,420`)·축척 바(`207,403`–`336,420`)·컨트롤 스택 3개의 rect 를 전부 넣어 교차를 돌렸고 **pill·배지와 교차 0** ⭕ |
| **524 ★ 배율 축** | **z16 · z17 · z18 · z19 네 배율**에서 «화면 «안»에 있는 항목만» 골라 반복. **마커 겹침**: z16 **1건(F2)** → z17 **0** → z18 **0** → z19 **0** ⭕ (**확대가 해소한다**는 처방 E 가 실증됐다). **★ 양방향으로도 쟀다** — «확대하면 도형이 커지고 pill 은 그대로»: z18 에서 **`대오 2`·`대오 3` pill 이 이웃 «무대 원» bbox 를 스친다**(원 면적의 **0.6% · 0.9%**), z19 **1건 1.3%**. **자기 도형(밴드)과의 교차는 전 배율 0** ⭕ → **QA-514 기준(«자기가 가리키는 도형»)으로는 통과.** ⚠ 원을 **사각 bbox 로 근사**했으므로 «실제 원호와 닿는가»는 미확정(→ 관찰 O2) |
| **523 ①②** | ① 시청역 점이 **흰 링 3px 로 밴드 위에 «얹힌 층»으로 보인다** ⭕ ② 화장실 픽토그램도 **흰 면 + 흰 링 2px 로 얹힌 층** ⭕ (스크린샷 `crop-dot-band.png`). ③ → **F3** |
| **503 · 509 확신도 단일** | **z19(최대 확대)에서** 상자 안 모든 도형 stroke 를 계산값으로 집계: **`#ffffff | 7px | (dash 없음)` ×8**(casing) · **`#4b5563 | 3px | 3,3` ×4**(무대 원) · **`#093389 | 3px | 1,6` ×4**(대오 밴드). **`solid`(dasharray 없는 유채색 stroke) 0** ⭕ · **점선이 확대에서 실선이 되지 않는다** ⭕(스크린샷 `strike-map-z19.png` 육안 일치). ★ **`3,3` 도 `1,6` 도 둘 다 `shortdot` 이다** — 네이버 `maps.js` 의 `DASH_STYLE.shortdot = "1,1"` 을 `_getDashStyle` 이 `o*strokeWeight + (o>1?strokeWeight:0)` 로 3,3 으로 만들고, 세 번째 인자가 참인 경로에서 `dash=max(1,3−3)=1 · gap=3×2=6` 으로 **1,6** 이 된다(원본 minify 코드 확인). **`shortdash` 였다면 9,3 또는 6,6 이다** — 아니다 ⭕ (→ 관찰 O1) |
| **504 대비 — 흰 casing** | ★ **8/28 값을 인용하지 않고 9/4 타일에서 픽셀로 쟀다**(`strike-map-initial-375.png`). 무대 4 원의 서쪽 테두리 주변 `y=266` 스캔: 초록 타일 `#deeecf`(x153) → **`#ffffff`(x158 — casing)** → `#4b5563`(x160·161 — 원 선). 대오 1 밴드 `y=130`: `#ffffff`(x167) → `#093389`(x169·170). **casing 이 실제로 렌더된다** ⭕. 대비(`check-contrast.mjs`): **`#093389` on `#ffffff` = 11.37** · **`#4b5563` on `#ffffff` = 7.56** — **비텍스트 3:1 을 크게 상회** ⭕ |
| **530 pill 폭** | 최대 = **`광화문역(5호선)` 90.06px**(설계 예측 93.6px · 11px/600) · 차순 `시청역(1·2호선)` 88.17px. **`--strike-label-max` = 235px**(=336×0.7) — **10개 전부 상한 안 · 전부 1줄** ⭕ |
| **525 광화문역 한 항목** | 범례 광화문역 행 **1행** ⭕ · 지도 pill **1개** ⭕ · 점 **2개**(역 배지 3개 중 광화문 2 + 시청 1) ⭕ · **두 점 사이 선·도형 0**(밴드 4 + 원 4 = 도형 8개가 전부이고 그중 역을 잇는 것 없음) ⭕ |
| **526 중점 금지** | `(a+b)/2` 류 전수 검색 결과 **3곳** — `strikeMap.ts:599`(`(b.west+b.east)/2`) · `:606` · `:608`. **셋 다 `anchorAtExtreme` 의 «라벨을 놓을 화면 자리» 계산**이고 데이터에 저장·도형으로 그림·범례 출력 **모두 0** ⭕ **정당한 출현**(그 파일 `:586` 주석이 미리 지목해 두었다). `StrikeMap.tsx` 의 2건은 부채꼴 반각·마커 앵커라 무관 |

#### (5) 문안 게이트 · 범례

| # | 결과 |
|---|------|
| **506 · 528 금지어** | ★ **«렌더되는 문자열»에서 셌다**(소스 grep 금지 · `grep -c` 금지). 방법: `<head>`·`<script>`·`<style>` 제거 → **본문(태그 제거)** 과 **속성(`aria-label`·`title`·`alt`·`placeholder`·`aria-valuetext`)** 을 갈라 세고 `<title>`·`meta[content]` 는 따로. **프리렌더 + 라이브 DOM 두 계층 모두** 돌렸다. **기대 0 인 행 전부 0** — `구역`·`세부 무대`·`서울특별시 본관`·`종로경찰서`·`동화면세점`·`남산`·`청파`·`대형버스`·`주차장`·`버스 하차`·`정확한`·`정확히`·`verified`·`calculated`·`집회 3구역`·`국회`·`여의도`·`더샵`·`번 출구`·`5번`·`6번`·`①`~`⑥`·`손피켓`·`상품권`·`5만원`·`인증샷`·`수기접수`·`지부 공지로`·`확인 중`·`확인되는 대로`·`반영하겠습니다`·`준비 중`·`업데이트 예정` ⭕. **기대가 0 이 아닌 8행**(«전부 0» 일괄 판정 금지): `대오` **14/18**(≥14) ⭕ · `지도` **7/8**(≥1) ⭕ · `추후 안내` **2** ⭕ · `※` **2** ⭕ · `광화문역` **2/3**(≥2) ⭕ · `시청역` **4/5**(≥4) ⭕ · `5호선`·`1·2호선` **1/2** → **F4** |
| **512 `※` 개수** | 프리렌더 **2** · 라이브 DOM(거리뷰 켜짐 + 시트 열림 = 최대 상태) **2** ⭕ **완화 안내 줄에 `※` 없음** ⭕ |
| **511 키 줄 자리** | `점선 = 근사 · 옅은 면 = 범위 근사` — 화면 출현 **정확히 1회** ⭕ **자리는 `figure` 안 · 지도 상자 «위»**(DOM 순서: `sr-only` → 완화 안내 → **키 줄** → 상자 → 코스콤 한 줄 → `figcaption`) ⭕ 범례 `<ul>` 안에는 없다 |
| **527 범례 13행 축자** | `strikeMap.ts` 의 `legend` 문자열 13개를 정규식으로 뽑아 §54.16-10 (2) 와 **한 글자도 다르지 않음** 확인 ⭕. **7행 `무대 4 — 대오 3 남쪽입니다` 에 「끝」 없음** ⭕(QA-507) · **13행이 「구간」이지 「끝」이 아님** ⭕ |
| **501 · 529 범례 확대** | **최악 슬롯**(360 뷰포트 · `clientWidth 345` · 루트 32px)에서 **13행 전부 넘침 0** ⭕. 방법은 **§5.9.1** 대로 — 범례 행은 **flex 아이템**이라 `width:0` 의 `scrollWidth` 판정선을 쓰지 않고, **아이템 자신의 `getBoundingClientRect().width`(슬롯 233~239px) vs 자기 `scrollWidth`** 로 판정했다. 최대 초과 **+0.14px**(정수 반올림 · 13행 중 5행) — 실질 0. `clientWidth 360` 에서도 동일하게 0 ⭕. ⚠ **최장 «어절» 은 재현되지 않았다** — 같은 폰트 프로브 span(`white-space:pre`)으로 재니 `광화문역(5호선)` **193.92px** > `시청역(1·2호선)` **187.50** > **`화장실입니다` 155.58**(설계 표는 `화장실입니다` **202px**). 설계는 **`break-keep` min-content**(괄호에서 끊긴 값), 나는 **끊기지 않은 통짜 폭**을 쟀다 — **다른 정의**다(§5.7). **문면은 안 바뀌었고**(527 축자 확인) **판정(넘침 0)은 실측으로 직접 확정**했다 |
| **502 범례 ↔ 지도 기호** | 화장실 픽토그램: 범례 `<svg width=16>` 과 지도 배지 `<svg width=13>` 의 **`width`/`height` 만 빼면 outerHTML 이 완전히 동일** ⭕ · 자식 8개(원2·사각5·삼각1)와 색(`#1785DE`·`#F2492A`)이 순서까지 일치 ⭕ (같은 `symbolSvg()` 한 곳에서 나온다) |
| **500 · 521 ★ 200% 가로 스크롤** | **`documentElement.scrollWidth − clientWidth`** 로 판정. **4조합 + 스크롤바 보정 2조합 = 6조합 전부 0** ⭕ — 360(`cw 345`) 기본 **0** / 200% **0** · 375(`cw 360`) 기본 **0** / 200% **0** · 1280(`cw 1265`) 기본 **0** / 200% **0**. 상자 실측: `cw 360`·루트 12px **336×420** → 루트 32px **296×370**, `cw 1265` 는 두 배율 모두 **420×525**(`max-w-[420px]` 상한). **원복 후 루트 12px · 인라인 `""` 확인.** ★ **§52.11 이 확보한 0 을 지도가 깨지 않았다** ⭕ |

#### (6) 거리뷰

| # | 결과 |
|---|------|
| **515 ★ 안 가져온 API** | 소스 전수 검색 결과 `fromOffsetToCoord` **1건** · `map: pano`류 **1건** — ★ **둘 다 `StrikeMap.tsx:977` 한 줄의 JSDoc 주석**이다(*«…를 둘 다 안 가져온다»*). **실행 코드 0건** ⭕ (리더 경고대로 소스 grep 만으로 판정하지 않고 «어느 줄인가»까지 확인했다) |
| **516 조건부 렌더** | ★ **런타임으로 «값을 없애» 실측했다.** ⓐ `naver.maps.Panorama`·`StreetLayer` 를 `undefined` 로 재정의한 뒤 **SPA 재마운트**(링크 클릭 왕복) → **`거리뷰` 버튼이 렌더되지 않음**(`확대`·`축소`만) ⭕ **죽은 어포던스 0**, 지도는 정상(pill 10 · 범례 13 · `main` 1) ⭕. ⓑ `Panorama` 는 살리고 **`StreetLayer` 만 `undefined`** → **토글 버튼 유지 · `aria-pressed true` · 지도 클릭 → 시트 정상 개방** ⭕ **«길만 빠지고 나머지가 남는다»** ⭕ |
| **517 초기 시점** | 토글을 켠 «직후» **`[role="dialog"]` 0개**(파노라마 안 열림) ⭕ · **지도를 눌러야 열린다** ⭕ («우리가 한 곳을 고르지 않는다» 이행) |
| **519 거리뷰 세로** | 시트 열기 «전» `main` 높이 **2671.63** → 시트 열린 상태 **2671.63** ⭕ **변화 0**. 시트 computed `position: fixed` · `z-index: 300` · `max-height: calc(100dvh − 180px)` = **460px** ⭕ |
| 시트 접근성·문면 | `role="dialog"` · **`aria-labelledby="strike-streetview-title"`**(화면 제목 `거리뷰` 를 가리킨다 — `aria-label` 아님) ⭕ · `aria-modal="false"` ⭕ · 제목 **`거리뷰`** 그것뿐 ⭕ · 촬영 **`촬영 2026.04`**(메타에 있어서 나왔다) ⭕ · `sr-only` **`거리뷰는 시각 자료입니다. 위치 안내는 지도 아래 범례를 참고해 주세요.`** ⭕(«페이지 본문» 0) · 상태 영역 **`role="status"` · `aria-live="polite"` · idle 일 때 `sr-only` 로 DOM 유지** ⭕ **`alert`·`assertive`·적색 0** ⭕ · **헤딩 차례에 「거리뷰」가 끼어들지 않음**(h1 → h2×4 만) ⭕ |
| 손잡이 | `role="separator"` · `aria-orientation="horizontal"` · `aria-label="거리뷰 높이 조절"` · `aria-valuenow` · `aria-valuetext="화면 높이의 32 퍼센트"` · `tabIndex 0` ⭕. **↑ 키로 204.8 → 230.4px**(=+4vh) ⭕ 연속 3회 230.4 → **307.2** ⭕ |
| **`localStorage` 키 분리** | 드래그/키보드 조작 후 **`koscomlabor:strike-streetview-height` = "36"** 이 생겼고 **8/28 키 `koscomlabor:roadview-height` 는 `null` 유지** ⭕. **측정 후 내가 삭제했다**(§5.8.4) |
| 파노라마 크기 감시 | `ResizeObserver` 관측 대상 = `panoMount.parentElement` 이고 그것이 **파노라마 박스와 동일 노드임을 값으로 확인**(`observedParentIsPanoBox: true`) ⭕. 시트를 키보드로 키우자 마운트 rect 가 **230 → 307** 로 따라옴 ⭕ |
| 「로드뷰」 | 라이브 DOM 본문·속성 **각 0** ⭕ (§55-4 · 게시 조건 20) · `파란` **0** ⭕ · `거리뷰` **본문 3 + 속성 4 = 7**(기대 ≥6) ⭕ |

#### (7) 접근성 · 정합성 · 회귀

| 항목 | 결과 |
|------|------|
| 대비(본문) | 지도 블록 텍스트가 **전부 `#1a1a1a` on `#ffffff` = 17.40**(완화 안내 · 키 줄 · 코스콤 한 줄 · 범례 13행 · 글리프 · `h2`) — **AAA** ⭕. 컨트롤 `#ffffff` on `#093389` = **11.37** AAA ⭕. 픽토그램 2색 `#1785DE` 3.86 / `#F2492A` 3.64 — **도형 전용 · UI 3:1 통과 · 텍스트 미사용** ⭕(§54.7 대로) |
| 랜드마크 이름 | `<section>` **4개 전부 `aria-labelledby`** → 화면 헤딩 `집결시간`·`개요`·**`세종대로 안내지도`**·`총파업 식순` ⭕ (사이트 규칙: 헤딩 있으면 `labelledby`) |
| 헤딩 위계 | `h1` → `h2`×4 · **건너뜀 0** ⭕ |
| 마운트 노드 | **`aria-hidden` 없음** ⭕ (네이버 로고·저작권 링크가 그 안에 있다) · **`tabindex` 제거됨** ⭕ · 네이버 로고 링크 접근성 이름 **`NAVER`**(`img alt`) ⭕ |
| 마커 DOM | pill·배지 래퍼 전부 **`aria-hidden="true"`** ⭕ (뜻은 범례 13행이 진다) |
| 터치 대상 | 지도 컨트롤 **확대 44×44 · 축소 44×44 · 처음 위치로 44×44 · 거리뷰 51.7×44** ⭕ · 시트 닫기 **44×44** ⭕. 44 미만 2건은 **이번 라운드 밖**: 네이버 로고 링크 45×10(서드파티·약관상 필수) · 푸터 `관리자` 29.2×44(기존) |
| `:focus-visible` | **§5.9.2 대로 전이를 넘겨 읽었다**(computed `transitionProperty`·`Duration`·`Delay` 를 찍고 `delay+duration+120ms` 대기 후 **같은 값 2회 일치** 확인). 컨트롤 = `transition-property: opacity`(→ `outline-color` 미포함) · 값 `3px solid rgb(9,51,137) offset 2px`. **값 자체는 스펙대로인데 그려지지 않는다 → F1** |
| 타입 우회 | `as any` · `as unknown` · `@ts-ignore` · `@ts-expect-error` **0건** ⭕. `eslint-disable` **1건**(`StrikeMap.tsx:837` · 근거 주석 21줄) |
| 경계면(타입 ↔ 런타임) | `naverMaps.ts` 의 `Panorama?` · `StreetLayer?` · `PanoramaStatus?` 가 **선택 필드**라 런타임 조건부와 일치 ⭕ · `getPov?`·`getLocation?`·`setSize?`·`getProjection?` 도 선택 ⭕. **`naverMaps.ts` 는 이번 라운드에 수정되지 않았다**(git) → 8/28 회귀 위험 0 |
| 빌드·린트·타입 | `npx tsc --noEmit` **exit 0** ⭕ · `npm run lint` **출력 없음** ⭕ · `npm run build` **성공**(10/10 정적 생성) ⭕ |
| **8/28 회귀** | `/rally-2026-08-28` — `main` **1** · 가로 스크롤 **기본 0 · 200% 0** · 지도 렌더 ⭕ · **`koscomlabor:roadview-height` 미오염(`null`)** ⭕ |

---

### §54-QA-4 리더가 «참값을 정하라»고 지목한 3건 — 답

| # | 답 |
|---|-----|
| **1. 완화 문구의 «지도 위나 아래 빈 곳»** | **설계 예측 106px → 실측이 더 크다.** `360×640`(가시 높이 640) 최악 지점에서 **위·아래 합 152px · 큰 쪽 76.17px** — **판정선 44px 의 1.73배** ⭕. 예측이 작았던 이유: 예측은 «헤더 68 + 캡션 46 + 상자 420» 을 **한 화면에 다 있다고** 놓았는데, **최악 스크롤 지점에서는 캡션이 화면 위로 빠져** 가용 572 중 420 만 덮인다. → **문면은 참이다.** 그리고 «참»의 근거를 기하로만 두지 않고 **`touch-action` 히트 체인**으로도 확인했다(위·아래 `auto` · 지도 위 `none`) |
| **2. 「지도에 표시된 길」이 실제로 어떻게 보이는가** | ★ **8/28 주석의 «파란 길»은 9/4 렌더에서 재현되지 않는다 — 실제 색은 «연보라»다.** 방법: 토글 OFF/ON 두 스크린샷을 픽셀 차분. **도로망 전체가 바뀌고**(차분 이진화 이미지가 도로 그래프 모양) 흰 도로 `rgb(253,253,252)` → **`rgb(205,203,239)`**, 변화 픽셀 3,246개의 **ΔRGB 중앙값 112 · 평균 108** — **육안으로 명확히 구별된다** ⭕. → **§55-6 의 «「파란 길」을 쓰지 마라» 판정이 실측으로 뒷받침된다.** 「지도에 표시된 길」이 정확한 문면이다 |
| **3. 실기기 툴바가 «빈 곳»을 얼마나 먹는가** | ⚠ **못 잰다 — «통과»로 적지 않는다.** `100svh == 100lvh == 100dvh == 100vh == innerHeight == visualViewport.height == 640` 이라 **이 환경에는 «툴바 보임/숨김» 구분 자체가 없다**(§5.8.3 판별법). **대신 판정선을 식으로 남긴다**: 가시 높이 `x` 에서 **빈 곳 합 = x − 488**(헤더 68 + 상자 420), **최악 지점 한쪽 = (x − 488) / 2**. → **44px 을 «최악 지점에서도» 보장하려면 `x ≥ 576`.** 즉 **툴바가 64px 이상 먹으면 최악 스크롤 지점에서 44px 이 깨진다**(스크롤을 옮기면 한쪽에 몰 수 있으므로 «완전히 못 쓴다»가 되는 선은 `x < 532`). **실기기 측정은 후속으로 넘긴다** |

---

### §54-QA-5 관찰 — 실패는 아니나 기록한다

| # | 내용 |
|---|------|
| **O1** | ★ **같은 `strokeStyle: "shortdot"` 이 도형 종류에 따라 두 가지로 렌더된다** — **무대 원 `stroke-dasharray: 3,3`**(점 3px · 간격 3px) · **대오 밴드 `1,6`**(점 1px · 간격 6px). 우리 코드는 `SHAPE_STYLE.strokeStyle` **한 상수**를 둘 다에 넘긴다 — 갈리는 곳은 네이버 `_getDashStyle(t,e,i)` 의 세 번째 인자 분기다(`dash=max(1,o−weight)` · `gap=o×2`). **계약(«선종 단일»)은 위반이 아니다**(둘 다 `shortdot` 의 산출물이고 `shortdash` 는 9,3 또는 6,6 이다). 다만 **화면에서 두 도형의 점선 밀도가 눈에 띄게 다르다** — 디자이너가 «같은 점선으로 보여야 한다»고 판정하면 밴드 쪽 `strokeLineCap` 을 손대야 한다 |
| **O2** | z18·z19 에서 **대오 pill 이 이웃 «무대 원» 의 사각 bbox 를 0.6~1.3% 스친다.** 원을 사각으로 근사한 값이라 **실제 원호와 닿는지는 미확정**이다. 자기 도형(밴드)과는 전 배율 0 |
| **O3** | `naverMaps.ts:175` 의 `map: NaverMap | NaverPanorama` 가 **`Marker({ map: panorama })` 를 타입상 여전히 허용**한다(8/28 이 쓴다). 9/4 사용은 0 이지만 **타입이 막지는 않는다** — `FOLLOWUPS #12` 의 남은 절반과 같은 자리다 |
| **O4** | 401 상태에서 네이버가 마운트 노드에 **`background: url(auth_fail.png)`** 를 박는다. 우리 대체면(`bg-surface`)이 그 위에 덮여 보이지 않지만, **대체면이 없었다면 조합원이 네이버의 영문 실패 이미지를 봤을 것**이다 — `StrikeMapFallback` 의 존재 이유가 하나 더 확인됐다 |

---

### §54-QA-6 미검증 — **3건 (통과로 적지 않는다)**

| # | 항목 | 사유 |
|---|------|------|
| **U1** | **실기기 가용 높이 · 툴바 잠식분** | `100svh == 100lvh == 640` — **이 환경에는 툴바 구분이 없다**(§5.8.3). QA-510 의 **76.17px 은 «선언 판정선 360×640» 기준값**이고, **실기기에서 툴바가 먹는 높이는 못 쟀다.** 판정선 식은 §54-QA-4 #3 에 남겼다 |
| **U2** | **실기기 터치 스크롤·핀치 줌** | 이 컨텍스트는 `maxTouchPoints 0` · `(pointer: coarse) false` 라 **터치 에뮬레이션이 없다**(`hasTouch` 는 컨텍스트 생성 시점 옵션이라 바꿀 수 없었다). 드래그·클릭은 **합성 pointer/mouse 이벤트**로, 페이지 스크롤 가능 여부는 **`touch-action` 히트 체인**으로 확인했다 — **«실제 손가락»은 못 밟았다.** `pinchZoom` 은 **옵션이 `true` 인 것과 더블탭 줌이 열린 것만 확인**했고 두 손가락 제스처 자체는 미측정 |
| **U3** | **`today`·`past` 의 «서버 렌더» 실물** | 오늘(2026-08-29)은 `upcoming` 이라 서버가 `today` 마크업을 내지 않는다. **361.52 는 소스 분기를 그대로 옮긴 DOM 재구성값**이다(주입 클래스의 computed 검산은 했다). `past` 는 이번 라운드 범위 밖 |

---

### §54-QA-7 ★ 이번 회차에 내가 걸릴 뻔한 «세는 방법» — 기록

**① «측정 전후 해시»를 빌드 동결 기준으로 쓸 뻔했다.** 측정 끝에 서버 응답 sha256 `bb2bc9d3…` 과 디스크 프리렌더 `94306904…` 가 **3회 모두 다르게** 나와 회차를 버릴 뻔했다. 갈라 보니 **전체 바이트가 44,669 로 동일**했고 **`<script>` 제거 후 해시가 완전 일치**(`0dd3833d…` · 19,611바이트 · diff **0줄**)했다 — 차이는 **RSC 플라이트 페이로드 안 ISR 캐시 수치**뿐이다. §5.7 이 이미 이름 지은 **오탐 모드 ①** 이고, 이 페이지는 `revalidate 60` 이라 **더 자주 걸린다.** → **Next 산출물을 셀 때는 `<script>` 를 먼저 지운다**를 이 페이지에서도 기본값으로 쓴다.

**② «기대 개수 표»가 어느 계층을 세는지 안 적혀 있었다(F4).** `5호선` **1** 이라고 적힌 표를 **라이브 DOM** 에 대고 세면 **2** 다 — 초과분은 지도 pill 이다. 프리렌더로 세면 1 이다. **둘 다 «렌더되는 문자열»인데 계층이 다르다.** §5.9 가 «주석이 아니라 렌더를 세라»까지는 말했지만 **«프리렌더인가 라이브 DOM 인가»는 아직 이름이 없다.** → 이 프로젝트의 «세는 방법이 질문에 안 맞는» 목록에 **«같은 렌더의 두 계층»** 으로 이어 붙인다.

**③ 처방이 원인을 못 짚는 것을 «처방이 있으니 통과»로 넘길 뻔했다(F3).** §54.16-11 은 «끊겨 보이면 점 12 → 10px» 라는 **대체 처방을 미리 적어 두었다.** 그대로면 «실패지만 처방이 있으니 개발자에게 넘기면 끝»이다. **그런데 세로 가림의 지배 기여자가 점이 아니라 화장실 배지였다** — 처방을 적용해도 32.6% → 30.6% 다. **«남는 위험»을 예측한 것과 «그 크기와 원인을 맞게 예측한 것»은 다르다.**

**④ `광화문역(5호선)` 을 «9자 덩어리»로 볼 뻔한 자리에서, 반대로 «설계 표의 202px 이 재현되지 않는다»에 걸렸다.** 설계는 `break-keep` **min-content**(괄호에서 끊긴 값), 나는 **통짜 폭**을 쟀다 — **다른 정의를 같은 이름(«최장 어절»)으로** 부르는 §5.7 그 계보다. **두 값 다 정확했고 판정도 같았다**(넘침 0). 그래서 **판정을 어절 값이 아니라 «flex 아이템 슬롯 폭 vs 자기 `scrollWidth`»(§5.9.1)로 직접 냈다** — 정의 논쟁이 판정에 닿지 않게.

---

## §54 QA 회귀 (2026-08-29 · F1~F3 수정분)

> 대상: `§54.17` · `M-23` 처방. **F1**(포커스 링 · 안쪽 링 + 색 반전) · **F2**(마커 z = 범례 행 순서) · **F3**(죽은 처방 주석 제거).
> **판정: 세 처방 전부 해소 ⭕ · 회귀 0건 · 신규 발견 1건**(8/28 에 같은 결함 — **이번 라운드 범위 밖**).

### §54-R-0 요약

| 항목 | 판정 |
|---|---|
| **F1 포커스 링** | ⭕ **해소.** 4개 버튼 + **거리뷰 ON 상태**까지 **5개 상태 전부 픽셀로 확인**. 대비 **11.37**(5/5) |
| **F2 z 순서** | ⭕ **해소.** z 가 `MARKER_Z_BASE(100) + 범례 행 index` 로 실측되고, 렌더 방향(«역 점이 위»)도 픽셀로 확인 |
| **F3 죽은 처방** | ⭕ **해소.** 주석이 «되살리지 마라 + 왜 못 고치는가(기각 4건)»로 바뀌었다 |
| **회귀** | ⭕ **0건.** 지도 상자 **141,456 픽셀 차분 0** · 첫 화면·200%·선종·casing·거리뷰·8/28 전부 불변 |
| **신규 발견 N1** | ⚠ **`/rally-2026-08-28` 에 F1 과 «같은» 결함이 있다**(픽셀 변화 0 = 포커스 표시 없음). **이번 수정이 만든 것이 아니다 — 잠복 결함이고 범위 밖.** `FOLLOWUPS` 신규 |
| **미검증** | **U1·U2·U3 그대로 미검증.** 환경이 안 바뀌었으므로 «못 잼»이 유지된다 |

---

### §54-R-1 측정 환경표 (§5.5 · §5.6 · §5.7 · §5.8.4)

| 항목 | 값 |
|------|-----|
| 서버 | 프로덕션 standalone · 기동 전 포트 3000 listener **0** → 기동 후 **PID 31638** · 로그 `EADDRINUSE` **0건** |
| 확인 ② | **서버 응답 == 디스크 프리렌더**(`<script>` 제거 후) — 같은 시각 **3회 일치** `7c8ad8c7…` |
| **빌드 동결** | `BUILD_ID` **`CKbJZHmhqMQ926757u6qG`** · `build-manifest.json` mtime 불변 · **빌드보다 새로운 소스 0건**. 측정 중 재빌드 **0회**(§54 회차와 달리 이번엔 빌드를 안 바꿨다) |
| ★ **소스 문면 불변 증명** | 직전 회차 프리렌더와 **`<script>` 제거 후 diff 4줄** — 그 4줄이 **CSS 청크 파일명 한 줄뿐**(`0hbco4sbr9-i_.css` → `2zwvca9k8n23g.css`). **바이트 수 19,611 동일.** → **본문·속성 문자열이 한 글자도 안 바뀌었다**(F1~F3 은 클래스·주석·z 값 변경이고 문면 변경이 아니다) |
| 뷰포트 | 375×640(`cw 360`) · 360×640(`cw 345`) · 1280×800(`cw 1265`) · **스크롤바 15** |
| 루트 폰트 | 기본 12px · 확대 32px(인라인) |
| `visibilityState` | `visible` |
| **정리 확인** | 프로브 **0**(`qa-crop-probe`·`qa-today-probe` 둘 다 `null`) · `html` 인라인 `""` · 루트 **12px** · 상자 인라인은 컴포넌트 자신의 `--strike-label-max: 235px` 만 · 컨트롤 스택 인라인 **`null`** · **내가 쓴 `localStorage` 키 0**(`koscomlabor:strike-streetview-height` = `null`) · **8/28 키 `koscomlabor:roadview-height` = `null`** · 문서 가로 스크롤 **0** |
| ⚠ 잔여 | 저장소 최상단에 **`f1-ring.png`** 가 있다 — **내 산출물이 아니다**(개발자 검증분). 내 증거는 `_workspace/screenshots/2026-08-29-strike-map/regression/` 13장 |

---

### §54-R-2 ★★ QA-531 — F1 픽셀 확인 (리더가 «computed 로 끝내지 마라»고 지목한 자리)

**방법**: 각 버튼에 대해 **컨트롤 스택 요소를 «포커스 없음»과 «포커스» 두 번 스크린샷**해 픽셀 차분.
★ **스택 자체를 찍는 것이 요점**이다 — 링이 이제 **안쪽**(`-3px`)이라 `overflow-hidden` 안에 들어오므로 스택 rect 안에서 보여야 한다.
computed 는 §5.9.2 대로 **전이(`transitionProperty: opacity` · `duration 0.15s` · `delay 0s`)를 넘겨 270ms 대기 후 2회 읽어 일치**를 확인했다.

| 버튼 | computed `outline` | **변화 픽셀** | diff bbox | **링 최빈색** | **면 최빈색** | **대비** |
|---|---|---|---|---|---|---|
| **확대** | `3px solid rgb(255,255,255)` · **`-3px`** | **442** / 3,916 | `(0,0,44,44)` | `#ffffff` (410px) | `#093389` (1,393px) | **11.37** |
| **축소** | 〃 | **439** | `(0,44,44,88)` | `#ffffff` (410px) | `#093389` (1,409px) | **11.37** |
| **처음 위치로** | 〃 | **440** | `(0,88,44,132)` | `#ffffff` (410px) | `#093389` (1,367px) | **11.37** |
| **거리뷰 (OFF)** | 〃 | **442** / 2,340 | `(0,0,52,44)` | `#ffffff` (376px) | `#093389` (1,474px) | **11.37** |
| ★ **거리뷰 (ON)** | `3px solid rgb(9,51,137)` · **`-3px`** | **448** | `(0,0,52,44)` | **`#093389`** (376px) | **`#ffffff`** (1,474px) | **11.37** |

- ★ **다섯 상태 전부 «화면에 그려진다».** 종전(F1)에는 **변화 픽셀 0**(= 아무것도 안 그려짐)이었다.
- ★ **ON 상태에서 링과 면이 정확히 반전**된다(흰 면 · 남색 링) — 설계대로다. 육안으로도 확인했다(`regression/r-right-on-focus.png`).
- **검산**: 44×44 버튼의 3px 안쪽 링 이론 면적 = `44² − 38² = 492px`. 실측 흰 픽셀 **410**(둥근 모서리 때문에 줄어든 값) — **CSS 상 가능한 값**이다(§5.5 종횡비 검산과 같은 계열).
- ⚠ **`MapControlStack` 의 `overflow-hidden` 은 그대로다**(computed `hidden` 확인). **구조를 안 바꾸고 링만 안쪽으로 옮겨 해결했다** — 디자이너 판정(«바깥 링은 잘리기 «전»에도 근거가 없었다»)이 그대로 이행됐다.

#### ★ 컴파일 CSS 확인 — 리더가 실수할 뻔한 자리를 값으로 닫는다
`.next/static/css/` 는 **존재하지 않는다.** 실제 CSS 는 **`.next/static/chunks/2zwvca9k8n23g.css`(36,856바이트) 한 파일**이다.
`grep -o … | wc -l`(**`grep -c` 금지 — 미니파이라 한 줄이다**) 결과:

| 문자열 | 개수 |
|---|---|
| `outline-offset:-3px` | **1** |
| `outline-color:var(--color-white)` | **3** |
| `outline-color:var(--color-primary)` | **5** |

→ **셋 다 실재한다.** ★ 이 프로젝트 계보(`e3b0c44…` 오독 · `grep -c` 오독)에 **«0 이 나오면 대상이 있었는지부터 의심하라»** 가 하나 더 붙는다.

---

### §54-R-3 F2 — z 순서가 «규칙»으로 고정됐는가

**실측 z-index**(네이버 마커 래퍼의 계산값 · 초기 뷰):

| 항목 | 배열 index | **z** | 규칙값 `100 + index` |
|---|---|---|---|
| 간이화장실(북) | 8 | **108** | 108 ⭕ |
| 간이화장실(남) | 9 | **109** | 109 ⭕ |
| **시청역 화장실** | 10 | **110** | 110 ⭕ |
| 광화문역(두 점) | 11 | **111 · 111** | 111 ⭕ ★ **두 점이 같은 z** — «한 항목» 계약과 일치 |
| **시청역** | 12 | **112** | 112 ⭕ |
| pill 10개 | 0~12 | **1000~1012** | `LABEL_Z_BASE + index` ⭕ |

→ **`시청역 화장실(110) < 시청역(112)` → 역 점이 위.** 규칙 = 범례 행 순서 ⭕

**렌더 방향 확인(픽셀)**: 같은 크롭 영역을 **수정 전 · 후로 비교 → 변화 픽셀 0 / 2,440**.
★ **이 «0» 이 오히려 강한 증거다** — z 를 명시했는데 렌더가 안 바뀌었다는 것은 **명시한 값이 종전 동작과 일치한다**는 뜻이다.
종전에는 «생성 순서» 때문에 우연히 그랬고 **지금은 규칙이 그것을 보장한다.**
육안(`regression/r-f2-dotband.png`): 시청역 `◉` 가 화장실 배지의 `dashed` 테두리 **위에** 그려지고, 화장실 픽토그램은 **면이 남아 판독된다** — 주석 «2. 확신도 손실이 작다»의 실측 확인.

---

### §54-R-4 F3 — 죽은 처방이 제거됐는가

- `StrikeMap.tsx:281` — **«종전의 «점 지름을 12 → 10px 로 줄여라» 처방은 죽었다. 되살리지 마라»** + **적용해도 32.6% → 30.6%(2%p)** 라는 수치 + **동쪽 지배 기여자가 화장실 배지(24px)** 라는 원인이 들어갔다 ⭕
- **대체 처방 4건을 «왜 못 쓰는가»와 함께 기각**했다(화장실 배지 축소 → 픽토그램 판독 불가 / 좌표 이동 → M-19 / 밴드 폭 확대 → M-9 B안 기각 / `fillOpacity` 인상 → 확신도 위계 붕괴) ⭕
- **소스에 «10px» 을 처방으로 쓰는 자리 0** — 남은 `10px` 언급은 **«죽었다»고 못박는 그 문장 하나**다 ⭕
- ⚠ **QA 로서 이 판정에 동의한다.** 남는 위험은 **확대에 위임**되고, 그 완화는 «확대 버튼이 보인다»에 의존하는데 **F1 수정으로 그 버튼의 키보드 접근성까지 복구**됐다 — **두 처방이 서로를 떠받친다.**

---

### §54-R-5 회귀 — 수정이 깬 것이 «없다»

| 항목 | 회귀 전 | **회귀 후** | 판정 |
|---|---|---|---|
| **QA-493 첫 화면** | `upcoming` 334.52 / `today` 361.52 | **334.52 / 361.52** · 원복 334.52 | ⭕ 불변 |
| **세로 예산** | 910.26 | **910.27**(반올림 차 0.01) | ⭕ 불변 |
| **QA-500/521 200% 가로** | 6조합 0 | **6조합 전부 0** — 360(`cw345`) 0/0 · 375(`cw360`) 0/0 · 1280(`cw1265`) 0/0 | ⭕ |
| **QA-501/529 범례 200%** | 넘침 0 | **넘침 0**(`cw 345`·`cw 360` 둘 다) | ⭕ |
| **QA-513 상자** | 336×420 / 420×525 | **336×420 / 420×525**(200% 에서도 420×525) | ⭕ |
| **QA-503/509 선종** | casing 8 · 원 `3,3` 4 · 밴드 `1,6` 4 | **z16·z19 둘 다 동일** · 유채색 solid **0** · z19 `확대` **disabled** | ⭕ |
| **QA-504 casing** | 초록 타일 `#deeecf` → 흰 `#ffffff` → 회색 `#4b5563` | **동일 픽셀열 재현**(y=266 · y=130) · 대비 11.37 / 7.56 | ⭕ |
| ★ **지도 전체 픽셀** | — | **수정 전 지도 스크린샷과 차분 0 / 141,456** | ⭕ **렌더 무변화** |
| **pill · 배지 개수** | 10 / 6 | **10 / 6** | ⭕ |
| **QA-519 거리뷰 세로** | `main` 2671.63 | 시트 열림 **2671.63** · 닫은 뒤 **2671.63** | ⭕ |
| **거리뷰 동작** | 시트 개방 · `aria-labelledby` · `aria-modal=false` · z300 | **전부 동일** · 촬영 `2026.04` · `sr-only` 문면 동일 | ⭕ |
| **문안 게이트** | `※`2 · 키줄1 · 로드뷰0 · 파란0 · 대오18 · 지도7 · 추후안내2 · 광화문역3 · 시청역5 · 5호선2 · 1·2호선2 · 나머지 0 | **전부 동일** · 시트 열린 최대 상태에서 `거리뷰` **본문3+속성4=7**(≥6) | ⭕ |
| **8/28 회귀** | main 1 · 가로 0/0 · 지도 렌더 | **동일** · `koscomlabor:roadview-height` **`null`**(미오염) | ⭕ |
| **`tsc` / `lint`** | 0 / 0 | **exit 0 / 출력 없음** | ⭕ |
| **build** | 성공 | 산출물이 **현 소스에서 생성됨**(빌드보다 새로운 소스 0건) · 서버가 그것을 서빙(sha256 3회 일치) | ⭕ |

---

### §54-R-6 ⚠ 신규 발견 N1 — **`/rally-2026-08-28` 에 F1 과 «같은» 결함이 있다** (범위 밖 · `FOLLOWUPS` 후보)

**이번 수정이 만든 것이 아니다.** 9/4 의 F1 처방이 8/28 에는 적용되지 않았고, 8/28 은 **원래부터** 같은 조건이다.

| | 값 |
|---|---|
| 위치 | `src/components/rally/RallyMap.tsx:1364`(컨트롤 스택 `… overflow-hidden`) × **`:1342`**(`MAP_CTRL_BASE` 의 `focus-visible:outline-primary focus-visible:outline-offset-2`) |
| 기하 | 스택 rect `(21, 298.1, 44, 88)` ↔ `확대` 버튼 rect `(21, 298.1, 44, 44)` — **9/4 F1 과 동일하게 좌·우·상변이 정확히 겹친다** |
| computed | `outline: 3px solid rgb(9,51,137)` · **`outline-offset: 2px`** · 버튼 면 `rgb(9,51,137)` |
| ★ **픽셀** | 포커스 «전» ↔ «후» **변화 픽셀 0 / 3,916 · bbox `None`** → **포커스 표시가 화면에 전혀 없다** |
| ★ **두 겹으로 실패** | ① 바깥 링이 `overflow-hidden` 에 잘린다 ② **잘리지 않았더라도 링 색(`#093389`)과 버튼 면(`#093389`)이 같아 대비 1:1** 이다 |
| 판정 | **WCAG 2.4.7 실패.** 8/28 페이지도 `keyboardShortcuts` 를 안 열었다면 이 버튼이 키보드 확대 경로다 |
| 처방 | **9/4 와 같은 한 줄**: `:1342` 를 `focus-visible:-outline-offset-3` + `focus-visible:outline-white`(면이 남색일 때) 로. ⚠ 8/28 은 `MAP_CTRL_ON_CLASS` 계열이 다르므로 **개별 확인 필요** |
| ⚠ 범위 | **이번 라운드 밖이다** — `RallyMap.tsx` 는 «고치지 마라» 규율이 두껍고 리더 승인 사항이다. **QA 는 보고만 한다.** |

---

### §54-R-7 미검증 — **U1·U2·U3 그대로**

**환경이 바뀌지 않았으므로 «못 잼»이 그대로다. 통과로 바꾸지 않았다.**

| # | 항목 | 상태 |
|---|------|------|
| **U1** | 실기기 가용 높이·툴바 잠식분 | **미검증 유지** — `100svh == 100lvh == 640`(이번 회차에서도 같다) |
| **U2** | 실제 터치·핀치 제스처 | **미검증 유지** — `maxTouchPoints 0`. 이번 회차의 클릭·드래그도 **합성 pointer/mouse** 다 |
| **U3** | `today`·`past` 서버 렌더 실물 | **미검증 유지** — 오늘은 `upcoming`. `361.52` 는 **DOM 재구성값**이다(주입 클래스 computed 검산은 재실행했다) |

---

### §54-R-8 ★ 이번 회귀에서 «세는 방법»에 대해 확인한 것

**① 리더의 경고가 정확했다 — F1 은 «computed 가 처음부터 정상»인 결함이었다.**
그래서 이번에도 **computed 를 근거로 쓰지 않고** 스택 요소를 두 번 찍어 **픽셀 차분**으로 판정했다.
computed(`-3px`)만 봤으면 «고쳐졌다»는 말은 할 수 있지만 **«보인다»는 말은 못 한다** — 두 문장이 다르다.

**② «변화 0» 이 두 번 나왔는데 뜻이 정반대다.**
- F2 크롭 차분 **0** = **좋다**(«명시한 z 가 종전 렌더와 일치한다»)
- 8/28 포커스 차분 **0** = **나쁘다**(«포커스 표시가 없다»)
★ **같은 숫자가 반대 뜻**이므로 **«무엇과 무엇을 비교했나»를 값 옆에 반드시 적어야 한다.** 이 프로젝트 «값 + 방법 + 기준이 한 벌»(§5.7)의 가장 날것의 사례다.

**③ 컴파일 CSS 를 «없는 디렉터리»에서 찾을 뻔한 것**(리더 보고)은 이 프로젝트의 **`e3b0c44…`(빈 문자열 해시) 오독과 같은 계보**다 — 둘 다 **«0/빈 값이 나오면 대상이 존재했는지부터 의심하라»** 로 닫힌다. 실제 CSS 는 `.next/static/chunks/*.css` 한 파일이고 세 문자열이 전부 실재했다.

---

## §54.18 QA (2026-08-29 · 화장실·역 거리뷰 팝업)

> 대상: `src/lib/strikeMap.ts`(`roadview`·`MAP_AFFORDANCE_NOTE`·`APPROX_NOTE`·`featureShortName`·`featureRoadviewPoint`·`ROADVIEW_IDS` 가드) · `StrikeMap.tsx`(히트·팝업·와드) · `StrikeRoadviewSheet.tsx`(`approximate`·열릴 때 포커스)
> 체크리스트 **539~553**(§54.18-6) + 리더 지목 A~G.

### §54.18-QA-0 판정 — **통과 26 | 실패 4 | 회수 1 | 관찰 3 | 미측정 3**

> ⚠⚠ **2026-08-29 정정 (M-41 수령)** — 아래 **F-A 는 회수한다.** 리더가 접근성 이름 기대치를 **«전문»으로 되돌렸고**(메시지 엇갈림 · 개발자가 반영 중이던 쪽이 최종), **기대 개수표(M-40)도 함께 회수**됐다.
> **산출물은 정정된 기대치와 일치한다** — 재측정으로 확인했다(§54.18-QA-3 (2') · (5')). **판정 요약의 수치는 정정 «후» 값이다.**

**게시 차단 0건.** 게시 전 수정을 권고하는 것은 **F-C**(마커 생성에 방어가 없어 페이지가 죽는다) 하나다.
★ **F-B 는 내 §54 회귀 리포트의 QA-505 판정을 정정하는 항목이다** — 그때 나는 **프록시**로 판정했다.
...
| 항목 | 값 |
|------|-----|
| 서버 | 프로덕션 standalone · 기동 전 listener **0** → **PID 72583** · `EADDRINUSE` **0** |
| 확인 ② | 서버 응답 == 디스크 프리렌더(**`<script>` 제거 후**) — 같은 시각 **3회 일치** |
| 확인 ③ | 정적 자산 **11개 전수 200** |
| 빌드 | `BUILD_ID` **`OkWdbnlHlbVJwOsU8GHp2`** · **재빌드 0회**(측정 중 동결) · `tsc` **exit 0** · `lint` **출력 없음** · `build` **성공** |
| ★ `ROADVIEW_IDS` 가드 | **빌드 성공 자체가 가드 통과의 증거다** — 모듈 로드 시 `throw` 하므로 프리렌더에서 반드시 평가된다(M-39) |
| 뷰포트 | **375×640**(`cw 360`) · 1280×800(`cw 1265`) · **스크롤바 15** |
| **루트 폰트** | **12px**(개발자 고지 확인 · `rem` 파생값은 ×0.75). **히트 24px·팝업 50% 는 px·% 라 무관** |
| `visibilityState` / rAF | `visible` / 발화 ⭕ |
| 터치 | `maxTouchPoints 0` · `(pointer: coarse) false` → **합성 이벤트로 측정**(§54.18-QA-6 U2) |
| **정리 확인** | 프로브 **0** · `html` 인라인 **`null`** · 루트 **12px** · 상자 인라인은 컴포넌트 자신의 `--strike-label-max: 235px` 만 · **`localStorage` 에 내가 쓴 키 0**(`strike-streetview-height` `null` · 8/28 `roadview-height` `null` · `font-scale` `null`) · `naver.maps.Marker/Panorama/StreetLayer` **전부 `function` 으로 원복** · 문서 가로 스크롤 **0** · `main` **1** |

#### ★ 계수 방법 — 내가 «0» 에 걸릴 뻔한 자리
컴파일 CSS 에서 `focus-visible:outline-3` 을 **그대로** grep 하니 **0** 이 나왔다. 실제 CSS 는 **셀렉터가 이스케이프**돼 `.focus-visible\:outline-3:focus-visible` 이다.
`\\:` 로 다시 세니 **3**. 함께 확인: `focus-visible\:outline-offset-2` **3** · `focus-visible\:-outline-offset-3` **1** · `focus-visible\:outline-primary` **3** · `max-h-\[50\%\]` **1** · `sr-only` **1** · `min-h-touch` **1**.
★ 이 프로젝트 «0 이 나오면 대상이 있었는지부터 의심하라» 목록에 **«CSS 셀렉터 이스케이프»** 를 더한다(`.next/static/css/` 없음 → `chunks/*.css` 에 이은 **두 번째 층**).

---

### §54.18-QA-2 실패 항목 (심각도순 · 5건)

| # | 분류 | 위치 | 실측 / 기대 | 처분 |
|---|------|------|------------|------|
| ~~**F-A**~~ | ~~판정 ↔ 산출물 불일치~~ **→ 회수 (2026-08-29 · M-41)** | 실측은 그대로 유효하다 — AX 트리 계산 이름 5개가 **전부 «범례 전문»**이고 `aria-label` **0건**이다. **틀린 것은 내 기대치였다**: 나는 **M-37**(«짧은 이름»으로 잠근다)과 **M-40** 기대표를 기준으로 삼았는데, **리더가 둘 다 회수**하고 **«전문»을 최종으로 확정**했다(M-41). ★ **§54.18-6 QA-543 이 처음부터 «전문»을 요구했고 산출물은 그것을 통과한다** — 내가 «문서 3건 모순»으로 올린 것은 **M-37·M-40 이 이미 죽어 있었기 때문**이다. **죽은 절을 인용해 실패를 만들었다.** | **처분 없음.** 아래 §54.18-QA-3 (2')·(5') 가 정정된 기대치로 재측정한 결과다. ⚠ 파생값도 함께 회수한다 — **§54.18-QA-3 (5) 의 «전문 2 → F-A» 표기**와 **§54.18-QA-2 F-E ③**(«범례 문면 라이브 2회» 정정)은 **둘 다 무효**다. §54.18-1 (2)의 원래 예고가 참이다 |
| **F-B** | **지도 재적합 — QA-505 «회귀»이자 내 이전 판정의 정정** | `StrikeMap.tsx:981~996`(RO 콜백: `syncLabelWidth()` + `fit()`) | ★ **«뷰포트는 그대로인데 상자만 좁아지는» 경우 지도가 재적합되지 않는다.** 글자 크기 200%(루트 32px)로 상자 **336×420 → 296×370**: `--strike-label-max` 는 235 → **207** 로 갱신되고 마운트·저작권·축척 바도 새 크기를 따르는데 **마커 5개가 «한 픽셀도» 안 움직인다**(2.5초·5초 후 재측정 동일) → 오른쪽 여백 **50.8 → 10.8px** · 아래 **86 → 36px**. 상자를 **200×250** 으로 줄이면 **히트 4개가 상자 밖**(`toilet-south`·`cityhall-toilet`·`gwanghwamun`·`cityhall`). ⭕ **실제 뷰포트 리사이즈(375↔1280)에서는 정상**(마커가 (+42,+52) 재중심화 · 상자 밖 0) | **진단까지 냈다**: `window.dispatchEvent(new Event('resize'))` 를 쏘면 **즉시 재중심화**된다(오른쪽 여백 10.8 → 30.8) → **네이버는 `window.resize` 만 듣고 «컨테이너만» 바뀐 것은 모른다.** `map.fitBounds` 는 네이버가 옛 크기를 들고 있는 동안 계산된다. → **처방**: RO 콜백에서 `fit()` **앞에** `safely(() => map.refresh())` 또는 `safely(() => maps.Event.trigger(map, "resize"))`. **둘 다 공식 API 이고 로드된 `maps.js` 에 실재**한다(`refresh:function` 5건 · `trigger:function` 2건) — **문서 없는 API 를 늘리지 않는다.** ★★ **내 §54 회귀 리포트의 QA-505 «재적합이 실제로 일어난다»는 틀렸다** — `--strike-label-max` 변화는 «RO 가 울렸다»의 증거일 뿐 **«지도가 다시 맞춰졌다»의 증거가 아니다**(§5.8.4 ③ «프록시로 판정하지 마라»). **참값은 «마커가 새 크기에 맞춰 움직이는가»** 다 |
| **F-C** | **§54.10 방어 누락 — 페이지가 죽는다** | `StrikeMap.tsx:1305` `new maps.Marker({...})`(지도 위 «지금 보는 위치» 표식 effect) — **`try/catch` 없음.** 정리 쪽(`:1324`)에만 `safely()` 가 있다 | **QA-546 을 «발동시켜» 측정했다**: `naver.maps.Marker` 를 `undefined` 로 만든 뒤 팝업 → 「거리뷰 보기」 → **`TypeError: r.Marker is not a constructor`**(콘솔 원문) → **`main` 요소 0개 · 시트 안 열림 · 파노라마 0**. **기대**(§54.18-4 (1) · `union-webapp-dev §7`): *«없어져도 거리뷰 자체는 그대로 열린다»* → **안 열린다. 페이지가 통째로 죽는다** | ⚠ **와드 쪽은 방어가 있다**(`typeof maps.Marker !== "function"` 가드 + `try/catch`) — 그런데 **같은 상황에서 이 effect 가 «먼저» 죽어 그 방어가 무의미해진다.** ⚠ 현실성은 낮다(`Marker` 는 핵심 API 라 소멸하지 않는다). **그러나 기전이 §54.10 과 같다** — 인증이 깨진 뒤 «있지만 내부가 null» 상태에서 **생성이 throw** 하는 것을 이 프로젝트가 이미 재현했다(`new maps.Map()`). → **처방**: `:1305` 를 `try/catch` 로 감싸고 실패 시 표식만 생략(와드와 같은 «조용한 비활성»). 그러면 부채꼴만 사라지고 **거리뷰·지도·페이지는 남는다** |
| **F-D** | **히트 교차(QA-541)** | `strikeMap.ts:519`(`cityhall` `placement:"right"` · `labelGap:20`) ↔ `:481`(`cityhall-toilet`) | **z16 교차 0** ⭕(최소 여유 **1.0px** · `cityhall-toilet` ↔ `cityhall` pill) → **z17 에서 `7 × 1px` 교차** ❌. 원인: **시청역의 히트는 pill 이고 pill 은 «점에서 화면 20px» 고정 오프셋**인데 화장실 배지는 «지리 고정»이라 **배율이 바뀌면 둘의 관계가 바뀐다.** **기대**(QA-541): «z16·z17 두 배율에서 교차 0» | 교차 면적 7px²(배지 24×24 의 **1.2%**)이고 그 자리는 z17 에서 **상자 밖**이라 초기 화면에는 안 보인다(드래그로 들어온다). z 순서상 위인 `cityhall` pill(z 1012)이 먹으므로 «누를 수 없는 지물»은 생기지 않는다. → **심각도 경미.** `labelGap` 을 20 → 24 로 올리면 z17 에서도 벌어진다(디자이너 판정) |
| **F-E** | **설계 예측 3건이 실측과 다르다**(문서 정정) | §54.18-1 (1) · §54.18-5 · §54.18-1 (2) | ① **«서쪽 열 잔여 여유 2.7px»** → 실측 **히트↔히트 1.0px**(z16 최소) · **히트↔장식 pill 0.0px**(`toilet-north` 상단 96 ↔ 메인무대 pill 하단 96 — **정확히 맞닿는다**). ② **«세로 약 950px»** → 실측 **910.27px**(**증분 0**) — 어포던스 문장이 **기존 2줄 안에 흡수**됐다(`<p>` 78자 · `scrollHeight === clientHeight === 34` · 줄 상자 2개 · 클리핑 0). ~~③ «범례 문면 라이브 2회»~~ **→ 회수(M-41)**: 이 항목은 **정정이 필요 없다.** §54.18-1 (2)의 예고(**라이브 2회**)가 **참**이고 실측이 그대로 일치한다((5') 표) — 내가 «죽었다»고 본 근거(M-40)가 오히려 회수됐다 | **①②만 유효**하다. 두 값으로 표를 갱신하라. **첫 화면 판정선 영향은 여전히 0** |
...
| # | 결과 |
|---|------|
| **540 히트 규칙** | 히트 **정확히 5개**(`toilet-north`·`toilet-south`·`cityhall-toilet`·`gwanghwamun`·`cityhall`) ⭕ · **대오 4 pill 클릭 → 팝업 안 열림** ⭕(클릭 대상 `DIV/none`) · **점(dot) 3개 전부 `pointer-events: none`** ⭕ — **광화문역 두 점의 중심에서 `elementFromPoint` 가 «타일(IMG)»을 집는다**(점이 안 눌린다) ⭕ |
| **542 터치 24** | 히트 5개 **24×24 / 24×24 / 24×24 / 90.1×24 / 88.2×24** — **전부 24 이상** ⭕ (pill 세로 **정확히 24**). 200%(루트 32px)에서도 **24 유지**(캔버스 안 고정 px) ⭕ |
| ★ **44 예외 미확산** | 페이지 전체 포커스 대상 14개 실측 — **24 예외는 지도 마커 5개뿐**. 지도 컨트롤 **44×44 · 51.7×44** · 팝업 버튼 **44** · 복귀 링크 **164×44 · 202.2×44** · 헤더 로고 **207×44** · QR 카드 **336×88.5** ⭕. 44 미만 비마커 **2건은 전부 기존·범위 밖**: 네이버 로고 링크 45×10(약관상 필수) · 푸터 `관리자` 29.2×44 |
| ★ **«44 를 못 만든다»가 참인가** | **계산으로 확인했다.** 44×44 를 중심 기준으로 주면 — `cityhall-toilet`(중심 184,311) → `162..206 × 289..333`, `cityhall` pill 세로 44 → `197..285.2 × 300..344` → **교차 9 × 33 = 297px²**. 서쪽도 같다: `toilet-north`(143,108) ↔ 메인무대 pill → **교차 12 × 20 = 240px²**. → **«인접 히트가 서로를 삼킨다»가 참이다** ⭕ (실측 여유는 F-E ①대로 2.7px 이 아니라 1.0/0.0px 이므로 **근거가 더 강해졌다**) |

#### (2) ★ 접근성 (C)

| # | 결과 |
|---|------|
| **543** ⭕ | **`aria-label` 0건** ⭕(§8 — 이름은 `sr-only` **내부 텍스트**가 진다) · **간이화장실 2개의 이름이 서로 다르다** ⭕ · ★ **이름 = 「범례 행 문면 전문」** ⭕ — **M-41 확정 기대치와 축자 일치**(아래 (2') 참조). 지도 안 `aria-label` 은 **컨트롤 3개(확대·축소·거리뷰)뿐이고 마커에는 0** |
| ★ **버튼화된 5개만 `aria-hidden` 이 벗겨졌는가** | 히트 5개: `aria-hidden` **없음** · **조상에도 없음**(`ancestorAriaHidden: "no"`) ⭕ — 각 히트 안에서 **시각 자식은 `aria-hidden="true"`**, `sr-only` 자식이 이름을 진다 ⭕. **나머지 8개(대오 4·무대 4 pill) + 점 3개는 `aria-hidden="true"` 유지** ⭕ |
| ★ **탭 순서 = 범례 순서** | 문서 포커스 순서에서 히트 5개가 **`toilet-north` → `toilet-south` → `cityhall-toilet` → `gwanghwamun` → `cityhall`** = **범례 9·10·11·12·13행 순서** ⭕. 그 뒤 네이버 링크 → 지도 컨트롤(확대·축소·거리뷰) |
| ★★ **«한 걸음 뒤 구별»이 실제로 되는가** | ⭕ **실측했다.** 마커에 포커스 → **Enter** → 팝업 열림 · **`document.activeElement === 팝업 패널`**(포커스가 옮겨 간다) · 팝업 첫 줄 = **`간이화장실 — 대오 1 서쪽입니다`**(범례 전문). **Space** 도 동작하고 **`preventDefault` 되어 페이지가 안 스크롤된다** ⭕ |
| ★ **§55-7 부분 해소 — «충분한가»** | ⭕ **키보드 단독으로 거리뷰까지 도달한다.** 마커 포커스 → Enter → 팝업 → 「거리뷰 보기」 → **시트 열림 · 포커스가 시트로 이동**. → **M-32 판정 28 의 «미달이면 범례 경로를 연다» 조건에 걸리지 않는다. 범례 두 번째 경로는 불필요하다** |

##### (2') ★ M-41 정정 반영 — 접근성 이름 재확인 (2026-08-29)

**AX 트리 계산된 이름 5개**(Playwright 접근성 스냅샷 · `button "…"` 로 나오는 값):

| 마커 | 계산된 이름 | M-41 기대 |
|---|---|---|
| `toilet-north` | `간이화장실 — 대오 1 서쪽입니다` | 동일 ⭕ |
| `toilet-south` | `간이화장실 — 대오 3 남쪽 끝 부근입니다` | 동일 ⭕ |
| `cityhall-toilet` | `시청역 화장실 — 시청역 안 화장실입니다` | 동일 ⭕ |
| `gwanghwamun` | `광화문역(5호선) — 메인무대 북쪽입니다` | 동일 ⭕ |
| `cityhall` | `시청역(1·2호선) — 대오 4 남쪽 구간입니다` | 동일 ⭕ |

**5/5 축자 일치** · `aria-label` **0건** ⭕
★ **«한 걸음 뒤 구별»은 이제 필수 요건이 아니다**(전문이면 훑는 시점에 이미 구별된다). 다만 **팝업이 열릴 때 포커스가 팝업으로 가는지**는 M-36 대로 별도 확인했다 — **`document.activeElement === 팝업 패널`** ⭕(마커 Enter 직후).
...
| # | 결과 |
|---|------|
| **539 제스처·높이** | 기본 배율: 팝업 **112.9px = 상자의 26.9%**(≤50%) ⭕ · `max-height: 50%` · `overflow-y: auto` · `z-index: 20`. **200%**: 상자 296×370 · 팝업 **185px = 정확히 50%** · **`scrollHeight 473 > clientHeight 181` → 패널이 스크롤한다** ⭕ (설계 지시 «넘치면 패널을 스크롤시켜라. 지도를 줄이지 마라» 이행) · **문서 가로 스크롤 0**(팝업 열린 상태 포함) ⭕. **팝업 밖 드래그 가능 영역**: 마커 반대편에 항상 절반 이상 남는다(마커 y96 → 팝업 298~411 / 마커 y310 → 팝업 9~99.9) ⭕ |
| **544 광화문역** | pill 을 눌렀을 때 팝업 **1개** ⭕ · 첫 줄 = 범례 전문 ⭕ · **두 점 각각에서는 안 열린다**(점이 `pointer-events:none`) ⭕ |
| **545 거동** | 다른 마커 → **내용 교체**(개수 1 유지: 시청역 → 간이화장실) ⭕ · **지도 빈 곳 클릭 → 닫힘** ⭕ · **`Esc` → 닫힘 + 포커스가 «연 마커»로 복귀** ⭕ · **「닫기」 → 닫힘 + 포커스 복귀** ⭕ · **「거리뷰 보기」 → 팝업이 함께 닫힘** ⭕ · **마커가 화면 밖으로 나가면(z19 확대) 닫힘** ⭕. ★ 장식 pill(대오 3)을 눌러도 **팝업이 정상적으로 닫힌다** — `pointer-events:auto` 지만 네이버 `clickable:false` 라 «빈 곳 클릭»으로 처리된다 ⭕ |

#### (4) 와드 (F)

| # | 결과 |
|---|------|
| **546 조건부·격리** | 소스: `typeof maps.Marker !== "function"` **기능 감지** + `try/catch` + 조용한 비활성 ⭕ · **와드 실패를 사용자에게 알리지 않는다** ⭕. ⚠ «없어져도 거리뷰가 열리는가» → **F-C** |
| **와드 개수** | 대상 5개인데 **화면에 1개만** ⭕(«지금 연 그 지점» 하나) · `aria-hidden="true"` ⭕ · `pointer-events: none` ⭕ · 라벨이 **짧은 이름**(`간이화장실` · `◉ 시청역(1·2호선)`) ⭕ |
| **547 확신도 분기** | **화장실 = `border-style: dashed` · `#4b5563`** / **역 = `solid` · `rgb(9,51,137)`** ⭕. 분기는 **`confidence` 축**(`approximate` 파생) — **소스에 `id === "…"` 문자열 분기 0건**(1건은 «하지 마라» 주석) ⭕ |
| **548 ★ 대비** | **픽셀로 확인했다**(`popup-ward/w-ward-toilet.png` · pill rect DOM 실측 `121,112–209,137`). ① **`#1a1a1a` 1px 외곽선이 실제로 렌더된다** — pill 경계 1px 표본 **226개 중 105개가 «매우 어두움»**(상대휘도 < 0.05). ② 배경 표본 **78점**(둘레 전체)에 **밝은 하늘빛 `(245,238,213)` L=0.854 와 어두운 그늘 `(20,23,16)` L=0.008 이 «한 프레임 안에» 있다** — 요구된 «두 장면»이 이 한 장에 들어 있다. ③ **흰 면 단독은 78점 중 33점(42%)이 3:1 미달** — «흰 casing 만으로는 밝은 하늘에서 사라진다»는 설계 근거가 **실증**됐다. ④ ★ **«흰 면 또는 외곽선» 중 하나가 3:1 이상인 표본 = 78/78 (100%)** → **와드는 전 둘레에서 배경과 구별된다** ⭕ |
| **549 시야 밖** | 시청역 와드가 시야 밖일 때 rect **`left: -9874.9` · `top: -9928.9`**(네이버가 `-9999px` 로 치운다) ⭕ · **가장자리 «방향 표시» 없음** ⭕(스크린샷 확인) |
| **552 미문서 API** | `fromOffsetToCoord` — **코드 0건**(1건은 주석) ⭕ · `map: pano` — **코드 1곳**(`:1217` 와드) ⭕ **정확히 «하나»** |

#### (5) 문면 (D) · 세로 (E) · 회귀 (G)

| 항목 | 결과 |
|------|------|
| **문면 계수**(닫힘 상태 라이브 · `<script>` 제거 · 본문/속성 분리) | `간이화장실` **4** ⭕ · `시청역 화장실` **2** ⭕ · `광화문역(5호선)` **3** ⭕ · `시청역(1·2호선)` **3** ⭕ · **어포던스 문장 1** ⭕ · `무대·대오는` **0** ⭕ · **`근사` 2** ⭕(§5.3 «셋째 자리 금지» — **4 가 아니다**) · `로드뷰` **0** ⭕ · `여의도`·`국회`·`집회 3구역` **각 0** ⭕ · **`※` 2**(불변) ⭕ · `번호를 누르면` **0** ⭕(QA-551) · `대오` 21 · `지도` 8 · `추후 안내` 2. ★ **범례 13행 전문 계수는 (5') 에서 전수 재측정했다** |
| **550 「근사」 자리** | **화장실 팝업 ⭕ · 화장실 시트 ⭕ · 역 팝업 ❌(없음) ⭕ · 역 시트 ❌(없음) ⭕** — `confidence` 축 분기가 실제로 갈린다. 색은 `text-ink`(**17.40**) · `ink-muted` 아님 ⭕ |
| **551 조작 안내** | 지도 위 캡션에 **`※` 없이** 있다 ⭕ · `font-semibold` · `#1a1a1a`(**17.40**) · **3문장이 공백으로 이어져 한 문단**(M-38) ⭕ |
| **553 · E 세로** | 지도 블록 **910.27px**(예측 950 · **F-E ②**) · 대형 `10시 30분` 하단 **`upcoming` 334.52 / `today` 361.52** — **불변** ⭕ · 원복 334.52 ⭕. ⚠ `main` 높이는 2671.63 → **2772.13**(+100.5)인데 **증가분은 전부 «QR 출석 카드»(별도 커밋)** 다 — 식순 `<section>` top 이 1735.86 → 1835.86 으로 밀렸고 **지도 블록은 그대로**다 |
| **200% 가로** | 375(`cw 360`) 기본 **0** / 200% **0** / **팝업 열린 200% 0** ⭕ |
| **G 회귀 — 401 방화벽** | 프로덕션 · 실제 401(`127.0.0.1` 오리진): `main` **1** · 대체면 · 상자 **336×420**(CLS 0) · **히트 0 · 버튼 0**(죽은 어포던스 0) · 범례 13 · 개요·집결·식순 유지 · **언마운트(클라이언트 내비게이션) 후에도 생존** ⭕ |
| **G 회귀 — F1 포커스 링** | 지도 컨트롤 `outline-offset: -3px`(안쪽) 유지 ⭕ · `outline-color` OFF `#ffffff` / ON `#093389` ⭕ |
| **G 회귀 — 8/28** | `main` 1 · 가로 **기본 0 / 200% 0** · 지도 렌더 ⭕ · **`koscomlabor:roadview-height` 미오염(`null`)** ⭕ |
| **G 회귀 — `/bargaining-2026`** | `main` **1** · `h1` 정상 · 에러 화면 0 ⭕ |
| **타입 우회** | `as any`·`as unknown`·`@ts-ignore`·`@ts-expect-error` **0건** ⭕ |

---

#### (5') ★ M-41 정정 반영 — 기대 개수표 전수 재측정 (2026-08-29)

**M-40 표(«간이화장실 4 / 전문 1»)는 회수됐다. §54.18-1 (2)의 원래 예고가 정본이다.**
**범례 13행 «전문»을 프리렌더·라이브 두 계층에서 전수로 셌다**(`<head>`·`<script>`·`<style>` 제거 후 본문/속성 분리 · `grep -c` 미사용):

| 행 | `roadview` | 프리렌더 | **라이브** | 기대 | 판정 |
|---|---|---|---|---|---|
| 1~8 (메인무대·대오 1·무대 2·대오 2·무대 3·대오 3·무대 4·대오 4) | — | 각 **1** | 각 **1** | 각 1 | ⭕ |
| **9~13** (간이화장실 ×2 · 시청역 화장실 · 광화문역(5호선) · 시청역(1·2호선)) | ⭕ | 각 **1** | **각 2** | 각 2 | ⭕ |

→ **13행 전수 일치 · 불일치 0** ⭕ (라이브의 +1 은 **마커 `sr-only`** 다. 속성 계수는 13행 모두 **0** — 이름이 `aria-label` 이 아니라 내부 텍스트임을 계수로도 확인한다.)

**정정 3(어포던스 문장) 재확인** — «없는지»가 아니라 «맞는지»로 봤다:
- 어포던스 문장 **1** ⭕ · **조작 안내 문단 «안»** 에 있다 ⭕(`MAP_GESTURE_NOTE` 다음 · 공백으로 이어진 한 `<p>`)
- **지도 «위» 문단 2개 유지** ⭕ — ① 조작 안내(완화 + 어포던스) ② `LEGEND_KEY`. **3덩어리가 되지 않았다**(M-38)
- `ROADVIEW_IDS` 가드는 **빌드 성공 자체가 통과 증거**다(모듈 로드 시 `throw` · 프리렌더에서 평가)

⚠ **«전부 0» 일괄 판정은 여전히 못 쓴다** — 위 표의 기대가 전부 0 이 아니다.

---

### §54.18-QA-4 관찰 — 실패는 아니나 기록한다

| # | 내용 |
|---|------|
| **O-1** | ★ **히트의 포커스 링에는 «지도 배경» 처방이 없다.** `HIT_FOCUS_CLASS` 는 `outline-offset-2`(바깥) + `outline-primary` 다. **픽셀로 확인했다** — 링은 **실제로 그려진다**(변화 픽셀 **770** · bbox가 히트 rect를 2~5px 둘러싼다 · 색 `rgb(9,51,137)`). 다만 링이 **지도 타일 위**에 서므로 **둘레 표본의 9.2%(31/336)가 3:1 미달**이다(중앙값 **9.79** · 최소 1.09). **와드는 같은 문제를 `#1a1a1a` 1px 외곽선으로 풀었는데 링에는 그 처방이 없다.** → 링의 90.8%가 3:1 이상이라 포커스 위치는 명확히 보인다 — **디자이너 회부**(2겹 링 또는 `outline-offset-0`) |
| **O-2** | **지도 실패(401) 상태에서도 조작 안내 3문장이 남는다** — 그때 «지도는 손가락 하나로 움직입니다» · «빈 곳을 쓸어 주세요» · **«누르면 거리뷰가 열립니다»** 가 **전부 거짓**이 된다. 어포던스 문장이 추가되며 **거짓 문장이 2 → 3** 으로 늘었다. ⚠ **단순히 `status === "ready"` 로 숨기면 CLS 가 생긴다**(로딩→성공에서 40px 이동) — 그래서 처방이 자명하지 않다. M-33 주석이 적은 «양방향»(늘리면/줄이면)에 **«지도가 없을 때»라는 세 번째 방향**이 빠져 있다 → **검증·디자이너 회부** |
| **O-3** | **팝업이 다른 히트를 덮는다** — 간이화장실(남) 팝업이 `toilet-north`·`gwanghwamun` 을 덮는다. 「최대 50%」 상한의 필연이고 **Esc·빈 곳 클릭·「닫기」 세 경로**가 있어 막다른 길은 아니다. §54.18-3 (3) 의 «다른 마커를 누르면 교체»는 **팝업에 덮이지 않은 마커에 한해** 성립한다 |

---

### §54.18-QA-5 미측정 — **3건 (통과로 적지 않는다)**

| # | 항목 | 사유 |
|---|------|------|
| **U1** | **실기기 가용 높이·툴바** | `100svh == 100lvh == 640`(§5.8.3 판별법) — 환경이 안 바뀌었다 |
| **U2** | **실제 터치·핀치·파노라마 회전** | `maxTouchPoints 0` · `(pointer: coarse) false`. 히트 클릭·드래그·팝업 조작을 **합성 pointer/mouse 이벤트**로 밟았다. ★ **파노라마 시선 회전은 합성 이벤트로 「듣지 않았다」** — QA-548 의 «두 장면»을 회전으로 만들지 못해 **한 프레임 안의 극단 두 배경(L 0.854 / L 0.008)** 으로 대신했다. **다른 촬영 지점·다른 시각의 파노라마는 미측정** |
| **U3** | **`today`·`past` 서버 렌더 실물** | 오늘은 `upcoming`. `361.52` 는 DOM 재구성값(주입 클래스 computed 검산은 재실행했다) |

---

### §54.18-QA-6 ★ 이번 회차에 내가 걸린 것 — 기록

**① 내가 이전 회차에 «프록시로 판정»했다(F-B).**
§54 회귀에서 QA-505 를 **`--strike-label-max` 가 235 → 196 으로 따라온다**로 통과시키며 *«재적합이 실제로 일어난다»* 라고 적었다.
그 값이 증명하는 것은 **«ResizeObserver 가 울렸다»** 까지다. **«지도가 새 크기에 맞춰 다시 맞춰졌다»는 별개 사실**이고, 이번에 참값(**마커가 움직이는가**)으로 재니 **안 움직였다.**
★ §5.8.4 ③(«프록시로 판정하지 마라 — 참값을 재라»)에 정확히 걸린 것이고, **내가 그 규율을 인용해 F1 을 잡은 바로 다음 회차에 같은 함정에 빠졌다.**

**② «0» 이 나왔는데 대상이 있었다 — 이번엔 «CSS 셀렉터 이스케이프».**
컴파일 CSS 에서 `focus-visible:outline-3` 을 그대로 grep 해 **0** 을 얻었다. 실제로는 `.focus-visible\:outline-3:focus-visible` 로 **이스케이프**돼 있었고 다시 세니 **3**.
직전 회차의 «`.next/static/css/` 가 아니라 `chunks/*.css`» 에 이어 **같은 계보의 두 번째 층**이다 — **경로를 맞춰도 «표기»가 틀리면 여전히 0 이 나온다.**

**③ «변화 0» 과 «값 동일»을 다르게 읽어야 했다.**
200% 에서 마커 좌표가 **직전 배율과 완전히 같았다.** 처음엔 «측정 아티팩트»를 의심했는데(§5.5 양방향 의심), 마운트·저작권·축척 바가 **전부 새 크기를 따르는 것**을 보고 **«측정이 아니라 구현»** 으로 갈렸다. 그리고 `window.resize` 를 쏘아 **원인까지** 좁혔다.
★ **«같은 값»이 나왔을 때 «측정이 안 됐다»와 «실제로 안 변했다»를 가르는 것은 «주변 값이 변했는가»** 다.

**④ 진단이 상태를 오염시켰다.**
`window.resize` 진단 뒤 지도 뷰가 초기 뷰와 달라졌다(leftMargin 127 → 125 · bottomMargin 86 → 142). **페이지를 새로고침해 원복**하고 이후 측정을 다시 시작했다. §5.8.4 ①의 «내가 남긴 것을 내가 재지 않는다» 는 **인라인 스타일뿐 아니라 «내가 발동시킨 이벤트의 결과»에도 걸린다.**

---

## §66 QA (2026-09-01 · 9/4 지도 **v2** 폭별 실측 · 200% · 조작 · 포커스 · 대체면)

**대상** `/strike-2026-09-04` — `StrikeMap.tsx` · `strikeMap.ts` v2 재작성분
**도구** Playwright MCP · `http://localhost:3000`(기존 dev 서버 · 재기동 안 함 · `lsof` 로 PID 57018 확인)
**판정 요약** 통과 24 · 실패 0 · 미검증 3 · 관찰 3 · **게시 차단 0**

### §66-0. ★ 빌드 동결 (§5.7) — **한 블록을 «버렸다»**

| 블록 | `StrikeMap.tsx` | `strikeMap.ts` | 처리 |
|---|---|---|---|
| 최초 320·390 (04:29~04:31) | `31f747ab…` | `d6a43f9c…` | ⛔ **폐기** — 블록 도중 dev7 저장으로 dev 서버가 **500** 을 냈고 두 파일 해시가 갈렸다 |
| **A 폭별 재측정** | `ddd393b4…` | `f3e4d48f…` | ⭕ 전·후 동일 |
| **B 200%** | `ddd393b4…` | `f3e4d48f…` | ⭕ 전·후 동일 |
| **C 조작 · D 포커스 · E 대체면** | `ddd393b4…` | `f3e4d48f…` | ⭕ 전·후 동일 |

⚠ **폐기한 블록의 값은 이 리포트에 «한 줄도» 올리지 않았다.** 재측정 결과가 폐기분과 **모든 자리에서 같았지만**
그것을 «그러니 폐기 안 해도 됐다»의 근거로 쓰지 않는다 — 같았다는 것은 **재측정한 뒤에야 알 수 있는 사실**이다.

### §66-1. 측정 환경표 (§5.5 · §5.8.4)

| 항목 | 값 |
|---|---|
| 뷰포트 | `320·360·390·412·768·1280` **× 640**(높이 판정선) |
| `innerWidth − clientWidth` | **0** (스크롤바 없음 — 6개 폭 전부) |
| `devicePixelRatio` | 1 |
| `document.visibilityState` | `visible` (§5.6) |
| 루트 `font-size` | **12px**(기본) / **32px**(텍스트 확대 200% 블록 · 이 프로젝트 관행 §36.12-1) |
| `localStorage` 정리 | 측정 전·후 `koscomlabor:font-scale` **부재**를 «값으로» 확인 · 종료 시 루트 `font-size` **`12px`** 확인 |
| 지도 상자 정의 | 히트의 조상 중 `overflow:hidden` 이고 폭 > 200 인 것(리더 정의). ★ 실측상 **마운트 노드와 종횡비 상자의 rect 가 완전히 같다** — 어느 쪽을 잡아도 판정이 갈리지 않는다 |

### §66-2. A. 폭별 지도 실측 — **6개 폭 전부 통과**

**방법**: 한 번 로드한 뒤 뷰포트만 바꿔 `ResizeObserver → setSize → fit()` 재적합을 태웠다(빌드 1벌 고정).
★ **경로 교차검증**: `390` 을 «새로고침 경로»와 «리사이즈 경로»로 각각 재어 **모든 값이 일치**했다(상자 366×457.5 · 최소 간격 67.98 · 광화문역 −37). 리사이즈 경로가 새로고침 경로와 다른 결과를 내지 않는다.

| 폭 | 지도 상자 | 초기 줌 | 축척 | 히트↔히트 **2차원** 최소 | 가로 스크롤 |
|---|---|---|---|---|---|
| **320** | 296 × 370 | **z16** | **1.892 m/px** | **67.98 px** (`koscom-area`↔`toilet-north`) | **0** |
| **360**(대조) | 336 × 420 | z16 | 1.892 | **67.98** | 0 |
| **390** | 366 × 457.5 | z16 | 1.892 | **67.98** | 0 |
| **412** | 388 × 485 | z16 | 1.892 | **67.54** | 0 |
| **768** | 420 × 525 | z16 | 1.892 | **67.54** | 0 |
| **1280** | 420 × 525 | z16 | 1.892 | **67.54** | 0 |

- **기준** WCAG 2.5.8 **24 px** → **최소값 67.54 px = 기준의 2.81배.** 6개 폭 전부 통과.
  2위 쌍도 68.24~69.07(`toilet-south`↔`cityhall`)로 여유가 크다.
- **2차원 간격 산식**(리더 지정): `dx = max(0, max(ax,bx) − min(ar,br))` · `dy` 동일 · 둘 다 >0 이면 `hypot`, 아니면 `max`. **한 축 성분으로 판정하지 않았다.**
- **줌 참값 근거 2중**: ① 타일 URL `…/styles/basic/…/{z}/…` 의 `z` 가 **6개 폭 전부 16** ② **지물 실거리 역산** — 광화문역↔시청역 **683.3 m / 361.1 px = 1.8921 m/px**(z16 이론값 `156543.034·cos(37.5685°)/2^16 = 1.8936` 과 **0.08% 일치**). 화장실 남↔북 255.5 m / 134.5 px = 1.8991 로 교차 확인.
  ★ **`m/px` 가 6개 폭에서 «완전히 같다»** — `max-w-[420px]` 와 `aspect-[4/5]` 로 세로가 축척을 정하는데, 320 의 상자 높이 370 px 도 `fitBounds` 대상 464 m(245 px) + 패딩 92 px = **337 px** 을 넘어 **z16 이 유지된다.** 어느 폭에서도 **z15 로 떨어지지 않았다** — M-63 ④가 막으려던 상태가 실제로 발생하지 않는다.
- **상자 안에 있는가**: `koscom-area` · `toilet-north` · `toilet-south` · `cityhall` **4개는 6개 폭 전부 «완전히 안»**.

**광화문역만 상자 «위»** — 설계다(M-63 ④). 폭에 따라 노출이 연속적으로 변한다:

| 폭 | 히트 rect 상단 − 상자 상단 | 히트 **하단**이 상자 상단보다 | 초기 화면 노출 |
|---|---|---|---|
| 320 | −81 | **57 px 위** | 0 px (통째 밖) |
| 360 | −56 | **32 px 위** | 0 px ★ **M-66-5 참값 재현** |
| 390 | −37 | 13 px 위 | 0 px |
| 412 | −24 | **0 px**(맞닿음) | 0 px |
| 768·1280 | −5 | −19(= 19 px 안) | **19 px 부분 노출** |

### §66-3. B. 텍스트 확대 200% — **이 페이지 첫 통과. 3개 폭 전부 초과 0**

**방법** `html { font-size: 32px !important }`(이 저장소 관행 · §36.12-1) · **참값은 `documentElement.scrollWidth`**

| 폭 | 루트 | `scrollWidth` | `clientWidth` | **초과** |
|---|---|---|---|---|
| **320** | 32px | 320 | 320 | **0** |
| **360** | 32px | 360 | 360 | **0** |
| **390** | 32px | 390 | 390 | **0** |

- ★ **«뷰포트를 넘는 요소»는 있었다 — 18개.** 전부 **네이버 타일 `<img>` 256px** 이고 `overflow:hidden` 상자 «안»이라 **문서를 밀지 않는다.** 리더 경고대로 **«요소가 넘는다» ≠ «문서가 스크롤한다»** 로 갈랐다. **타일을 제외한 넘침 요소는 0개**였다(3개 폭 전부).
- **범인 격리(`display:none` 토글)는 «필요하지 않았다»** — 초과가 0 이라 가를 대상이 없다. 저장소 상습범(`숫자·괄호·기호`)에 해당하는 문자열(`무대 1`·`시청역(1·2호선)`·`광화문역(5호선)`·`촬영 2026.04`)이 모두 **`break-words` 가 걸린 자리**에 있거나 **지도 캔버스 안 px 고정**이라 rem 확대를 안 받는다.
- ★ **200% 에서 지도 재측정(§5.6 «열이 늘면 배율 축을 다시 재라»)**: 360 × 200% 에서 상자가 `rem` 패딩 증가로 **336 → 296 × 370** 으로 줄었는데 **줌 z16 · 축척 1.892 유지 · 히트↔히트 최소 67.98 px 유지 · 히트 상자 24×24 유지**(캔버스 안은 px 고정이라 확대를 안 받는 설계대로다). **200% 에서도 2.5.8 통과.**
- **정리 확인(§5.8.4 ①)**: 인라인 `font-size` 제거 후 루트 **`12px`**, `document.documentElement.style.fontSize` **없음**, `localStorage` 에 `koscomlabor:font-scale` **부재**. 남은 키는 네이버가 쓴 `__mantle_tile_meta_data` 하나뿐(우리 것이 아니다).

### §66-4. C. 조작 — 히트 5개 × 키보드/클릭 **양쪽**. **전부 통과**

**방법** 실제 키 이벤트(`page.keyboard.press`) · 실제 마우스(`locator.click`). 합성 `KeyboardEvent` 를 쓰지 않았다.

| 히트 | Enter→팝업 | **Enter 이중발화** | Esc→닫힘 | **Esc 후 포커스 복귀** | Space→팝업 | 거리뷰 열림 | **시트 닫은 뒤 포커스 복귀** |
|---|---|---|---|---|---|---|---|
| `koscom-area` | ⭕ | **없음** | ⭕ | ⭕ 마커 | ⭕ | ⭕ (촬영 2026.04) | ⭕ 마커 |
| `toilet-north` | ⭕ | 없음 | ⭕ | ⭕ 마커 | ⭕ | ⭕ | ⭕ 마커 |
| `toilet-south` | ⭕ | 없음 | ⭕ | ⭕ 마커 | ⭕ | ⭕ | ⭕ 마커 |
| `gwanghwamun` | ⭕ | 없음 | ⭕ | ⭕ 마커 | ⭕ | ⭕ (촬영 2025.05) | ⭕ 마커 |
| `cityhall` | ⭕ | 없음 | ⭕ | ⭕ 마커 | ⭕ | ⭕ | ⭕ 마커 |

- ★ **«Enter 가 click 을 두 번 발화시켜 열자마자 닫히는» 회귀 — 재현되지 않았다**(5/5). `role="button"` + 위임 `keydown` 구조가 의도대로 작동한다. **`<button>` 으로 바꾸지 마라**(주석의 근거가 실측으로 확인됐다).
- **클릭 경로**: 4개(`koscom-area`·`toilet-north`·`toilet-south`·`cityhall`)는 **초기 화면에서 그대로 눌린다**(`elementFromPoint` 가 자기 자신을 반환). 팝업 → `거리뷰 보기` 클릭 → 시트 열림 → **시트 「닫기」 버튼 클릭 → 포커스가 원래 마커로 복귀**까지 4/4 통과.
- **`gwanghwamun` 만 초기 화면에서 «클릭 불가»** — `elementFromPoint` 가 지도 밖 요소를 반환하고 `locator.click` 이 타임아웃한다. **설계 결과이며 새 결함이 아니다**(M-63 ④). 아래 §66-5 가 도달 경로를 실측했다.

### §66-5. 게시 조건 **61 확장** — 드래그 도달을 **6개 폭 전부**에서 재측정 [해소]

M-66-4 는 **360 한 폭**에서만 쟀다. 나머지 5개 폭을 채웠다.

| 폭 | 아래로 드래그 | 광화문역 **완전히 상자 안** | **클릭 가능** | 「처음 위치로」 출현 |
|---|---|---|---|---|
| 320 · 390 · 412 · 768 · 1280 | 200 px | ⭕ | ⭕ | ⭕ |
| 360 | 130 px | ⭕ | ⭕ | ⭕ |

⚠⚠ **이 항목에서 «없는 결함»을 낼 뻔했다 — 방법 오류 1건을 기록한다.**
첫 시도에서 **5개 폭 전부 «드래그가 먹지 않는다»**(`처음 위치로` 미출현)가 나왔다. 원인은 코드다 —
**같은 `page.evaluate` 안에서 `window.scrollTo()` 직후에 `getBoundingClientRect()` 를 읽어** 스크롤 «전» 좌표로 드래그 시작점을 잡았다.
`scrollTo` 와 rect 읽기를 **다른 호출로 분리하고 700 ms 기다리자 6개 폭 전부 통과**로 뒤집혔다.
★ **§5.8.4 ③(«프록시로 판정하지 마라»)의 사촌이다** — 참값을 재긴 했는데 **«언제» 잰 것인지가 틀렸다.**
**«전부 실패»가 나오면 대상이 아니라 방법을 먼저 의심하라**(§5.6 의 `visibilityState` 사례와 같은 계보).

### §66-6. D. 포커스 표시 — **잠정 통과**(⚠ dev7 완료 통보 «전» 측정)

**방법** 실제 `Tab` 연타로 도달(프로그램적 `focus()` 아님 — `:focus-visible` 이 걸려야 한다) · panTo 정착까지 900 ms 대기(§5.9-2)

| 폭 | Tab 도달 | `:focus-visible` | 링 선언 | **히트 rect 완전히 상자 안** | **링(rect+4px)까지 상자 안** | 「처음 위치로」 |
|---|---|---|---|---|---|---|
| 320·360·390·412·768·1280 | ⭕ (7번째 정지점) | ⭕ | `0 0 0 2px #fff, 0 0 0 4px #1a1a1a` + `outline 2px transparent`(HCM 용) | ⭕ **6/6** | ⭕ **6/6** | ⭕ |

최소 여백(320 기준) 위 147 · 아래 199 · 좌우 각 103 px → **링 4 px 이 잘릴 여지가 없다.**

- ★ **«페이지가 스크롤됐다»를 통과로 적지 않았다.** `scrollY` 는 실제로 움직였지만(488~674) **판정은 rect 포함 관계로만** 했다.
- ★ **«그려졌다»를 선언이 아니라 «픽셀»로 확인했다**(§66-E 의 교훈을 이 항목에도 적용): 360 에서 히트 주변 **106 × 40 px** 을 포커스 상태·해제 상태로 각각 캡처해 **바이트가 달랐다**(4,284 B 차이). 지도는 그 사이 움직이지 않았다(panTo 정착 후 `blur()` 만 했다). **`getComputedStyle` 만 보고 «있다»로 적지 않았다.**
- ⚠ **이 절은 dev7 이 «수정 완료»를 알리기 «전»에 잰 값이다.** 해시 `ddd393b4…`/`f3e4d48f…` 기준이며, dev7 이 이후 이 파일을 고치면 **회귀 재측정이 필요하다.**

### §66-7. E. 실패 대체면 — **배치로 확인. 문자열로 통과시키지 않았다**

| 시나리오 | 방법 | 대체면 rect | 상자 rect | **상자를 채우는가** | 페이지 |
|---|---|---|---|---|---|
| 스크립트 차단 | `*.map.naver.com` · `*.pstatic.net` 전부 `abort` · 8 s 타임아웃 경과 후 12 s 시점 | **336 × 420** | 336 × 420 | ⭕ **완전 일치** | `main` 1 · `h2` 4 · 본문 1,320자 |
| `/v3/auth` → 401 | `route` 로 401 fulfill | 336 × 420 | 336 × 420 | ⭕ | `main` 1 · `h2` 4 · `figure` 1 · 본문 1,320자 |

- **문면**(렌더 텍스트) `지도를 불러오지 못했습니다. / 집회 장소는 아래 목록에 있습니다.` — **「여의도」·「국회의사당역」·「집회 3구역」 류 8/28 문자열은 0**(M-5 가 막은 거짓 게시 경로가 실제로 없다).
- **`absolute inset-0` 이 «실제로» 일한다** — 높이 420 > 0. 주석이 경고한 «래퍼를 빼면 0 px» 상태가 아니다.
- 가로 스크롤 **0**.

### §66-8. 미검증 — **3건. 「통과」로 적지 않는다**

| # | 항목 | 사유 |
|---|---|---|
| **U1** | **§54.10 «401 인데 `window.naver.maps` 는 있고 내부가 null» 모드** | `/v3/auth` 401 을 만들었더니 **`window.naver.maps` 자체가 생기지 않았다**(`naverMapsPresent: false`) — 스크립트 부트스트랩이 함께 죽는다. **§54.10 이 재현한 «트리 전체가 날아가는» 경로를 이 방법으로는 못 만들었다.** 지금 관측된 실패 모드에서는 페이지가 온전하다(`main` 1 · `h2` 4) |
| **U2** | **실제 터치·핀치** | `maxTouchPoints 0`. 드래그·클릭을 **합성 마우스 이벤트**로 밟았다. 실기기 한 손가락 드래그·핀치 확대는 미측정(직전 회차 U2 와 같은 한계) |
| **U3** | **프로덕션 빌드** | 전부 `next dev`(StrictMode 이중 마운트) 위 측정이다. M-16 이 남긴 «프로덕션 미확인»은 이 회차도 못 닫는다 |

### §66-9. 관찰 — 결함 아님 · 리더 판단용

| # | |
|---|---|
| **O1** | **768·1280 에서 광화문역 히트가 «19 px 만» 노출된다**(24 px 중). 포인터 사용자의 유효 타깃이 88 × 19 로 **한 축이 24 미만**이다. ⭕ **WCAG 2.5.8 위반은 아니다 — 「간격(spacing)」 예외**로 통과한다(가장 가까운 다른 히트가 **67.54 px**). ⚠ 그러나 «부분만 보이는 정지점»은 320~412 의 «아예 안 보임»보다 **오히려 설명하기 어렵다** — 그 두 폭에서만 조합원이 «잘린 알약»을 본다. 종횡비 후속(#30) 판정 때 같이 볼 것 |
| **O2** | **키보드가 포인터보다 «먼저» 닿는다.** 320~412 에서 광화문역은 **탭으로는 즉시 도달·자동 panTo 로 완전 노출**되는데 **포인터로는 드래그 200 px 이 선행**돼야 한다. 접근성 역전이라 문제는 아니지만, M-66-4 가 «드래그로 닿는다»로 닫은 조건 61 은 **포인터 경로만** 말한다 — **키보드 경로는 별개로 성립**한다(§66-6) |
| **O3** | **콘솔 에러 2건이 매 로드마다 난다 — 9/4 와 무관한 기존 건.** `src/app/layout.tsx:30-37` 의 글자 크기 선반영 `<script>` 가 **`<html>` 직계 자식(= `<head>` 밖)** 이라 React 가 *«In HTML, `<script>` cannot be a child of `<html>`. This will cause a hydration error.»* 를 낸다. **전 페이지 공통**이고 이번 라운드 diff 밖이다. 하이드레이션은 실제로 깨지지 않았다(측정 중 모든 상호작용 정상). **FOLLOWUPS 후보** |

### §66-10. 이번 회차에 내가 걸린 것 — 기록

**① «전부 실패»가 나왔을 때 대상이 아니라 방법을 의심해야 했다**(§66-5). 5개 폭에서 드래그가 통째로 안 먹은 것은
`scrollTo` 와 rect 읽기를 **한 evaluate 안에 묶은 것** 때문이었다. 그대로 올렸으면 **«드래그가 360 에서만 된다»는 거짓 실패**가 났고, 리더가 M-66-4 를 되돌렸을 것이다. ★ **§5.6(«잴 수 있는 상태인가»)의 시간 축 버전이다** — 환경이 아니라 **«읽은 시점»** 이 문제였다.

**② 빌드가 흔들리는 동안 잰 값을 버리는 데 «두 번» 망설였다.** 재측정값이 폐기분과 전부 같아서 *«그럼 안 버려도 됐네»* 로 갈 뻔했다. **같았다는 것은 재측정한 뒤에야 알 수 있는 사실**이고, 그 논리를 한 번 허용하면 다음번 «다른» 경우를 못 잡는다. §66-0 에 그대로 적었다.

**③ 상자 정의가 «두 요소에 다 맞는» 것을 확인하고 넘어갔다.** 리더 정의(`overflow:hidden` · 폭 > 200)로 잡히는 것이 **마운트 노드**였는데 종횡비 상자와 **rect 가 완전히 같아** 판정이 갈리지 않는다. ★ 이 저장소가 반복한 **«같은 이름으로 다른 양»**(§5.7 `min-content` 사례)이 **이번엔 «다른 이름인데 같은 양»** 이었다 — 그래도 **확인하고 나서** 넘어갔다.

---

## §67 QA (2026-09-01 · 9/4 지도 v2 — **재측정 회차**. §66 을 대체한다)

> ⛔ **§66 의 수치를 인용하지 마라.** 그 회차 이후 소스 6개가 두 번 바뀌었고 **§66 은 히트↔장식을 아예 안 쟀다**(리더 정정 M-67 이전이라 «무대 pill 이 없다»는 거짓 전제 위에 있었다). §67 이 그 자리를 대신한다.
> ⭕ **§66 에서 «살아 있는» 것**: 히트↔히트 값 · 조작 결과 · 대체면 배치는 §67 에서 **전부 같은 값으로 재현**됐다. 다만 **인용은 §67 로 하라**(두 벌이 되면 다음 사람이 낡은 쪽을 쓴다).

**판정 요약** 통과 41 · **실패 0** · 미검증 3 · 관찰 4 · **게시 차단 0**

### §67-0. ★★ 빌드 동결 — **회차를 «두 번» 버렸다** (§5.7)

| 회차 | 기준 해시 | 처리 |
|---|---|---|
| §66 최초 320·390 | `31f747ab`/`d6a43f9c` | ⛔ 폐기 — 측정 중 dev 서버 **500** · 해시 갈림 |
| §66 본체 | `ddd393b4`/`f3e4d48f` | ⛔ **폐기** — 그 뒤 소스 **6개 전부** 변경 |
| **§67 (이 회차)** | 아래 표 · **전·후 동일** | ⭕ 유효 |

**§67 동결 기준 (측정 블록 전·후 6개 파일 + 프리렌더 모두 동일):**
```
78eddeb08808  src/lib/strikeMap.ts
e18d0f140a18  src/lib/naverMaps.ts
91db83c122a3  src/components/strike/StrikeMap.tsx
01de808253f5  src/components/strike/StrikeRoadviewSheet.tsx
e7466e110ec4  src/app/strike-2026-09-04/page.tsx
901624b974ac  src/components/rally/RallyMap.tsx      (8/28 회귀용)
프리렌더 가시 마크업  18,410자 · 2c4be85ba66d   (정의: `<script>…</script>` 제거 후 sha256 · 24초간 3회 읽어 동일)
```

#### ★★★ 이 회차가 세운 규율 — **«프리렌더가 같다»는 지도에 대해 아무것도 증명하지 않는다**

개발자가 `page.tsx` 재수정 후 *«프리렌더 가시 마크업 sha256 이 전/후 완전 일치하므로 진행 중인 A~E 블록은 그대로 유효하다»* 라고 보고했다. **본문에 대해서는 옳지만 지도에는 적용되지 않는다** — **지도 DOM 은 클라이언트에서 주입되어 프리렌더에 «존재하지 않는다».** 히트 5개·무대 pill 4개·타일 어느 것도 그 해시에 들어가지 않는다.
→ ★ **«산출물 대조»를 쓸 때는 «그 산출물이 검사 대상을 담고 있는가»를 먼저 물어야 한다.** 이 저장소의 «세는 방법이 질문에 안 맞는» 계보에 **«대조 «면»이 질문에 안 맞는»** 형태가 추가된다.
→ 그래서 **A~E 를 전부 다시 쟀다.** (결과적으로 모든 값이 같았지만, 그것은 재측정한 «뒤에» 알 수 있는 사실이다.)

⚠ 부수 기록: 개발자의 프리렌더 수치(17,911자 / `7fe71304…`)와 내 수치(18,410자 / `2c4be85b…`)는 **다르다. 모순이 아니라 «스트립 정의»가 다른 것**이다(§5.7 «같은 이름으로 다른 양»). **서로의 숫자로 상대를 반박할 수 없다** — 각자 «자기 정의로 전·후»를 비교해야 한다.

#### 참고 — 두 기준선 사이의 «가시 텍스트» 차이는 딱 한 곳
```
- 사전집회 / 구호제창 / 동영상 상영/경과보고
+ 사전집회
+ 구호제창 / 동영상 상영/경과보고
```
식순 표의 한 칸이 두 칸으로 갈렸다(지도와 무관). ★ **그런데 이것은 §5.9(«원문과 화면이 달라지는 지점은 순서·열·묶음 세 축으로 센다»)의 «묶음» 변경이다 — `grep` 으로는 안 잡힌다.** 검증자 회부 대상으로만 적는다(QA 판정 아님).

### §67-1. 측정 환경표 (§5.5 · §5.8.4)

| 항목 | 값 |
|---|---|
| 뷰포트 | `320·360·390·412·768·1280` **× 640** |
| `innerWidth − clientWidth` | **0** (전 폭 · 스크롤바 없음) |
| `dpr` 1 · `visibilityState` `visible` | (§5.6 확인) |
| 루트 `font-size` | **12px** 기본 / **32px** 200% 블록(관행 §36.12-1) |
| 정리 확인 | 200% 블록 4개 전부 종료 시 **`12px` / 인라인 `none`** 을 값으로 확인 · `koscomlabor:font-scale` 키 부재 |
| 서버 | PID 57018 유지(재기동 0) · `/strike-2026-09-04` 200 · `/rally-2026-08-28` 200 |

### §67-2. A. 폭별 실측 — 6폭 통과

| 폭 | 지도 상자 | 줌 | 축척 | **히트↔히트 2차원 최소** | 가로 스크롤 |
|---|---|---|---|---|---|
| 320 | 296 × 370 | z16 | 1.8921 m/px | **67.98** (`koscom-area`↔`toilet-north`) | 0 |
| 360 | 336 × 420 | z16 | 1.8921 | **67.98** | 0 |
| 390 | 366 × 457.5 | z16 | 1.8921 | **67.98** | 0 |
| 412 | 388 × 485 | z16 | 1.8921 | **67.54** | 0 |
| 768 | 420 × 525 | z16 | 1.8921 | **67.54** | 0 |
| 1280 | 420 × 525 | z16 | 1.8921 | **67.54** | 0 |

- **기준** WCAG 2.5.8 = 24 px → 최소 **67.54 = 기준의 2.81배**. 히트 상자는 전 폭에서 `24×24`(화장실) · `87~90 × 24`(pill).
- **축척 근거 2중**: 타일 `z`(전 폭 16) + 광화문역↔시청역 **683.3 m / 361.1 px**. z16 이론값 1.8936 과 0.08% 일치.
- **상자 안**: 4개는 전 폭 «완전히 안». **광화문역만 상자 «위»**(설계 M-63 ④) — 320 −81 / 360 −56 / 390 −37 / 412 −24 / 768·1280 −5 (rect 상단 − 상자 상단).

### §67-3. ★ A-2b. **히트 ↔ 장식** — 신설 항목 (판정선 없음 · 규격 아님 · 시각)

**리더 정정(M-67) 반영.** 「무대 pill 이 없다」는 거짓이었고, **`span` 으로만 훑으면 안 잡힌다**(무대 pill 은 `<div>`).
→ **태그를 가정하지 않고** 지도 상자 안 후손 전수에서 **«자기 직접 텍스트 노드»를 가진 요소**를 모았다.

**상자 안 장식 목록**(전 폭 공통 · 히트 안·`sr-only` 제외):
`div:무대 1[45×24]` · `div:무대 2[46×24]` · `div:무대 3[47×24]` · `div:무대 4[47×24]` · `span:100m`(축척바) · `span:© NAVER Corp.` · **`a:더보기[29×19]`**(320 폭에서만 출현)

| 폭 | 최근접 쌍 | **2차원 간격** | 축 | 겹침 |
|---|---|---|---|---|
| 320·360·390·412·768·1280 | `toilet-north` ↔ **무대 2 pill** | **2.0 px** | 세로(`dx 0 · dy 2`) | **없음** |
| 〃 | `toilet-north` ↔ 무대 1 pill | 7.0 px | 세로 | 없음 |
| 〃 | `toilet-south` ↔ 무대 4 pill | 10.0 px | 대각(`dx 8 · dy 6`) | 없음 |
| 〃 | 4위 | 20.88(≤390) / 21.84(≥412) | | 없음 |

★ **3출처 일치**: 리더 2.0 / 개발자 2.0 / QA 2.0. **안정된 값으로 본다.**
★ **음수(겹침) 0 — 6폭 × (히트↔히트 · 히트↔장식 · 장식↔장식) 전수에서 겹침이 하나도 없다.** 리더가 지정한 결함 조건에 걸리지 않는다.
⭕ **WCAG 2.5.8 대상이 아니다** — 무대 pill 은 `roadview` 없음 = 히트 아님. **규격이 아니라 시각 사안**으로 분류한다.

### §67-4. B. 텍스트 확대 200% — **초과 0. 그러나 «새 사실»이 나왔다**

**방법** `html { font-size: 32px !important }` · 참값 `documentElement.scrollWidth`

| 폭 | `scrollWidth` − `clientWidth` | 지도 상자 | **줌** | 축척 | 히트↔히트 최소 | 히트↔장식 최소 |
|---|---|---|---|---|---|---|
| **320** | **0** | 256 × 320 | ⚠ **z15** | **3.7946 m/px** | **32.25** (`toilet-south`↔`cityhall`) | **4.0** (`toilet-north`↔무대 1·2) |
| 360 | **0** | 296 × 370 | z16 | 1.8921 | 67.98 | 2.0 |
| 390 | **0** | 326 × 408 | z16 | 1.8921 | 67.98 | 2.0 |
| 412 | **0** | 348 × 435 | z16 | 1.8921 | 67.54 | 2.0 |

- **가로 스크롤 초과 0 — 4개 폭 전부.** ★ **이 페이지가 이 규율을 통과한 첫 기록이다.**
- 넘치는 «요소»는 12~18개 있으나 **전부 클리핑 조상(`overflow:hidden`) 안**이다 — **문서를 밀지 않는다.** 클리핑되지 않은 넘침 요소는 **4개 폭 전부 0개**.

#### ★★ 신규 사실 — **`320` 폭 + 200% 에서만 z15 로 떨어진다**

M-63 ④의 기각 근거가 *«z15 로 떨어지면 아무것도 못 누른다»* 였다. **z15 가 실제로 발생하는 조건이 발견됐다.** 그 상태를 전수로 쟀다:

| 항목 | z15 실측 | 판정 |
|---|---|---|
| 히트↔히트 2차원 최소 | **32.25 px** | ⭕ **≥ 24 통과**(여유 8.25) |
| 히트↔장식 최소 | **4.0 px** (가로 `dx 4 · dy 0`) | 규격 아님 · **z16 의 2.0 보다 «넓다»** |
| 장식↔장식 최소 | 4.0 px (무대 1↔무대 2) | 규격 아님 |
| 겹침 | **0** (전 조합) | ⭕ |
| 히트 5개 상자 안 | **5/5 «완전히 안»** — 축척이 절반이라 **광화문역이 초기 화면에 들어온다** | ⭕ |

★ **M-63 ④가 예보한 «13.2 px 24 미달»은 v2 좌표에서 재현되지 않는다.** 그 예보는 **v1 좌표 + «무대 2 에 pill 이 있다»는 (지금은 철회된) 전제** 위에 있었다. ⚠ **이것을 «그러니 종횡비를 고쳐도 된다»로 읽지 마라** — 후속 #30 은 그대로 열려 있고, 이 절은 **«z15 가 되어도 규격 실패가 없다»는 사실만** 말한다.
⚠ **축이 바뀐다**: z16 은 세로로(dy 2), z15 는 가로로(dx 4) 갈린다. **한 축 성분으로 판정하면 두 배율에서 서로 다른 축을 본다.**

### §67-5. C. 조작 — 히트 5개 × 키보드·클릭. **전부 통과**

| 히트 | Enter→팝업 | **이중발화** | Esc→닫힘·포커스 복귀 | Space→팝업 | 거리뷰 시트 | **시트 닫은 뒤 마커 복귀** | 클릭 |
|---|---|---|---|---|---|---|---|
| `koscom-area`·`toilet-north`·`toilet-south`·`gwanghwamun`·`cityhall` | ⭕ 5/5 | **없음 5/5** | ⭕ 5/5 (원래 마커) | ⭕ 5/5 | ⭕ 5/5 | ⭕ 5/5 (원래 마커) | ⭕ 5/5 |

- ★ **«Enter 가 click 을 두 번 발화시켜 열자마자 닫히는» 회귀 재현 0.** `role="button"` + 위임 `keydown` 구조를 **`<button>` 으로 바꾸지 마라**(주석 근거가 실측으로 확인됐다).
- ⚠ **`gwanghwamun` 의 «클릭»은 «초기 화면»이 아니다** — 이 표의 클릭은 앞선 키보드 조작으로 `panTo` 가 일어난 «뒤»의 상태다. **초기 화면에서 포인터로는 안 눌린다**(§67-6).

### §67-6. 게시 조건 61 확장 — 초기 화면 포인터 도달성

| 폭 | 초기 화면 포인터 클릭 | 드래그 후 |
|---|---|---|
| 320·360·390·412 | ❌ **불가**(히트가 상자 밖 · `elementFromPoint` 가 지도 밖 요소 반환) | ⭕ 아래로 130~200 px 드래그 → **완전히 상자 안 + 클릭 가능 + 「처음 위치로」 출현** |
| 768·1280 | 부분(24 px 중 **19 px** 노출 · 중심점은 상자 밖이라 «중앙 클릭»은 실패) | ⭕ 동일 |

⚠⚠ **방법 오류를 기록한다 — 하마터면 «드래그가 전 폭에서 안 먹는다»는 거짓 실패를 낼 뻔했다.**
첫 시도에서 5개 폭 전부 「드래그 무효」가 나왔다. 원인은 **같은 `page.evaluate` 안에서 `window.scrollTo()` 직후에 `getBoundingClientRect()` 를 읽어** 스크롤 «전» 좌표로 드래그 시작점을 잡은 것이다. **호출을 분리하고 700 ms 기다리자 6/6 통과**로 뒤집혔다.
★ **§5.8.4 ③의 시간 축 버전이다** — 참값을 재긴 했는데 **«언제» 잰 것인지가 틀렸다.** **«전부 실패»가 나오면 대상보다 방법을 먼저 의심하라.**

### §67-7. D. 포커스 표시 — **30/30 통과** (6폭 × 5정지점)

**방법** 실제 `Tab` 연타(프로그램적 `focus()` 아님 · `preventScroll` 없음) · panTo·전이 정착 **850 ms** 후 판독(§5.9-2) · **상자 rect 와 히트 rect 를 «같은 시점»에** 읽음

| 검사 | 결과 |
|---|---|
| 탭이 5개 히트에 전부 도달 | ⭕ 30/30 (지도 첫 히트는 **4번째 탭 정지점**) |
| `:focus-visible` 성립 | ⭕ 30/30 |
| 링 선언 = `0 0 0 2px #fff, 0 0 0 4px #1a1a1a` (+`outline 2px transparent` — HCM 용) | ⭕ 30/30 |
| **포커스된 히트가 상자 안에 «완전히»** | ⭕ 30/30 · **가시 면적 100%** |
| **링(rect+4px)까지 상자 안** | ⭕ 30/30 · 최소 여백 **14.8 px**(320 `koscom-area`) |
| 「처음 위치로」 복귀 경로 출현 | ⭕ |

- ★ **«페이지가 스크롤됐다»를 통과로 적지 않았다.** 판정은 **rect 포함 관계**로만 했다.
- ★ **«그려졌다»를 선언이 아니라 «픽셀»로 확인했다**: 360 에서 히트 주변 **106 × 40 px** 을 포커스 상태·해제 상태로 캡처해 **바이트가 달랐다**(4,284 B). panTo 정착 후 `blur()` 만 했으므로 지도는 그 사이 움직이지 않았다.

#### ★★ 가드 검증 — **«정확히 0.0 px»이다. 다만 «탭 순회 중 지도가 두 번 움직인다»**

⚠⚠ **첫 측정에서 «가드가 깨졌다»(cityhall 에서 228 px 이동)가 나왔다. 교락이었다.**
`cityhall` 로 가는 탭 경로가 **`gwanghwamun` 을 반드시 통과**한다(배열 순서상 바로 앞). 그 228 px 은 **통과 잔여**다.
→ **탭 «한 칸마다» 상자 기준 상대좌표로 지도 이동을 쟀다**(개발자 경고대로 뷰포트 절대좌표를 쓰지 않았다 — 페이지가 488 px 스크롤되므로):

| 탭 | 포커스 대상 | **지도 이동(상자 기준)** | |
|---|---|---|---|
| 4 | `koscom-area` | **0.00 · 0.00** | ⭕ 가드 |
| 5 | `toilet-north` | **0.00 · 0.00** | ⭕ 가드 |
| 6 | `toilet-south` | **0.00 · 0.00** | ⭕ 가드 |
| 7 | `gwanghwamun` | +2.00 · **+226** | ⭕ 의도된 `panTo` |
| 8 | `cityhall` | 0.00 · **−352** | ⭕ **가드가 아니라 «필요한» 이동** — 7번 팬으로 `cityhall` 이 실제로 상자 밖이 됐다 |

★ **가드는 «초기 뷰에서 이미 안쪽인 히트»에 대해 정확히 0.0 px 이다.** 8번의 이동은 «가드 실패»가 아니라 **«그 시점에 대상이 진짜로 밖이라 옮긴 것»** 이고, 그 결과 **모든 정지점이 100% 가시**가 된다(위 30/30).
⚠ **파생 사실(관찰 O4)**: 키보드 사용자가 마커 5개를 순회하면 **마지막 두 정지점에서 지도가 한 번씩 움직인다.** 각 이동은 «그 정지점을 보이게 하는 데 필요한 최소»이며 **WCAG 2.4.7 을 만족시키는 쪽**이다. 흔들림을 없애려면 광화문역을 `fitBounds` 에 넣어야 하는데 **그것은 M-63 ④를 되돌리는 것**이다 — **QA 가 결정할 사안이 아니라 리더 판정 사항으로 올린다.**

### §67-8. E. 실패 대체면 — **배치로 확인**

| 항목 | 값 |
|---|---|
| 대체면 rect | **336 × 420** = 지도 상자 336 × 420 → **완전 일치**(`absolute inset-0` 이 실제로 일한다 · 높이 > 0) |
| 문면 | `지도를 불러오지 못했습니다. / 집회 장소는 아래 목록에 있습니다.` |
| 8/28 오염 문자열 | `여의도` **0** · `국회의사당` **0** · `집회 3구역` **0** (M-5 가 막은 경로가 실제로 없다) |
| 페이지 온전성 | `main` 1 · `h2` 4 · 본문 1,318자 · 가로 스크롤 0 |

### §67-9. ★ 8/28 회귀 스모크 (`naverMaps.ts` 공유 모듈 변경분)

| 검사 | 결과 |
|---|---|
| 페이지 응답 | **200** |
| 지도 그려짐 | ⭕ **네이버 타일 16장**(`nrbe.map.naver.net`) · `window.naver.maps` 있음 · 히트 **7개** · 지도 상자 366×458 |
| 문서 구조 | `main` 1 · `h1` 1 · 가로 스크롤 0 |
| **「지난 안내」 문면** | ⭕ **`2026년 8월 28일 일정이 지났습니다. 아래 안내는 기록으로 남겨 둡니다.`** — 그대로 있다 |
| 콘솔 오류 | 2건 — **9/4 와 «같은» 건**(`layout.tsx` 글자 크기 선반영 `<script>`). `naverMaps.ts` 변경과 무관 |

⚠ **여기서도 세는 방법이 한 번 틀렸다**: 처음에 `img[src*="pstatic.net"]` 로 세어 **타일 0**이 나왔다. **8/28 타일 호스트는 `nrbe.map.naver.net`, 9/4 는 `nrbe.pstatic.net` 으로 다르다.** 호스트를 열거해 다시 세니 16장. **«0이 나오면 세는 방법부터 의심하라»가 이 회차에만 세 번째다.**

### §67-10. 미검증 — 3건 (「통과」로 적지 않는다)

| # | 항목 | 사유 |
|---|---|---|
| **U1** | **§54.10 «401 인데 `window.naver.maps` 는 있고 내부가 null»** | `/v3/auth` 를 401 로 막으면 **`window.naver.maps` 자체가 안 생긴다** — 그 실패 «모드»를 이 방법으로는 못 만든다. 관측된 두 실패 경로(스크립트 차단 · auth 401)에서는 페이지가 온전했다 |
| **U2** | **실제 터치·핀치** | `maxTouchPoints 0`. 드래그·클릭은 합성 마우스 이벤트 |
| **U3** | **프로덕션 빌드** | 전부 `next dev`(StrictMode 이중 마운트). M-16 의 «프로덕션 미확인»은 이 회차도 못 닫는다 |

### §67-11. 관찰 — 결함 아님 · 리더 판단용

| # | |
|---|---|
| **O1** | **768·1280 에서 광화문역 히트가 24 px 중 19 px 만 노출.** 유효 타깃 88×19 로 한 축이 24 미만이나 **WCAG 2.5.8 «간격» 예외로 통과**(최근접 히트 67.54 px). ⚠ 시각적으로는 **«잘린 알약»이 보이는 유일한 구간**이라 종횡비 후속 #30 때 함께 볼 것 |
| **O2** | **키보드가 포인터보다 먼저 닿는다** — 320~412 에서 광화문역은 탭으로 즉시 도달·자동 노출되는데 포인터는 드래그가 선행돼야 한다. 접근성 역전이라 문제는 아니나, **게시 조건 61 의 «드래그로 닿는다»는 포인터 경로만** 말한다 |
| **O3** | **콘솔 오류 2건(전 페이지 공통·기존)** — `src/app/layout.tsx:30~37` 의 글자 크기 선반영 `<script>` 가 **`<head>` 밖 `<html>` 직계**라 React 가 *«`<script>` cannot be a child of `<html>`. This will cause a hydration error.»* 를 낸다. 실제 하이드레이션은 깨지지 않았다(모든 상호작용 정상). 이번 라운드 diff 밖 · **FOLLOWUPS 후보** |
| **O4** | **탭 순회 중 지도가 두 번 움직인다**(§67-7) — 각각 «그 정지점을 보이게 하는 최소 이동»이다. 없애려면 광화문역을 `fitBounds` 에 넣어야 하고 **그것은 M-63 ④를 되돌리는 것**이라 **리더 판정 사항** |

### §67-12. 이번 회차에 내가 걸린 것 — 기록

**① 「0이 나왔다」를 세 번 만났고 세 번 다 «세는 방법»이었다.** ⓐ 리더의 «무대 pill 없음»(`span` 만 훑음 — 참값은 `<div>`) ⓑ 8/28 타일 0(호스트가 다름) ⓒ 드래그 전 폭 실패(스크롤 직후 rect 읽음). **셋 다 대상이 아니라 방법이었다.**

**② 「대조 «면»이 질문에 안 맞는」 새 형태를 만났다**(§67-0). 「프리렌더 해시 일치」는 **클라이언트에서 주입되는 지도에 대해 아무 말도 하지 않는다.** 이 저장소의 «세는 방법» 계보에서 **«무엇을 세느냐»가 아니라 «어디를 보느냐»가 틀린** 첫 사례다.

**③ 교락을 «가드 실패»로 올릴 뻔했다**(§67-7). `cityhall` 의 228 px 이동은 **탭 경로가 `gwanghwamun` 을 통과한 잔여**였다. **탭 한 칸마다 차분을 내서야** 갈렸다. ★ 개발자가 경고한 함정(뷰포트 절대좌표)은 **피했는데 다른 함정(경로 교락)에 걸렸다** — 상대좌표로 바꾼 것만으로는 부족했다.

**④ 회차를 두 번 버리면서 «안 버려도 됐다»는 유혹을 두 번 받았다.** 재측정 결과가 폐기분과 **모든 자리에서 같았다.** 그러나 **같았다는 것은 재측정한 뒤에야 알 수 있는 사실**이고, 그 논리를 한 번 허용하면 다음번 «다른» 경우를 못 잡는다.

---

## §68 QA (2026-09-01 · **런타임 문면 계층** + 식순 회귀) — §67 의 후속. 기준선 동일

**판정 요약** 통과 52 · **실패 0** · 미검증 0(이 회차 범위 내) · 관찰 2

### §68-0. 동결 — §67 과 **같은 기준선**. 측정 전·후 동일

```
78eddeb08808  strikeMap.ts        e18d0f140a18  naverMaps.ts       91db83c122a3  StrikeMap.tsx
01de808253f5  StrikeRoadviewSheet.tsx            e7466e110ec4  page.tsx
프리렌더 가시 마크업  18,410자 · 2c4be85ba66d
지도 청크(개발자 제안 채택 · 독립 확인)  79b75364b6da 2x30s49ubaouq.js  ·  b7c91f5a0b05 1fjlrv_zz7ye2.js
```
★ **개발자가 제안한 «소스 해시 말고 지도 청크 해시를 고정하라»를 채택했다.** 그 두 값을 직접 계산해 **개발자 보고와 완전히 일치**함을 확인했다. 앞으로 주석만 바뀌면 이 값이 유지되고 **A·C·D·E 재측정이 불필요하다** — §67-0 이 지적한 «프리렌더는 지도에 안 닿는다»의 **올바른 대체 계층**이다.

### §68-1. ★ 마커 접근성 이름 — **접근성 트리에서 읽었다. 5/5 통과**

**방법** Playwright `locator.ariaSnapshot()`(계산된 접근성 이름) — 소스가 아니다.

| 히트 | **접근성 이름(트리 실측)** | `aria-label` | `title` |
|---|---|---|---|
| `koscom-area` | `코스콤지부 구역 — 무대 2 남쪽, 코리아나호텔(폴바셋) 앞입니다` | **null** | **null** |
| `toilet-north` | `임시화장실 — 무대 1 과 무대 2 사이 서쪽입니다` | null | null |
| `toilet-south` | `임시화장실 — 무대 3 과 무대 4 사이 서쪽입니다` | null | null |
| `gwanghwamun` | `광화문역(5호선) — 무대 1 북쪽입니다` | null | null |
| `cityhall` | `시청역(1·2호선) — 무대 4 남쪽입니다` | null | null |

- ⭕ **규약 준수: 이름 = `feature.legend` «전문»** 이다. 짧은 이름이 아니라 전문이라 **임시화장실 둘이 «무대 1·2 사이» / «무대 3·4 사이»로 구별된다** — 스크린리더 링크·버튼 목록에서 둘이 같은 이름으로 뭉치지 않는다.
- ⭕ **`union-webapp-dev §8` 준수** — `aria-label`·`title` 이 **전부 null** 이고 **내부 텍스트가 이름을 진다.** 시각 pill 텍스트는 `aria-hidden="true"` 라 이름에서 빠지고 `.sr-only` 전문만 남는다(중복 낭독 없음).
- 전부 `role="button"` · `tabindex="0"` · `aria-hidden` 없음(포커스 가능 요소를 `aria-hidden` 뒤에 두지 않음 — WCAG 4.1.2).

### §68-2. 팝업 문면 — 5/5. **`APPROX_NOTE` 가 확신도대로 갈린다**

| 히트 | 확신도 | 팝업 렌더 문면 | 버튼 |
|---|---|---|---|
| `koscom-area` | `estimated` | `코스콤지부 구역 — 무대 2 남쪽, 코리아나호텔(폴바셋) 앞입니다` + **`표시 위치는 약 ±30 m 근사입니다.`** | `거리뷰 보기` · `닫기` |
| `toilet-north` | `estimated` | `임시화장실 — 무대 1 과 무대 2 사이 서쪽입니다` + **±30 m** | 〃 |
| `toilet-south` | `estimated` | `임시화장실 — 무대 3 과 무대 4 사이 서쪽입니다` + **±30 m** | 〃 |
| `gwanghwamun` | `calculated` | `광화문역(5호선) — 무대 1 북쪽입니다` · **근사 줄 없음** | 〃 |
| `cityhall` | `verified` | `시청역(1·2호선) — 무대 4 남쪽입니다` · **근사 줄 없음** | 〃 |

- ⭕ **§58-3 준수** — 역 2개에 근사 줄이 **붙지 않는다**(붙으면 `verified`·`calculated` 를 근사라 말하는 거짓이 된다).
- ⭕ 버튼 문면 **`거리뷰 보기`** — 8/28 의 `로드뷰 보기` 복제 **0**(§55-4).
- 팝업에 `aria-label`·`title`·`alt` **0개** — 문면이 전부 실제 텍스트다.

### §68-3. 거리뷰 시트 문면 — 5/5

| 항목 | 렌더 실측 |
|---|---|
| 제목 | **`거리뷰`** (그것만 · M-21 확정 문면) |
| 촬영 | `촬영 2026.04`(4곳) · **`촬영 2025.05`**(광화문역) — 지점마다 다르다 |
| `sr-only` | **`거리뷰는 시각 자료입니다. 위치 안내는 지도 아래 범례를 참고해 주세요.`** — M-21 축자 일치. ★ 8/28 의 *«페이지 본문 텍스트를 참고»* 를 그대로 옮기지 않았다(§55 이 잡은 함정) |
| 닫기 | 접근성 이름 **`거리뷰 닫기`** |
| 컨테이너 | `role="dialog"` · `aria-labelledby="strike-streetview-title"` · **`aria-modal="false"`**(배경 스크롤 미잠금 — 설계 계약대로) |
| `APPROX_NOTE` | `estimated` 3곳 **±30 m 있음** · 역 2곳 **없음** — 팝업과 **같은 규칙** |

⚠ 시트 안 `alt=지도 확대`·`alt=지도 확대/축소 슬라이더`·`alt=지도 축소`·`alt=NAVER` 는 **네이버 파노라마 위젯이 넣은 것**이고 우리 문면이 아니다(문안 게이트 대상 아님).

### §68-4. ★ 금지·기대 문자열 — **렌더된 문자열에서** 셌다. 46항목 전부 부합

**방법**(⛔ 소스 `grep` 아님 · §5.9): `<script>`·`<style>`·`<noscript>` 제거한 `body` 복제본의 `textContent` **+ 속성 텍스트**(`aria-label`·`title`·`alt`·`placeholder`) 를 이어 붙이고, **`indexOf` 순회로 셌다**(`grep -c` 아님).
**계층 2개를 나눠 적는다**: **`base`** = 최초 렌더(팝업·시트 닫힘) · **`runtimeUnion`** = base + 팝업 5 + 시트 5 의 합집합.
...
| `무대 1` | ≥2 | **6** | ⭕ |
| `대오` | 1 | **1**(식순 `대오정비`) | ⭕ |
| `대오 1`·`대오 2`·`대오 3`·`대오 4` | 각 0 | **각 0** | ⭕ |
| `추후 안내` | 1 | **1**(QR 한 줄) | ⭕ |
| `임시화장실` | ≥2 | **4** | ⭕ |
| `간이화장실` · `시청역 화장실` | 각 0 | **각 0** | ⭕ |
| `코스콤지부 구역` | ≥2 | **4** | ⭕ |
| `코리아나호텔(폴바셋)` | ≥1 | **3** | ⭕ |
| **`천막`** | **1**(M-61 개정) | **1** | ⭕ |
| `참석명단`·`지부천막`·`명단 작성`·`물품 수령` | 각 0 | **각 0** | ⭕ |
| **`14:20`** (회귀 감시 최우선) | 0 | **0** | ⭕ |
| `15:15` | ≥1 | **2** | ⭕ |
| `윤석구`·`김동명`·`한국노총` | 각 0 | **각 0** | ⭕ |
| `동측`·`서측`·`오른쪽 차로` | 각 0 | **각 0** | ⭕ |
| `서울시의회 본관`·`광화문빌딩` | 각 0 | **각 0** | ⭕ |
| 8/28 자산 `여의도`·`국회의사당`·`집회 3구역`·`손피켓`·`상품권`·`5만원`·`인증샷`·`수기접수` | 각 0 | **각 0** | ⭕ |
| **`±25 m`** · `±25` | 각 0 | **각 0** · runtime **0** | ⭕ |
| `로드뷰` | 0 | **0** | ⭕ |
| `거리뷰` | ≥1 | **4** | ⭕ |
| `※` | 2 | **2** | ⭕ |
| A 유형 금지어 `확인 중`·`확인되는 대로`·`반영하겠습니다`·`준비 중`·`업데이트 예정` | 각 0 | **각 0** | ⭕ |

#### ★★ `±30 m` — **한 줄이 「실패」로 떴는데 실패가 아니라 내 판정 규칙이 계층을 틀린 것이다**

| 계층 | 개수 | |
|---|---|---|
| `base`(프리렌더·최초 렌더) | **0** | ⭕ **리더가 명시한 «없는 것이 정상»** — 팝업·시트에서만 그려진다 |
| `runtimeUnion` | **6** | ⭕ **정확히 예측대로**: 팝업 3(`estimated` 3개) + 시트 3(같은 3개). 역 2개는 팝업·시트 **양쪽 다 0** |

내 스크립트가 «≥1» 기대를 **일괄로 `base` 에 적용**해 이 한 줄을 실패로 냈다. **그대로 올렸으면 거짓 실패 1건**이었다.
★ 이 저장소의 **«세는 방법이 질문에 안 맞는»** 계보에 **«판정 규칙이 계층을 일괄 적용한»** 형태를 추가한다 — §5.9 가 «어느 계층이 어느 질문에 답하는가»를 규정했는데, **기대치마다 계층이 다를 수 있다**는 것까지는 안 적혀 있었다.

### §68-5. 식순 수정 회귀 — 통과

**① 원문 「묶음」 복원이 실제로 렌더된다**

| 항목 | 실측(320·360·390 동일) |
|---|---|
| `white-space` | **`pre-line`** |
| 셀 텍스트 | `"사전집회\n구호제창 / 동영상 상영/경과보고"` — **실제 `\n` 있음** |
| **줄 상자**(`Range.getClientRects()`) | 폭 **38.9** · **0** · **142.4** → **가시 줄 2개** |
| 셀 높이 | **52.8 px** (다른 행 35.4~35.9) → **+17 px** |

⚠ **개발자 값(«줄 상자 2개»)과 내 값(«3개»)은 모순이 아니다** — `\n` 자체가 **폭 0 짜리 rect** 를 만든다. **폭 0 을 거르면 2개**로 같다. **같은 이름(«줄 상자»)에 필터가 다른 것**이라 값으로 갈라 적는다(§5.7).

**② 나머지 19행 미변경 — 프리렌더 대조**
변경 «전» 산출물(내가 §67 시작 시 저장해 둔 프리렌더)과 `<tr>` 단위로 전수 비교:
```
BEFORE 21행 · AFTER 21행 · 다른 행 1개(row 2) · 나머지 20행 완전 일치
```
(21 = 헤더 1 + 데이터 20.) **행 순서·열 수·다른 칸 문자열 전부 불변.**

**③ 200% 가로 — 3폭 재측정. 초과 0**

| 폭 | 기본 초과 | **200% 초과** | 200% 셀 줄 수 | 200% 표 폭 |
|---|---|---|---|---|
| 320 | 0 | **0** | 9 | 176 < 320 |
| 360 | 0 | **0** | 6 | 216 < 360 |
| 390 | 0 | **0** | 5 | 246 < 390 |

★ **표가 스크롤 컨테이너 안이 아닌데도**(`overflow-x` 조상 없음) **표 자체가 뷰포트를 안 넘는다** — 「가로 스크롤 0」이 **회피가 아니라 실제 폭**으로 성립한다.

**④ 세로 예산 — 첫 화면 판정선 영향 0**
표 상단이 문서 기준 **1,817.7 px**(360) — 첫 화면 **640 px** 보다 **1,177 px 아래**다. **+17 px 은 첫 화면에 닿지 않는다.**

### §68-6. 관찰 2건

| # | |
|---|---|
| **O5** | 지도 상자 안에 네이버 저작권 확장 링크 **`더보기`(`<a>` 29×19)** 가 **`320` 폭에서만** 나타난다(다른 폭은 `© NAVER Corp.` 만). **탭 정지점이 하나 늘어난다.** 네이버 위젯 기본 UI라 우리 결함은 아니지만, **폭에 따라 탭 순서 길이가 달라진다**는 사실은 남긴다 |
| **O6** | 시트가 `aria-modal="false"` 다. **설계 계약대로**(배경 스크롤 미잠금)이나, 시트가 열린 상태에서 탭이 **배경 요소로 나갈 수 있다**. §55-7 이 «거리뷰는 순수 시각 보조 · 정보는 지도와 범례가 진다»로 판정한 것과 정합하므로 **결함으로 올리지 않는다** |

### §68-7. 이번 회차에 내가 걸린 것

**① 내가 만든 판정 규칙이 «계층»을 일괄 적용했다**(§68-4). `±30 m` 를 거짓 실패로 낼 뻔했다. **리더가 브리핑에 «프리렌더에는 없는 것이 정상»이라고 미리 적어 준 덕에** 결과를 보고 규칙을 의심할 수 있었다 — **그 한 줄이 없었으면 그대로 올라갔다.**

**② 「줄 상자」가 또 «같은 이름 다른 양»이었다**(§68-5 ①). 개발자 2 · 나 3. **폭 0 rect 를 세느냐**가 갈랐다. 값을 나란히 적고 필터를 명시했다.

---

## §69 QA (2026-09-01 · 런타임 문면 **정본 바이트 대조**) — §68 을 «참값 출처» 기준으로 다시 닫는다

> §68 은 기대 **«개수»** 로만 셌다. 리더 지시대로 **참값을 «검증 승인 문면»에서 가져와 «바이트»로** 대조했다.
> ⛔ **개발자가 보낸 문면 자료는 대조 기준으로 쓰지 않았다** — 조작 경로(어느 마커를 눌러야 어느 시트가 뜨는지)에만 썼다.

**동결(측정 전·후 동일)** 소스 5개 `78eddeb0`/`e18d0f14`/`91db83c1`/`01de8082`/`e7466e11` · 프리렌더 `2c4be85b`(18,410자) · **지도 청크 `79b75364b6da`·`b7c91f5a0b05`**
**판정** 통과 30 · **불일치 0** · 미검증 **1** · 관찰 1

### §69-0. 참값을 «어디서» 가져왔나 — 내가 타이핑하지 않았다

문서에서 **프로그램으로 추출**했다(전사 오류 차단):

| 대상 | 참값 출처 | 추출 방법 |
|---|---|---|
| 범례 9행 · 어포던스 문장 | `MAP-PLAN` **M-57** | 절 아래 첫째·둘째 코드펜스 |
| 코스콤 한 줄 | **M-61** | 코드펜스 |
| 지도 `sr-only` intro · 대체면 둘째 줄 | **M-62** | 코드펜스 |
| 조작 안내 · 시트 문면 5종 | **M-21** 표 | 표 행 파싱 |
| `APPROX_NOTE` | 검증 **§63-4** | 해당 행 |
| 마커 접근성 이름 | **규약** = `sr-only` 가 `feature.legend` 전문 | 범례 9행에서 index 매핑 |

### §69-1. 범례 9행 — **순서·문자열 9/9 바이트 일치**

렌더된 `figcaption > li`(장식 `aria-hidden` span 제외)를 **순서 그대로** 뽑아 M-57 과 대조 — **9행 전부 일치.** 행 순서도 같다(마커 `zIndex` 가 범례 행 순서를 따르므로 순서가 계약이다).

### §69-2. 단일 문면 5종 — **전부 바이트 일치**

| 자리 | 정본(문서에서 추출) | 렌더 |
|---|---|---|
| 범례 키 | `점선 = 근사 · 옅은 면 = 범위 근사` | ⭕ |
| 어포던스 | `지도의 코스콤지부 구역·화장실·역을 누르면 거리뷰가 열립니다.` | ⭕ |
| 조작 안내 | `지도는 손가락 하나로 움직입니다. 페이지를 내릴 때는 지도 위나 아래 빈 곳을 쓸어 주세요.` | ⭕ |
| 코스콤 한 줄 | `코스콤지부는 무대 2 구역입니다. 천막과 집회 모두 코리아나호텔(폴바셋) 앞입니다.` | ⭕ |
| 지도 `sr-only` | `아래는 세종대로 집회 장소를 표시한 지도입니다. 같은 내용을 지도 아래 범례에서 텍스트로 제공합니다.` | ⭕ |

### §69-3. 마커 접근성 이름 — **5/5 가 범례 「전문」과 바이트 일치**

`ariaSnapshot()`(계산된 접근성 이름)에서 읽어 범례 행과 **문자열 동등** 비교:
...
★ **화장실 둘이 «완전히 다른 이름»** 이다(`무대 1 과 무대 2 사이` / `무대 3 과 무대 4 사이`). 짧은 이름이었으면 스크린리더 사용자가 **둘을 구별할 수 없다** — 규약이 지켜졌다.
⭕ `aria-label`·`title` **전부 null**(내부 텍스트가 이름을 진다 · `union-webapp-dev §8`).

### §69-4. `APPROX_NOTE` — **계층·대상 둘 다 정확**

정본(§63-4) **`표시 위치는 약 ±30 m 근사입니다.`**

| 계층 | 값 | 판정 |
|---|---|---|
| 프리렌더·최초 렌더 | **0** | ⭕ **«없는 것이 정상»**(팝업·시트에서만 그린다) |
| 팝업 | `estimated` 3개 **있음** · `calculated`(광화문역)·`verified`(시청역) **없음** | ⭕ M-70 매핑대로 |
| 시트 | 〃 동일 | ⭕ 팝업과 **같은 규칙** |
| **`±25`** | **모든 계층 0** | ⭕ v1 잔재 없음 |

### §69-5. 거리뷰 시트 상태 문면 — **4종 확인 · 1종 미검증**

| 상태 | 정본 | 결과 |
|---|---|---|
| 제목 | `거리뷰` | ⭕ `#strike-streetview-title` 텍스트가 **정확히** 이것 |
| `sr-only` | `거리뷰는 시각 자료입니다. 위치 안내는 지도 아래 범례를 참고해 주세요.` | ⭕ 5/5 시트에서 바이트 일치 |
| **로딩** | `거리뷰를 불러오는 중입니다.` | ⭕ **관측** — 시트를 60 ms 간격으로 40회 표본해 첫 프레임에서 잡았다 |
| **실패** | `거리뷰를 열지 못했습니다. 잠시 후 다시 눌러 주세요.` | ⭕ **관측** — `naver.maps.Panorama` 를 throw 하도록 바꿔 강제. ★ **그 상태에서도 페이지가 온전**(`main` 1 · `h2` 4 · 본문 1,598자) — 파노라마 실패가 트리를 죽이지 않는다 |
| **없음** | `이 지점 주변에는 거리뷰가 없습니다. 지도에 표시된 길 위의 다른 지점을 눌러 주세요.` | ⚠ **미검증** — §69-6 |

### §69-6. ⚠⚠ **거짓 결함을 낼 뻔했다 — 같은 항목에서 방법 오류 3연속**

「없음」 문면을 재현하려고 거리뷰 모드로 지도를 눌렀는데 **«눌러도 아무 일이 없는 지점»** 이 나왔다.
*«거리뷰 모드에서 눌렀는데 시트도 안 뜨고 안내도 없다»* 로 **결함 보고 직전까지 갔다.** 셋 다 내 방법이었다:

| # | 관측 | 참 원인 |
|---|---|---|
| **①** | 첫 클릭만 열리고 2~5번은 침묵 | ★ **시트를 닫으면 거리뷰 모드가 «꺼진다»**(`aria-pressed` `true → false` 실측). 2~5번은 **모드가 꺼진 상태**였다 |
| **②** | 모드를 매번 켜도 6곳 중 4곳 침묵 | — |
| **③** | 재시도 중 **페이지가 `/` 로 이동** | ★★ **내 클릭이 고정 헤더(높이 68 px)의 로고 링크에 맞았다.** ②의 침묵도 같은 원인이었다 |
| **결정** | 헤더를 피해 상자 하단부 8곳을 클릭 | ⭕ **8/8 전부 시트가 열린다. 침묵 0** |

★ **결론: «침묵»은 결함이 아니다.** 그리고 **세종대로 일대는 8/8 커버리지가 있어 「없음」 상태를 만들 수 없었다** → **미검증.** ⛔ **「통과」로 적지 않는다.**
★ 셋 다 **§5.8.4 계열**이다 — ①은 «상태가 바뀐 줄 몰랐다» · ③은 «다른 요소가 그 좌표를 덮고 있었다». **`elementFromPoint` 가 `IMG` 를 돌려준 것이 나를 안심시켰는데**, 그 값은 **스크롤 «전»** 에 읽은 것이라 클릭 시점의 참값이 아니었다(§67-6 과 **같은 «언제 쟀나»** 오류).

### §69-7. 관찰 1건

| # | |
|---|---|
| **O7** | **거리뷰 시트를 닫으면 거리뷰 모드가 함께 꺼진다**(`aria-pressed` `true → false`). 시트가 열려 있는 «동안»에는 `true` 로 유지된다. 연속으로 여러 지점을 보려면 **매번 모드를 다시 켜야 한다.** 버튼 문면이 `거리뷰 모드 켜기/끄기` 라 «모드»라는 말이 지속을 함의하므로 **의도 확인이 필요**하다. **결함으로 올리지 않는다**(정보 손실 0 · 기능 손실 0) — **디자이너·리더 판단 사항** |

### §69-8. 이번 회차에 내가 걸린 것

**① 참값을 «타이핑하지 않은» 것이 이번 라운드의 핵심 안전장치였다.** 범례 9행·문면 5종을 손으로 옮겼으면 **가운뎃점·공백·전각 하나로 «불일치»가 났을 것**이고, 그것은 **렌더가 아니라 내 전사가 틀린 «거짓 실패»** 였다. 문서에서 코드펜스·표를 **파싱해서** 가져왔다.

**② 같은 항목에서 방법 오류를 3번 냈다**(§69-6). 이 저장소의 «세는 방법» 계보가 아니라 **«조작 환경» 계보**다 — 세는 것은 맞았는데 **내가 만든 상태가 내가 믿은 상태와 달랐다.** ★ **«전부 실패»가 아니라 «절반만 실패»여서 더 위험했다** — 완전한 실패였으면 §67-6 의 교훈대로 방법을 먼저 의심했을 텐데, **일부가 성공하니 «지점에 따라 다르다»는 그럴듯한 설명이 먼저 떠올랐다.**

---

## §70 QA (2026-09-01 · **「주석이 선언한 기대」 ↔ 「렌더 참값」** 대조) — 리더 회부 신설 항목

**판정** 선언 자리 **9곳 · 문자열 주장 21개** · 어긋남 **0**(측정 시점 기준) · 관찰 1

### §70-0. ★★ 이 항목만은 «지도 청크 해시»로 동결할 수 없다 — 새 사실

| | 측정 «중» 변화 |
|---|---|
| `src/lib/strikeMap.ts` | **`78eddeb0` → `ced9d729`** (바뀜) |
| `src/components/strike/StrikeRoadviewSheet.tsx` | **`01de8082` → `a539f449`** (바뀜) |
| **지도 청크** `2x30s49ubaouq.js` · `1fjlrv_zz7ye2.js` | **`79b75364b6da` · `b7c91f5a0b05` — 바이트 동일** |
| `page.tsx` | `e7466e11` 불변 |

★★★ **개발자가 제안하고 내가 §68 에서 채택한 «지도 청크 해시 고정»이 이 항목에는 «맞지 않는다».**
그 기준이 성립하는 이유는 **«주석은 번들에 도달하지 않는다»** 인데, **이 항목의 검사 대상이 바로 그 주석**이다.
→ **청크가 동일해도 선언은 바뀐다.** 실제로 바뀌었다.
→ **분리해서 동결한다: «렌더 쪽»은 청크 해시로 · «선언 쪽»은 소스 해시로.**
★ 이 저장소의 «세는 방법이 질문에 안 맞는» 계보에 **«동결 기준이 질문에 안 맞는»** 형태가 추가된다.
그래서 **렌더 값은 청크 동일로 유효**하고, **선언은 현재본을 다시 읽어** 대조했다.

**§70 동결(선언 쪽)**: `strikeMap.ts ced9d729` · `StrikeRoadviewSheet.tsx a539f449` · `page.tsx e7466e11` — 재측정 전·후 동일
**§70 동결(렌더 쪽)**: 청크 `79b75364b6da` · `b7c91f5a0b05` — 회차 내내 동일

### §70-1. ★ 리더가 지목한 `strikeMap.ts:443` — **내가 도착했을 때 이미 고쳐져 있었다**

리더 인용(변경 «전»):
```
443  기대 개수 «천막» 0 · «참석명단» 0 · «지부천막» 0
```
현재본(`ced9d729` · 행 번호가 **443 → 445** 로 밀렸다):
```
445  ⚠ **기대는 «0 이 아니라 렌더 1»**(출처 `KOSCOM_LOCATION_NOTE`). **0 으로 되돌리면 «사용자 축자 문면»을 지운다.**
```
→ ⭕ **자기모순 해소됨.** `page.tsx:71` 과 뜻이 같아졌다.
⚠ **행 번호가 이동했다** — `443` 을 인용하면 앞으로 **다른 줄을 가리킨다.** `445` 로 인용하라.

### §70-2. 렌더 참값 — **`천막` 은 «1» 이고 출처가 한 곳뿐이다**

**방법**: `<script>`·`<style>`·`<noscript>` 제거한 `body` 복제본의 `textContent` + 속성 텍스트에서 `indexOf` 순회.
**계층**: **프리렌더·최초 렌더(`base`)**.

| 문자열 | 렌더 | 출처 |
|---|---|---|
| **`천막`** | **1** | ★ **DOM 전체에서 `천막` 을 «자기 직접 텍스트»로 가진 요소가 단 하나** — `<p>코스콤지부는 무대 2 구역입니다. 천막과 집회 모두 코리아나호텔(폴바셋) 앞입니다.</p>` = **`KOSCOM_LOCATION_NOTE`**. 리더가 요청한 «그 1건이 거기서 나오는지»까지 확인 |
| `참석명단` | **0** | ⭕ 유지 |
| `지부천막` | **0** | ⭕ 유지 — ⚠ **`천막` 과 한 덩어리로 읽지 않았다.** 8/28 낱말이고 9/4 근거가 실제로 0 이다 |

⚠ **합집합 계층에서 `천막` 이 11 로 나온다** — 이는 «11곳에 있다»가 **아니다.** 내 합집합은 **스냅샷 11장**(base 1 + 팝업 5 + 시트 5)을 이어 붙인 것이고 각 스냅샷이 base 전체를 포함한다. **참값은 `base` 의 1 이다.** (§5.1 «어느 계층이 어느 질문에 답하는가».)

### §70-3. 선언 전수 대조 — **9곳 · 21개 주장 · 어긋남 0**

| 선언 자리 | 주장 | 렌더 | |
|---|---|---|---|
| `strikeMap.ts:33` | `①`~`⑥` 각 **0** | 각 0 | ⭕ (6건) |
| `strikeMap.ts:192` | `참석명단`·`물품 수령`·`명단 작성`·`지부천막` 각 **0** | 각 0 | ⭕ (4건) |
| `strikeMap.ts:258` | `동측`·`서측`·`오른쪽 차로` 각 **0** | 각 0 | ⭕ (3건) |
| `strikeMap.ts:271` | `시청역 화장실` **0** | 0 | ⭕ |
| **`strikeMap.ts:445`** | **`천막` 렌더 1** | **1** | ⭕ **(고쳐진 뒤 일치)** |
| `strikeMap.ts:448` | `6,000명`·`3,000명`·`4,000명` 각 **0** | 각 0 | ⭕ (3건) |
| `strikeMap.ts:460` | `메인무대` **0** | 0 | ⭕ |
| `page.tsx:62` | `대오` **1** | 1(식순 `대오정비`) | ⭕ |
| `page.tsx:71` | `천막` **렌더 1** | 1 | ⭕ |

⚠ **세는 «단위»가 검증자와 다르다**(§5.7 «값 + 방법 + 기준»): 검증자는 **«11곳»**, 나는 **«선언 자리 9곳 / 문자열 주장 21개»** 다. **나는 «`기대` 라는 낱말이 든 주석 줄» 중 실제로 개수를 선언한 것만 자리로 셌고**(런타임 가드 메시지 `기대: ${expected}` 와 «기대던» 같은 다른 뜻은 제외), **한 줄이 여러 문자열을 선언하면 주장으로 쪼갰다.** **모순이 아니라 단위가 다르다.**

### §70-4. 관찰 1건

| # | |
|---|---|
| **O8** | **같은 사실이 두 파일에 선언돼 있다** — `천막` 기대가 `strikeMap.ts:445` 와 `page.tsx:71` 두 곳에 있다. 이번 사고가 **정확히 그 구조 때문**이었다(한쪽만 갱신돼 226줄 떨어진 자기모순이 됐다). 지금은 둘 다 맞지만 **다음에 또 갈라질 수 있다.** 「기대 개수」를 소스 주석 여러 곳에 두는 방식 자체를 리더가 판정할 사안으로 올린다. ⚠ **«그러니 주석을 지워라»로 읽지 마라** — 이 저장소는 **주석에 금지어를 적는 것이 설계**다(§5.9). 문제는 «적는 것»이 아니라 **«두 벌»** 이다 |

### §70-5. 이번 회차에 내가 걸린 것

**★ 내가 채택한 동결 기준이 이 항목에 안 맞았다**(§70-0). §68 에서 개발자 제안(«지도 청크 해시»)을 채택했고 **그 자체는 옳다** — 다만 그것이 성립하는 근거가 «주석은 번들에 안 들어간다» 인데, **이번 항목의 검사 대상이 그 주석**이었다. **기준을 만든 근거를 그대로 뒤집어 보면 적용 범위가 나온다** — 그것을 안 하고 넘어갈 뻔했다.

---

## §71 기록 갱신 (2026-09-01 · QA 회차 종료 후 · **재측정 없음**)

리더 판정(§69 접수 · 회차 종료)과 개발자 최종 저장 통보를 받아 **기록만 갱신한다.** 새 측정은 하지 않았다.

### §71-1. 최종 동결 해시 — **개발자 통보와 «한 줄» 다르다**

| 파일 | 개발자 통보 | **실측(내가 잰 값)** | |
|---|---|---|---|
| `src/lib/strikeMap.ts` | `ced9d729` | **`ced9d729`** | ⭕ |
| `src/lib/naverMaps.ts` | 불변 `e18d0f14` | **`e18d0f14`** | ⭕ |
| **`src/components/strike/StrikeMap.tsx`** | **«불변»** (`91db83c1`) | ⚠ **`e28cd7d4`** — **바뀌었다** | ❌ 통보가 낡았다 |
| `src/components/strike/StrikeRoadviewSheet.tsx` | `a539f449` | **`a539f449`** | ⭕ |
| `src/app/strike-2026-09-04/page.tsx` | 불변 `e7466e11` | **`e7466e11`** | ⭕ |
| 프리렌더 가시 마크업 | 18,410자 `2c4be85b` | **동일** | ⭕ |

`StrikeMap.tsx` 가 바뀐 «내용»은 확인했다 — **`closeStreetView` 주석**이고 **내 O7 보고를 반영한 것**이다(`QA O7 · 후속 #32` 를 인용한다). 통보 시점보다 저장이 늦었을 뿐이고 **렌더에는 영향이 없다**(아래 §71-2). **결함이 아니라 «통보가 실측보다 낡았다»는 기록**이다.

★ **그래서 통보받은 해시를 그대로 옮겨 적지 않고 다시 쟀다.** 그대로 적었으면 문서가 **존재하지 않는 해시**를 최종본으로 가리켰을 것이다.

### §71-2. ★★ 동결 기준을 «검증»했다 — 청크 2개 중 **9/4 지도는 하나뿐**이다

§68 에서 채택한 «지도 청크 해시» 기준이 **정말 지도 코드를 담고 있는지**를 이번에 처음 확인했다(그전까지는 개발자 주장을 받아들인 상태였다).

**방법**: `.next/static/chunks/*.js` 에서 런타임 특징 문자열의 **존재 여부**를 본다.
⚠ **`grep -c` 를 «개수»로 읽지 않았다** — 미니파이 산출물은 한 줄이라 무엇을 세든 1 이다(§5.1). **존재 판별에만** 썼다.

| 청크 | 크기 | `data-strike-hit` | `126.977135`(코스콤 경도) | `±30 m` | `rally` | `국회의사당` | **정체** |
|---|---|---|---|---|---|---|---|
| **`1fjlrv_zz7ye2.js`** `b7c91f5a0b05` | 30,427 B | **있음** | **있음** | **있음** | 없음 | 없음 | ★ **9/4 지도 청크** |
| `2x30s49ubaouq.js` `79b75364b6da` | 52,734 B | **없음** | 없음 | 없음 | 있음 | 있음 | **8/28 `RallyMap` 청크** |

★★ **개발자가 «지도 청크 2개»로 준 것 중 9/4 지도를 담은 것은 `b7c91f5a0b05` «하나»다.** 다른 하나는 **8/28 지도**다.
→ **기준 자체는 유효하다**(둘을 함께 보면 9/4·8/28 양쪽을 덮는다). **틀린 것은 «둘 다 9/4»라는 이해**다.
⚠ **다음 사람이 `79b75364` 만 보고 «9/4 지도 안 바뀌었네» 로 읽으면 틀린다.** 9/4 판정에 쓸 값은 **`b7c91f5a0b05` 하나**다.
⭕ 그 값은 §67 부터 지금까지 **한 번도 바뀌지 않았다** — **A~E · 런타임 문면 계층 전부 유효**하다.

### §71-3. 스모크 (측정 아님)

`/strike-2026-09-04` **200** · `/rally-2026-08-28` **200** · 프리렌더 불변. 개발자 최종 저장이 빌드를 깨지 않았다.

### §71-4. 판정 접수 — 기록

| 항목 | 판정 | 자리 |
|---|---|---|
| **O7**(시트 닫으면 거리뷰 모드 함께 꺼짐) | ⭕ **현행 유지 · 결함 아님.** 근거는 **«모드가 남으면 «팝업을 열려던 다음 탭»이 거리뷰를 연다»** 이고, 이 지도의 1차 목적은 위치 안내다. 문면(「모드」가 지속을 함의) 지적은 **유효 → 후속 #32** | 리더 |
| 〃 소스 사유 | ⚠ **개발자가 종전 사유를 스스로 폐기했다** — *«길만 남으면 눌러도 열 것이 없다»* 는 **거짓**이고, 주석에 그렇게 적혔다. ★ **그 거짓을 드러낸 것이 §69-6 의 실측**이다(모드를 켠 채 8곳을 누르니 **8/8 전부 시트가 열렸다**). **가설이 아니라 값이 사유를 갈랐다** | `StrikeMap.tsx:1021~1023` |
| 시트 **「없음」** 문면 | ⚠ **미검증 유지.** 세종대로 일대가 8/8 커버리지라 그 상태를 만들 수 없다. ⛔ **억지로 만들지 않는다** | 리더 |
| **O8**(`천막` 기대가 두 파일에 선언) | 리더 판정 대기 | §70-4 |

### §71-5. 이번 갱신에서 배운 것

**★ «기록만 갱신하라»는 지시에도 «값은 다시 재야» 한다.** 통보받은 해시 5개 중 **1개가 이미 낡아 있었다**(`StrikeMap.tsx`). 그대로 옮겨 적었으면 **최종 동결 기록이 존재하지 않는 해시를 가리켰을 것**이고, 다음 사람이 그 값으로 대조하면 **«누가 몰래 고쳤다»는 거짓 경보**가 난다.
★ 그리고 **동결 기준을 채택한 지 세 회차 만에 처음 «그 기준이 대상을 담고 있는지»를 확인했다** — 담고 있었지만 **둘 중 하나뿐**이었다. **§70-0(«동결 기준이 질문에 안 맞는»)의 짝**이다: 그때는 «기준이 대상을 «과하게» 덮지 못했고», 이번은 «기준의 절반이 «다른 대상»을 덮고 있었다».

---

## §72 QA (2026-09-01 · v3 — 조합원 상세일정 스택 + 원본 사진 3장 + 섹션 개명)

**판정** 통과 68 · **실패 1**(전송량) · **문서 정정 필요 8**(렌더 정상) · 관찰 3 · 미검증 0

### §72-0. 동결 · 환경

| 항목 | 값 |
|---|---|
| 소스 | `page.tsx 0cb371cd` · `strikeMap.ts 287fc087` · `StrikeMap.tsx d1bd0465` · `StrikeRoadviewSheet.tsx a539f449` |
| **9/4 지도 청크** | `0_0578ygrkjr0.js` **`93339a3c1cde` · 30,598 B** |
| `BUILD_ID` | 측정 중 `KAkGmlEF6qZqAQtyc0sCl` → `S1TMKDybjy2xvzPa9UtPn`(dev 재컴파일) |
| 서버 | **PID 57018 → 69793 재기동됨**(내가 죽인 것이 아니다) |
| 스크롤바 | **전 폭 0**(`innerWidth − clientWidth`) — 리더가 우려한 0↔15 흔들림 **없었다** |
| 정리 | 200% 블록 7회 전부 종료 시 루트 `12px` / 인라인 `none` 을 값으로 확인 |

#### ⚠⚠ 리더 전제 정정 — **「지도 청크 무변경」이 아니다**

| | §71 | **§72** |
|---|---|---|
| 9/4 지도 청크 | `1fjlrv_zz7ye2.js` **`b7c91f5a0b05`** 30,427 B | `0_0578ygrkjr0.js` **`93339a3c1cde`** 30,598 B |

**파일명도 해시도 크기도 바뀌었다(+171 B).** 주석만이면 미니파이 후 바이트 동일이어야 하므로 **지도 런타임 코드가 실제로 바뀌었다**(`TOILET_NOTE` 신설이 유력하나 내용 귀속은 안 했다).
→ **「A~E 재측정 불필요」의 «전제»가 성립하지 않는다.** 다만 이번 라운드 변경은 **범례 아래 문장 추가**로 보이고 지도 기하·조작을 건드리는 종류가 아니라, **전량 재측정 대신 리더 판단으로 넘긴다.** ⛔ **「청크 무변경이니 괜찮다」로 적지 않았다.**
⭕ **8/28 지도 청크 `2x30s49ubaouq.js` = `79b75364b6da` 로 완전 동일** — 8/28 회귀 위험 0.

### §72-1. ★ 실패 1건 — **모바일 이미지가 `w=1080` 이 아니라 `w=1200` 을 받는다**

**참값(dpr 3 · 390 뷰포트 · 실제 브라우저 요청 · `Accept: image/webp`)**

| 이미지 | 요청 URL | 응답 | **수신** |
|---|---|---|---|
| 지도 | `…location-map.png&w=1200&q=90` | 200 `image/webp` | **203.2 KB** |
| 로드뷰 | `…location-roadview.jpg&w=1200&q=90` | 200 `image/webp` | **201.6 KB** |
| 배치도 | `/images/…/branch-layout.png`(옵티마이저 미경유) | 200 `image/png` | **46.3 KB** |
| | | **첫 방문 합계** | **451.1 KB** |

**기준선 ≈390 KB 대비 +61.1 KB (+15.7%)**

**원인 — 값으로 갈랐다**

| | 선언 | 실제 |
|---|---|---|
| `sizes` | `(min-width: 768px) 624px, 100vw` ⭕ **스펙대로** | — |
| 모바일 슬롯 | `100vw` = **390 css** | ★ **실측 336 css**(390 − 페이지 패딩 24 − 패널 패딩 30) |
| dpr 3 요청 폭 | 390 × 3 = **1170** → `deviceSizes` 다음 값 **1200** | — |

**`w` 별 실측(WebP · q=90)**

| w | 지도 | 로드뷰 | +배치도 46.3 = 합계 |
|---|---|---|---|
| 828 | 124.2 | 102.8 | **273.3 KB** |
| **1080** | **178.7** | **166.4** | **391.4 KB** ← ★ **리더 예보 178·166 과 완전 일치. 기준선 390 에 맞는다** |
| **1200(현행)** | **203.2** | **201.6** | **451.1 KB** |

→ ★ **리더 예보는 «w=1080 일 때» 정확했다. 틀린 것은 예보가 아니라 «어느 `w` 가 선택되는가» 다.**
→ **처방 방향**(내가 정하지 않는다 · 디자이너·리더 판정): `sizes` 모바일 항을 실제 슬롯으로 좁히면 1008 → **`w=1080`** 이 선택된다. 슬롯 모델은 **`100vw − 54px`** 로 3폭 전부 설명된다(320→266 · 360→306 · 390→336 · 전부 실측 일치).
⚠ **`sizes` 값 자체는 스펙(`V3-IMAGES`)대로 구현돼 있다** — **구현 이탈이 아니라 스펙의 모바일 항이 슬롯을 반영하지 않은 것**이다. 귀속을 갈라 적는다.

**부수 확인(전부 리더 값과 일치)**: `q=92` → **400**(두 파일 다) · `w=1280` → **400** · 소스 지도 PNG **897.7 KB** · 로드뷰 JPEG **456 KB** · 배치도 `unoptimized` **46.3 KB**(옵티마이저 경로를 안 탄다).
⚠ **내가 한 번 틀렸다**: 첫 프로브에서 파일명을 `branch-map.png`·`branch-roadview.jpg` 로 **추측**해 8건 전부 400 이 나왔다. 참 파일명은 **`location-map.png`·`location-roadview.jpg`** 다. 그대로 올렸으면 **「q=90 도 400 이다」는 거짓 실패**였다. 그리고 두 번째 프로브는 **`Accept` 헤더가 없어 PNG/JPEG 폴백**을 쟀다 — **조합원이 받는 것은 WebP** 다. **두 번 다 «대상이 질문에 안 맞았다».**

### §72-2. ★ 200% 텍스트 확대 — **최우선 항목. 6조합 전부 통과**

| 폭 | `clientWidth` | 스크롤바 | 기본 초과 | **200% 초과** | 200% 문서 높이 |
|---|---|---|---|---|---|
| 320 | 320 | 0 | **0** | **0** | 19,309 |
| 360 | 360 | 0 | **0** | **0** | 17,615 |
| 390 | 390 | 0 | **0** | **0** | 17,422 |

- **참값** `documentElement.scrollWidth` vs `clientWidth`. **클리핑 조상이 없는 넘침 요소 = 6조합 전부 0개.**
- 리더가 지목한 새 위험원 셋 모두 통과: ① `<dl>` 스택 6행 ② **사진 3장**(`w-full h-auto` 가 문서를 안 민다) ③ 새 헤딩 2개.
- **범인 격리 불필요** — 초과가 0 이라 가를 대상이 없다.
- 최종 해시(`0cb371cd`)에서 **재확인**했다(alt 확정본 적용 후에도 동일).

### §72-3. 세로 예산 — 첫 화면 통과

| 항목 | 값 |
|---|---|
| **`10시 30분`** rect(360×640) | **top 304.5 · bottom 334.5** → ⭕ **첫 화면에 «완전히» 보인다**(여유 305.5px) |
| 문서 전체 높이(360 기본) | **5,402 px** (320: 5,280 · 390: 5,553) |
| 배치도 실측 | 733×2083 원본 · 슬롯 **336** → **951 px**(리더 예보 955 · 오차 4px) |

### §72-4. ★ 속성 계층 — 갈라 셌다

| | 가시 | 속성 | 합 |
|---|---|---|---|
| **`임시화장실`** | **3** | **1** | **4** | ← ★ 리더 보고와 **완전 일치** |
| `인근화장실` | **1** | 0 | 1 | ★ **M-53 화장실 줄이 «되찾아졌다»**(V3-IMAGES 가 «렌더 0» 이라 적었던 상태 해소) |
| `스벅` | **1** | 0 | 1 | 〃 (펴지 않았다) |

**속성 계층 전량 7개**(프리렌더 · `alt`·`aria-label`·`title`·`placeholder`): 빈 `alt` 1(장식) · `글자 크기` · **확정 `alt` 3개** · `전국금융산업노동조합` · `코스콤`.
**속성 계층 금지어: 33종 검사 → 출현 0건.** ⭕ 개발자 보고(23종 0건)를 **독립 재현**했고 10종을 더 넣어도 0 이다.

### §72-5. ★★ 기대표 대조 — **불일치 8건. 전부 «렌더가 아니라 문서»가 낡았다**

**계층·방법**: 프리렌더 → `<script>`/`<style>` 제거 → 태그 제거 → `find` 순회(§V3-PLAN §0 선언 방법과 같음).
**검사 58 주장 · 일치 50 · 불일치 8.** ⚠ **8건이 전부 «정확히 +1»** 이라 계통 원인을 먼저 의심했고, **출현 자리를 직접 열어** 갈랐다.

| 낱말 | 표 기대 | 렌더 | §7-A 두 문장 | 사진 캡션 | **나머지** | |
|---|---|---|---|---|---|---|
| `천막` | 3 | 4 | 1 | 0 | **3** | ⭕ 표와 일치 |
| `참석명단` | 1 | 2 | 1 | 0 | **1** | ⭕ |
| `명단 작성` | 2 | 3 | 1 | 0 | **2** | ⭕ |
...
★★ **초과분의 출처가 «승인된 것»임을 바이트로 확인했다** — 내가 타이핑하지 않고 검증 문서에서 옮겼다:
- **`금노 QR출석코드 인증과 시작 출석체크는 같은 것입니다. 한 번만 하면 됩니다.`** — 검증 §72-2 확정 문면 · **렌더 1 · 바이트 일치**
- **`참석명단 작성과 출석부 서명은 같은 것입니다. 천막에서 한 번만 하면 됩니다.`** — 검증 §72-3 확정 문면 · **렌더 1 · 바이트 일치**
- 사진 캡션 **`코스콤지부 구역`** — 검증 §73-6 확정 캡션 · 바이트 일치

→ **판정(리더 지시 3분류)**: 렌더가 틀린 것도, 소스가 틀린 것도, 승인 문면이 틀린 것도 **아니다.**
**`V3-PLAN §3` 기대표가 «§7-A 사용자 1급 답변이 만든 두 문장»과 «사진 3장 캡션»을 계수에 안 넣은 것**이다.
⚠ **그대로 세면 정상 문면 8건이 실패로 올라가고, 처방은 «사용자 답변이 만든 문장을 지우는 것»이 된다.** 문서를 고쳐라.

**그 밖 승인 문면 바이트 일치(가시 계층)**: `총파업 참가 조합원 상세일정` 1 · `총파업 일정` 1 · `금융노조 공식 순서` 1 · 캡션 `지부배치도` 1 · `집회 장소` 2 · `화장실 : 임시화장실 및 인근화장실(시청역, 스벅 등) 이용` 1.
**3-B 내려가는 것 달성**: `추후 안내` **0** · `인증 시각` **0** · `총파업 식순` **0** · `식순` **0**.
**`1차`·`2차` 렌더 0** — §7-A Q1 이 제목에서 「1차」를 뺀 결과이고 **계약 재작성이 필요 없다**(현행 문면에는 그 낱말이 없다).

### §72-6. 식순 표 회귀 — ★ **기준값과 «완전 일치». 단, 조건을 맞춰야 나온다**

| `clientWidth` | 200% 표 높이 | 열 | 최대 셀 줄 | 판정 |
|---|---|---|---|---|
| **345** | **4,755.0** | **78 / 123** | **5** | ⭕⭕ **세 값 모두 회귀 기준과 완전 일치** |
| 350 | 4,755.0 | 78 / 128 | 5 | |
| 355 | 4,755.0 | 78 / 133 | 5 | |
| 360 | **4,710.0** | 78 / 138 | 5 | ⚠ **기준값과 45px 다르다 — 회귀가 아니라 «조건이 다르다»** |

★ **회귀 기준 「4,755px」에는 `clientWidth 345` 가 붙어 있어야 한다.** 안 붙이면 다음 사람이 **360(스크롤바 0)에서 재고 «회귀»라고 한다.** §5.7 «값 + 방법 + 기준은 한 벌»의 사례를 하나 더 추가한다.
**행 20 · 시간 열 78px 고정 · 기본 최대 2줄 · 넘침 0** — 전부 불변.

### §72-7. `<dl>` 스택 접근성 — 통과

**상세일정 `<dl>`**: `dt` **6** · `dd` **12** · **묶음 `<div>` 6개** → 각 그룹이 `dt` 1 + `dd` 2 로 **범위가 닫힌다.**
**접근성 트리 실측**(`ariaSnapshot`):
```
- term: 08:40~10:30
- definition: 여의도 출근없이 오전 집결장소로 이동
- definition: 세종대로 코리아나호텔 앞 (광화문역 6번 출구, 시청역 3번 출구 인근)
- term: 10:30~11:00
- definition: 코스콤 천막 방문 후 집회구역 이동 - 참석명단 작성 - 투쟁용품 수령 …
```
⭕ **「시간 ↔ 내용」 짝으로 읽힌다.** 6행 전부 같은 구조.
⚠ **내 스크립트 한계 기록**: 개요 `<dl>`(묶음 `<div>` 없음)에서 `dt.parentElement` 를 범위로 써 «dt 하나에 dd 3개»로 보고했다. **개요 `<dl>` 의 결함이 아니라 내 측정 방법의 한계**다(HTML 은 순서로 짝을 정한다).

### §72-8. 8/28 스모크 — 통과

지도 렌더(네이버 타일 **20**·`window.naver.maps` 있음) · `main` 1 · `h1` 1 · 가로 스크롤 0 ·
**「2026년 8월 28일 일정이 지났습니다. 아래 안내는 기록으로 남겨 둡니다.」 문면 그대로** ·
콘솔 오류 2건 = `layout.tsx` **기존 공통 건**(9/4 와 동일).
**낱말은 페이지별로 갈라 적는다**: 8/28 렌더 `참석명단` **3** · `손피켓` **5** · `투쟁용품` **1** · `식순` **3**.
⛔ **9/4 기대값과 합산하지 마라** — 리더 지시대로 두 페이지를 섞지 않았다.

### §72-9. 관찰 3건

| # | |
|---|---|
| **O9** | ★ **`alt` 3개가 「임시」가 아니다 — 검증 §73-6 확정본과 바이트 일치한다.** 리더 브리핑의 *「alt 3개는 임시다(검증 판정 대기)」* 는 **낡았다**(측정 중 `page.tsx` 가 `cad0045e → 0cb371cd` 로 바뀌며 확정본이 적용됐다). 지시대로 문면 불일치를 결함으로 올리지 않았고, **일치한다는 사실**을 적는다 |
| **O10** | **`sizes` 의 모바일 항이 `100vw` 라 슬롯보다 16% 크다**(§72-1). 데스크톱 항(`624px`)은 슬롯 624 와 **정확히 일치**한다 — **두 항의 정밀도가 다르다** |
| **O11** | 콘솔 오류 2건(`layout.tsx` 글자 크기 선반영 `<script>` 가 `<head>` 밖)은 **양 페이지 공통 기존 건**. FOLLOWUPS 후보로 계속 남는다 |

### §72-10. 이번 회차에 내가 걸린 것

**★ 같은 항목에서 «대상» 오류를 두 번 냈다**(§72-1). ① 이미지 **파일명을 추측**해 400 8건을 얻었다(참 이름은 `location-*`) ② `Accept` 헤더 없이 재서 **WebP 가 아닌 폴백**을 쟀다. 둘 다 **숫자는 정확한데 «무엇을 쟀는지»가 질문과 달랐다.** 첫 번째를 그대로 올렸으면 **「q=90 도 400」이라는 거짓 실패**였다.
**★ «전부 +1» 을 보고 계통 원인을 먼저 의심한 것이 8건의 거짓 실패를 막았다.** 8건을 각각 «문면 오류»로 올렸으면 처방이 **«사용자 1급 답변이 만든 문장을 지우는 것»** 이 됐다.

---

## §73 QA (2026-09-01 · 섹션 순서 개정 후 **세로 축 재측정**) — §72 의 자리 의존 값을 대체한다

> ⚠ **§72 의 「세로」 값은 죽었다**(문서 높이 5,402 · 섹션 top · 첫 화면 여유). **§73 을 인용하라.**
> ⭕ **§72 의 「가로·계층·이미지·`<dl>`·8/28」은 살아 있고, 이 회차에서 더 가혹한 조건으로 재확인됐다.**

### §73-0. 이 재측정이 필요해진 사유 — **리더 요청에 따라 명시한다**

**리더가 「원본 자료」 섹션 자리 판정을 «열기 전»에 QA 를 걸었다.** 판정이 측정 도중에 나서 개발자가 섹션을 옮겼고, 자리에 의존하는 값이 통째로 낡았다.
★ **이번 폐기는 내 방법 문제가 아니다** — 직전 라운드(§67·§70)의 두 번은 «측정 중 소스가 갈렸다»였고, 이번은 **«측정 전제가 아직 확정되지 않았다»** 다. **성격이 다르다.**

### §73-1. 동결 — 재측정 전·후 동일

```
page.tsx            b43b2fa4e7f0     (0cb371cd → b43b2fa4 · 섹션 이동)
strikeMap.ts        287fc0878fd2     불변
StrikeMap.tsx       d1bd04651a20     불변
StrikeRoadviewSheet a539f4490b6d     불변
9/4 지도 청크        93339a3c1cde  0_0578ygrkjr0.js  30,598 B   ← §72 와 동일(지도는 안 건드렸다)
BUILD_ID            jgbQTauwXT3LRNlft8wDM
```
**안정 확인**: 18초 동안 3회 읽어 셋 다 동일.

### §73-2. ★ 섹션 순서 — **개정본이 이미 착지했다.** 렌더로 확인

```
집결시간 → 개요 → 세종대로 안내지도 → 총파업 일정 → 원본 자료
```
리더의 «개정» 그대로다. **두 메시지가 엇갈렸을 뿐이고, 추측 대신 렌더 `h2` 순서를 직접 읽어 확정했다.**

### §73-3. 세로 축 참값 (`360 × 640` · **`clientWidth` 345** · 스크롤바 15 · dpr 1 · 루트 12px)

| 섹션 | `id` | **top** | height |
|---|---|---|---|
| 집결시간 | `gather-heading` | **228.3** | 182.1 |
| 개요 | `overview-heading` | **464.3** | 229.9 |
| 세종대로 안내지도 | `map-heading` | **748.2** | 805.6 |
| **총파업 일정** | `program-heading` | **1,607.9** | **1,689.0** |
| **원본 자료** | `source-heading` | **3,350.9** | **1,441.3** |
| | | **문서 높이** | **5,335** |

| 대조 | 값 | |
|---|---|---|
| 개발자 예보 `program 1,610` · `source 3,353` | 실측 **1,607.9 · 3,350.9** | ⭕ **2~3px 일치** |
| 개발자 예보 문서 높이 **5,402** | 실측 **5,335** | ⚠ **−67px** — 개발자 환경표가 «스크롤바 0» 이다. **조건 차이**(내 조건은 스크롤바 15) |
| 리더 예보 「총파업 일정 시작 → 1,862」 | 실측 **1,607.9** | ⚠ **−254px.** 리더가 *「이건 예보다. 네 실측이 참값이다」* 라고 명시했다 |
...
| `집결시간` 헤딩 | 228.3 – 251.6 | ⭕ |
| `전 조합원` | 284.6 – 301.5 | ⭕ |
| **`10시 30분`** | **304.5 – 334.5** | ⭕ **완전히 보인다 · 아래 여유 305.5px** |

★ **섹션 이동이 첫 화면에 닿지 않았다** — 옮긴 것이 전부 판정선 «아래»다(개발자 예상대로이나 **이제 실측이다**).

### §73-4. ★★ 스크롤바 0 ↔ 15 흔들림이 **내 측정에서 실재했다**

| 회차 | 뷰포트 360 의 `clientWidth` | 스크롤바 |
|---|---|---|
| **§72** | **360** | **0** |
| **§73** | **345** | **15** |

**같은 도구 · 같은 뷰포트인데 갈렸다.** 리더가 브리핑에 *「뷰포트만 적지 말고 `clientWidth` 를 함께 적어라」* 고 못박은 것이 **정확히 이 상황**이다.
★ **§72 값을 버릴 필요는 없다 — 오히려 강화된다**: §72 는 관대한 조건(`cw` = 뷰포트)에서 쟀고, §73 은 **참 최악 조건**(`cw` = 뷰포트 − 15)에서 쟀는데 **둘 다 초과 0** 이다.

**§73 가로 축 재확인 — 스크롤바 15 조건 · 6조합 전부 통과**

| 뷰포트 | `clientWidth` | 기본 초과 | **200% 초과** | 클리핑 없는 넘침 요소 |
|---|---|---|---|---|
| 320 | **305** | 0 | **0** | **0** |
| 360 | **345** | 0 | **0** | **0** |
| 390 | **375** | 0 | **0** | **0** |

200% 문서 높이: 320 **20,402** · 360 **18,148** · 390 **17,563**.

### §73-5. ★ 식순 표 회귀 — **기준값 3개 전부 완전 일치. 그리고 «참 조건»이 밝혀졌다**

| 항목 | 회귀 기준 | **실측** |
|---|---|---|
| 200% 표 높이 | 4,755 px | **4,755.0** ⭕ |
| 열 폭 | 78 / 123 | **78 / 123** ⭕ |
| 최대 셀 줄 | 5 | **5** ⭕ |
| 행 | 20 | **20** ⭕ · 넘침 0 |

★★ **§72 에서 나는 이 조건을 «뷰포트 345» 로 «인위적으로» 만들었는데, 참 조건은 «뷰포트 360 + 스크롤바 15»였다.** 이번에는 **자연스럽게 재현**됐다.
→ **기준값 「4,755px」에는 반드시 `clientWidth 345` 를 병기하라.** 뷰포트만 적으면 다음 사람이 스크롤바 0 환경에서 재고 **4,710 을 얻어 «회귀»라고 한다.**

### §73-6. 헤딩 차례 — **우리 마크업은 건너뜀 0. `h5` 는 네이버 것이다**

| 계층 | 헤딩 |
|---|---|
| **프리렌더(우리 마크업)** | `h1` → `h2` → `h2` → `h2` → `h2` → **`h3` → `h3`** → `h2` — ⭕ **건너뜀 없음** |
| 런타임 DOM | 위 + **`h5` 「지도 데이터」 1개** |

**`h5` 소유자 판정 — 값으로 갈랐다**
- **프리렌더에 `<h5` 가 0개**(`h5InPrerender: false`)
- 조상 체인이 `div → div.size-full → div.rounded-card … → figure` 로 **네이버 지도 마운트 노드 «안»**(`insideNaverMount: true`)
→ ★ **네이버 위젯이 런타임에 주입한 저작권 패널 헤딩이다. 우리 마크업이 아니다.**
⚠ 그래도 **접근성 트리에는 `h2` → `h5` 3단계 건너뜀이 생긴다.** 우리가 못 고치는 자리이고 **8/28 에도 같은 위젯이 있으므로 같을 것**이다. **결함으로 올리지 않고 관찰로 남긴다**(O12).

★ **신설 `h3` 2개는 정확히 `h2`(총파업 일정) 아래 두 패널**이다 — 차례가 맞고, 두 표의 출처가 다르다는 것이 헤딩으로 갈린다.

### §73-7. 실패는 그대로 **1건** — §72-1 유지

이미지 요청은 이번에도 **`w=1200` · `q=90`**(304 캐시라 바이트는 §72 직접 프로브 값이 참값이다).
**첫 방문 451.1 KB · 기준선 390 KB 대비 +61.1 KB.** 원인·처방 방향은 §72-1 그대로.

### §73-8. 관찰 추가

| # | |
|---|---|
| **O12** | **네이버 지도 위젯이 `h5` 「지도 데이터」를 주입해 헤딩 차례가 `h2` → `h5` 로 3단계 건너뛴다.** 우리 마크업 아님(프리렌더 0). **8/28 도 같을 것** — 두 페이지 공통 · 우리가 못 고침. FOLLOWUPS 후보 |
| **O13** | **`clientWidth` 가 같은 도구·같은 뷰포트에서 회차마다 갈린다**(0 ↔ 15). **뷰포트만 적은 판정선은 재현되지 않는다** — 이 저장소의 모든 폭 판정선에 `clientWidth` 병기가 필요하다 |

### §73-9. 이번 회차 기록

**★ 두 통보가 엇갈렸을 때 «추측하지 않고 렌더를 읽었다».** 리더는 *「개발자가 옮기는 중」*, 개발자는 *「이미 다 됐고 이 값이다」* 였다. **둘 중 누가 맞는지 묻지 않고 `h2` 순서를 직접 읽어** 개정본이 착지했음을 확정한 뒤 측정을 재개했다.
**★ 「이번 폐기는 네 잘못이 아니다」를 리더가 먼저 적었고, 나는 그것을 그대로 기록했다**(§73-0). 원인 귀속을 흐리면 **다음에 같은 순서 실수가 «QA 방법 문제»로 잘못 학습된다.**

---

## §74 QA (2026-09-01 · `sizes` 수정 회귀) — **§72-1 실패 해소**
...
```
page.tsx  a880e6cdb7a1     (b43b2fa4 → a880e6cd · sizes 2곳)
strikeMap.ts 287fc087 · StrikeMap.tsx d1bd0465 · StrikeRoadviewSheet a539f449   (전부 불변)
9/4 지도 청크 93339a3c1cde  0_0578ygrkjr0.js  30,598 B                          (불변)
BUILD_ID vL93oaXQUY13k8A439pPt
소스 선언(2곳) sizes="(min-width: 768px) 624px, calc(100vw - 54px)"
```

### §74-1. ★ 전송량 — **해소. 목표값에 도달했다**

**방법**: 조건마다 **새 브라우저 컨텍스트**를 그 뷰포트·dpr 로 만들어 측정(아래 §74-2 사유) · 스크롤로 lazy 전량 로드 · `Accept: image/webp` 는 브라우저 기본값.

| 조건 | 슬롯 | 요청 `w` | 지도 | 로드뷰 | 배치도 | **총 전송** |
|---|---|---|---|---|---|---|
| **390 / dpr3** | **336** | **1080** | 178.7 | 166.4 | 46.3 | ★ **391.4 KB** |
| 360 / dpr3 | 306 | 1080 | 178.7 | 166.4 | 46.3 | **391.4 KB** |
| 320 / dpr3 | 266 | **828** | 124.2 | 102.8 | 46.3 | **273.3 KB** |
| 390 / dpr2 | 336 | **750** | 109.4 | 88.0 | 46.3 | **243.7 KB** |
| 360 / dpr2 | 306 | **640** | 86.7 | 66.9 | 46.3 | **199.9 KB** |
| 320 / dpr2 | 266 | **640** | 86.7 | 66.9 | 46.3 | **199.9 KB** |

- **최악 조건 391.4 KB** — 종전 **451.1 KB** 대비 **−59.7 KB(−13.2%)**. 기준선 **390 KB** 와 **1.4 KB 차**(사실상 도달).
- **개발자 예상표 6칸 전부 실측과 일치**(320/360/390 × dpr2/dpr3).
- **슬롯 실측 336 / 306 / 266** — `100vw − 54px` 모델이 세 폭에서 정확히 성립. 개발자가 밝힌 유래(`px-4` 1rem=12px + `p-5` 1.25rem=15px, ×2 = 54)도 값과 맞는다.
- 이미지 요청은 조건마다 **정확히 3건**(배치도 `unoptimized` + 최적화 2장) · 중복 0 · 비-200 응답 0.
- `branch-layout.png` 는 `sizes` 없이 **46.3 KB 그대로** — 손댈 것이 없었다는 개발자 판단이 맞다.

### §74-2. ⚠⚠ **거짓 결함을 낼 뻔했다 — `w=1920` 이 «내 계측 아티팩트»였다**

첫 측정에서 **360/dpr3 · 320/dpr3 · 390/dpr2 세 조건에 `w=1920` 요청이 «하나씩 더»** 나타났다(총계 935.8 / 817.7 / 788.1 KB). *«같은 이미지를 두 번 받는다»* 로 **보고 직전까지 갔다.**

**참 원인**: 내가 **CDP `Emulation.setDeviceMetricsOverride` 를 걸고 `goto` 했는데**, override 가 적용되기 «전»에 문서 파싱이 시작돼 **기본 뷰포트로 `sizes` 를 계산한 첫 요청**이 나가고, override 후 **다시 요청**한 것이다.
**검증**: 조건을 «처음부터» 가진 **새 컨텍스트**(`browser.newContext({viewport, deviceScaleFactor})`)로 재니 **6조건 전부 `w=1920` 이 0** 이고 요청이 정확히 3건이었다.

★ **§69-6 과 같은 계보다 — «내가 만든 상태»가 «내가 믿은 상태»와 달랐다.** 그때는 거리뷰 모드 자동 해제·고정 헤더였고, 이번은 **에뮬레이션 적용 시점**이다.
★ **그리고 «절반만 이상» 이 또 위험했다** — 390/dpr3 에서는 안 나오고 나머지 셋에서만 나와서 *«조건에 따라 다르다»* 는 그럴듯한 설명이 먼저 떠올랐다(§69-6 이 적은 그대로다).

### §74-3. 다른 축 회귀 — 영향 0

| 항목 | 결과 |
|---|---|
| **200% 가로**(320·360·390 × 기본/200%) | **6조합 전부 초과 0** · 클리핑 없는 넘침 요소 0 · `clientWidth` 305 / 345 / 375(스크롤바 15) |
| **첫 화면 세로 예산** | `10시 30분` **304.5 – 334.5 · 여유 305.5px** — §73 과 **동일** |
| 문서 높이(360 · cw 345) | **5,336**(§73 5,335 · **+1px**) |
| 지도 청크 | **불변** — 지도 A~E 영향 0 |

### §74-4. 관찰

| # | |
|---|---|
| **O14** | **에뮬레이션은 «적용 시점»이 결과를 바꾼다.** `setDeviceMetricsOverride` + `goto` 조합은 **첫 요청이 override 전 조건으로 나간다.** 이 저장소에서 반응형 이미지·`sizes` 를 잴 때는 **조건을 가진 새 컨텍스트**를 만들어라. ★ §5.8.4(«측정 위생»)의 새 항목 — 지금까지는 «내가 남긴 것을 지운다»였는데 이것은 **«내가 건 것이 언제 걸리는가»** 다 |

---

## §75 QA (2026-09-01 · `54px` → `4.5rem` 재수정 회귀) — ★ **회귀 1건. `54px` 로 되돌려야 한다**

**동결** `page.tsx 42fb0559d202` · 나머지 3개 파일 불변 · 지도 청크 `93339a3c1cde` 불변 · `BUILD_ID 3UJT7GSWxhVqA27y9M9D3`
**변경** `sizes` 2곳: `calc(100vw - 54px)` → **`calc(100vw - 4.5rem)`**

### §75-1. ★★ 실패 — **`rem` 판이 «슬롯보다 작은» 이미지를 받는다**

루트 폰트가 `12px`(`html{font-size:75%}`)이니 `4.5rem = 54px` 로 **같아야 하는데, 실측이 갈렸다.**

| 조건 | 슬롯 | 필요 px(슬롯×dpr) | **`54px` 판(§74)** | **`4.5rem` 판(현행)** | 판정 |
|---|---|---|---|---|---|
| 390 / dpr3 | 336 | 1,008 | 1080 | **1080** | = |
| 360 / dpr3 | 306 | 918 | 1080 | **1080** | = |
| **320 / dpr3** | **266** | **798** | **828** ⭕ | ★ **750** | ❌ **48px 부족(6.0%)** |
| **390 / dpr2** | **336** | **672** | **750** ⭕ | ★ **640** | ❌ **32px 부족(4.8%)** |
| 360 / dpr2 | 306 | 612 | 640 | **640** | = |
| 320 / dpr2 | 266 | 532 | 640 | **640** | = |

### §75-2. 원인 — **프리로드 스캐너가 `rem` 을 «16px» 로 평가한다.** 6조건 전부 예측과 일치

`4.5rem` 을 **16px 기준**으로 보면 **72px** 이다. 그 가정으로 계산한 값이 **관측과 6/6 일치**한다:

| 조건 | `100vw − 72` | × dpr | 다음 `deviceSize` | **관측** |
|---|---|---|---|---|
| 320 / dpr3 | 248 | 744 | **750** | **750** ✓ |
| 390 / dpr2 | 318 | 636 | **640** | **640** ✓ |
| 390 / dpr3 | 318 | 954 | 1080 | 1080 ✓ |
| 360 / dpr3 | 288 | 864 | 1080 | 1080 ✓ |
| 360 / dpr2 | 288 | 576 | 640 | 640 ✓ |
| 320 / dpr2 | 248 | 496 | 640 | 640 ✓ |

**`54px` 판도 6/6 일치**한다(320/dpr3 → 266×3=798 → **828** · 390/dpr2 → 336×2=672 → **750**).
→ ★ **`sizes` 안의 `rem` 은 우리 `html{font-size:75%}` 를 «못 본다».** 브라우저가 이미지 후보를 고르는 시점은 **CSS 적용 전(프리로드 스캐너)** 이고 그때 `rem` 은 **초기값 16px** 이다.

★★ **이것은 `V3-IMAGES` 가 이미 경고한 함정의 «같은 계보»다** — *「`--container-page: 60rem` = 720px 이다. CSS 주석의 「960px」는 **16px 가정이라 낡았다**」*. 그때는 **문서가** 16px 을 가정해 틀렸고, 이번은 **브라우저가** 16px 로 평가해 갈린다. **방향이 반대인 같은 함정이다.**

### §75-3. 손실의 성격 — 전송량이 아니라 **판독**

| | `54px` 판 | `4.5rem` 판 |
|---|---|---|
| 최악 전송(390/dpr3) | **391.4 KB** | **391.4 KB** (동일) |
| 320/dpr3 전송 | 273.3 KB | 243.7 KB(−29.6) |
| 390/dpr2 전송 | 243.7 KB | 199.9 KB(−43.8) |
| **화질** | **전 조건 슬롯 충족** | ★ **2조건에서 4.8~6.0% 부족** |

⚠ **「전송량이 줄었으니 좋다」로 읽지 마라.** 이 두 장은 **판독 대상이 걸린 이미지**다(지도의 `화장실 : 임시화장실 및 인근화장실(시청역, 스벅 등) 이용` · 로드뷰의 `PAUL BASSETT` 간판). `V3-IMAGES` 가 **Next 산출물에서 판독 생존을 확인**한 것은 **`w=1080` 기준**이었다.
→ **처방: `calc(100vw - 54px)` 로 되돌린다.** ⚠ **`rem` 이 「더 정확해 보인다」는 직관이 여기서는 틀린다** — 확대 시 실제 슬롯은 줄지만, **`sizes` 평가 시점에는 그 확대가 반영되지 않는다.**
⚠ 되돌릴 때 **주석에 사유를 남겨라** — 안 남기면 다음 사람이 *「루트가 12px 인데 왜 54px 하드코딩?」* 으로 **다시 `rem` 으로 고친다.**

### §75-4. 다른 축 — 영향 0

200% 확대 조건(390·360 × dpr3 · 루트 32px)에서 슬롯이 **246 / 216** 으로 줄지만 `w` 는 **1080 유지**(과대라 화질 손실 없음) · **가로 초과 0** · 이미지 요청 **조건마다 정확히 3건** · 비-200 응답 0.

### §75-5. 기록

★ **「값이 같아야 하는 두 표기」가 갈렸다.** `4.5rem` 과 `54px` 는 이 페이지에서 **CSS 상 같은 값**인데 **`sizes` 라는 «다른 평가 시점»에서 갈렸다.** 이 저장소의 «같은 이름 다른 양» 계보에 **«같은 값 다른 평가 시점»** 을 추가한다.
★ 그리고 **내가 §74 를 닫는 «도중»에 이 변경이 들어왔다** — 마감 해시를 찍지 않았으면 §74 값을 현행으로 오인해 **회귀를 통째로 놓쳤다.**

### §75-6. 최신 해시 재확인 (`page.tsx bf7104d230fe` · `sizes` 는 여전히 `4.5rem`)

| 조건 | 슬롯 | 필요(슬롯×dpr) | 받은 `w` | **부족** |
|---|---|---|---|---|
| **320 / dpr3** | 266 | 798 | **750** | **48px (6.0%)** |
| **390 / dpr2** | 336 | 672 | **640** | **32px (4.8%)** |
| 390 / dpr3 | 336 | 1,008 | 1080 | 0 |
| 360 / dpr3 | 306 | 918 | 1080 | 0 |

**결과 재현. 실패 유지.** 요청 건수는 조건마다 정확히 3건 · 비-200 응답 0.


---

## §76 QA (2026-09-01 · `54px` 복원 확인) — ★ **§75 회귀 해소. 실패 0**

**동결(재측정 전·후 동일)** `page.tsx f2bcb3f914ec` · `strikeMap.ts 287fc087` · `StrikeMap.tsx d1bd0465` · `StrikeRoadviewSheet a539f449` · **지도 청크 `93339a3c1cde` 불변** · `BUILD_ID w2eKnKO1JQXgQYNBSb0LP`
**선언 확인** 소스 2곳 + **프리렌더 2건** 모두 `calc(100vw - 54px)`

### §76-1. 전송량·해상도 — **6조건 전부 슬롯 충족. 부족 0**

| 조건 | 슬롯 | 필요(슬롯×dpr) | 받은 `w` | 부족 | **총 전송** |
|---|---|---|---|---|---|
| **390 / dpr3** | 336 | 1,008 | **1080** | **0** | ★ **391.4 KB**(최악) |
| 360 / dpr3 | 306 | 918 | 1080 | 0 | 391.4 KB |
| 320 / dpr3 | 266 | 798 | **828** | **0** | 273.3 KB |
| 390 / dpr2 | 336 | 672 | **750** | **0** | 243.7 KB |
| 360 / dpr2 | 306 | 612 | 640 | 0 | 199.9 KB |
| 320 / dpr2 | 266 | 532 | 640 | 0 | 199.9 KB |

- **§75 가 지목한 두 조건이 복구됐다**: 320/dpr3 **750 → 828** · 390/dpr2 **640 → 750**.
- 요청은 조건마다 **정확히 3건**(배치도 `unoptimized` 46.3 KB + 최적화 2장) · 중복 0 · 비-200 응답 0.
- **최악 391.4 KB** — 기준선 390 KB 와 1.4 KB 차. §72 의 **451.1 KB** 대비 **−59.7 KB(−13.2%)**.

### §76-2. 다른 축 회귀 — 영향 0

| 항목 | 결과 |
|---|---|
| **200% 가로**(320·360·390 × 기본/200%) | **6조합 전부 초과 0** · 클리핑 없는 넘침 요소 **0** |
| `clientWidth` | 이번 실행은 **스크롤바 0**(320/360/390) — §73·§74 는 15 였다. **양쪽 조건에서 모두 통과했다** |
| **첫 화면** | `10시 30분` **304.5 – 334.5 · 여유 305.5px** — §73 이후 **네 회차 연속 동일** |
| 섹션 top(360) | 집결시간 228.3 · 개요 464.3 · 안내지도 748.2 · **총파업 일정 1,609.7** · **원본 자료 3,352.7** |
| 문서 높이 | 320 **5,280** · 360 **5,402** · 390 **5,553**(스크롤바 0 조건) |
| 200% 문서 높이 | 320 **19,310** · 360 **17,615** · 390 **17,422** |
| 지도 | 청크 **`93339a3c1cde` 불변** → A~E 영향 0 |

### §76-3. 이 4회차(§72~§76)가 남기는 것 — **`sizes` 계약**
...
| 항 | 값 | 근거 |
|---|---|---|
| 데스크톱 | `624px` | 슬롯 실측 624 와 정확히 일치 |
| 모바일 | **`calc(100vw - 54px)`** | 슬롯 = `100vw − (px-4 12px + p-5 15px) × 2` · 336/306/266 세 폭 실측 일치 |
| ⛔ **`4.5rem` 금지** | — | **`sizes` 의 폰트 상대 단위는 «초기» 16px 로 평가**된다(프리로드 스캐너가 스타일 적용 «전»에 판단). `html{font-size:75%}` 를 못 본다 → 72px 로 풀려 **2조건에서 과소 요청**(§75) |

★ **이 계약의 «검사 방법»도 함께 못박는다**: 조건마다 **새 컨텍스트**(`viewport`+`deviceScaleFactor`)를 만들어 잰다. `setDeviceMetricsOverride` + `goto` 는 **첫 요청이 override 전 조건으로 나가** 유령 `w=1920` 을 만든다(§74-2).

### §76-4. 기록 — 이 4회차의 형태

| 회차 | 무엇이 있었나 |
|---|---|
| §72 | 실패 1(전송 451.1 KB · `w=1200`) 발견 + 기대표 불일치 8건이 **문서 문제**임을 갈랐다 |
| §73 | 리더의 **순서 실수**로 자리 의존 값 폐기 → 세로 축 재측정 |
| §74 | 수정 반영 확인(391.4 KB) + **유령 `w=1920` 이 내 계측 아티팩트**임을 갈랐다 |
| §75 | **`4.5rem` 재수정이 회귀**임을 산술로 확정(6/6 예측 일치) |
| **§76** | **복원 확인. 실패 0** |

★★ **§74 → §75 사이가 이 라운드의 핵심이다.** 개발자가 **좋은 의도로**(`rem` 이 확대를 따라간다) 고쳤고 리더가 **레이아웃 실측으로 보강**했는데, **둘 다 «측정은 맞았고» «그 측정이 이 결정에 닿는지»를 아무도 안 물었다.** `sizes` 는 **프리로드 시점**에 얼어 있어 **어느 쪽도 슬라이더를 못 따라간다.**
→ 이 저장소의 **«대상·계층·시점이 질문에 안 맞는»** 계보에 **«같은 값의 두 표기가 다른 «평가 시점»에서 갈린다»** 를 추가한다.

### §76-5. 최종 해시 재확인 — **`page.tsx b9dba3c7eccf`**(주석 추가분 반영 · 렌더 무변경)

소스 `calc(100vw - 54px)` **3건**(`sizes` 2 + 주석 1) · `4.5rem` **0건** · 프리렌더 `calc(100vw - 54px)` **4건** · `4.5rem` **0건**.

| 조건 | 슬롯 | 필요 | 받은 `w` | 충족 | 전송 |
|---|---|---|---|---|---|
| 390 / dpr3 | 336 | 1,008 | **1080** | ⭕ | **391.4 KB** |
| 320 / dpr3 | 266 | 798 | **828** | ⭕ | 273.3 KB |
| 390 / dpr2 | 336 | 672 | **750** | ⭕ | 243.7 KB |

**재현 확인. 실패 0.** 요청 3건 · 중복 0.
⚠ 이 회차에 `page.tsx` 가 **네 번** 저장됐다(`a880e6cd → 42fb0559 → bf7104d2 → f2bcb3f9 → b9dba3c7`). **매번 마감 해시를 찍어 §75 회귀를 잡았다** — 안 찍었으면 §74 값을 현행으로 오인했다.


---

### §76-6. ⛔ **§76-5 선언 정정 — 내가 «축 없는 값»을 적었다** (개발자 지적 · 독립 재현)

**§76-5 에 이렇게 적었다:**
> ~~소스 `calc(100vw - 54px)` **3건**(`sizes` 2 + 주석 1) · **`4.5rem` 0건** · 프리렌더 `calc(100vw - 54px)` **4건** · `4.5rem` **0건**~~

**두 곳이 계층 없이 적혔다. 직접 다시 세어 갈랐다:**

| 문자열 | 소스 리터럴 | **`sizes` 속성** | **프리렌더(`<script>` 제거)** | 프리렌더 전체 |
|---|---|---|---|---|
| `calc(100vw - 54px)` | **3**(`sizes` 2 + 주석 1) | **2** ⭕ | **2** ⭕ | 4 |
| **`4.5rem`** | ★ **4** | **0** ⭕ | **0** ⭕ | 0 |

#### ① `4.5rem` — **소스에 4건 있다. 「0건」은 거짓이었다**

그 4건은 **전부 `page.tsx:679~685` 의 «⛔ 바꾸지 마라» 금지 기록 주석**이다 — **내가 §75 에서 «주석에 사유를 남겨라» 라고 요청해 개발자가 쓴 바로 그것**이고, 내 실측표(`320`/dpr3 828↔750 · `390`/dpr2 750↔640)를 인용하고 있다.

★★ **`V3-PLAN §0` 이 경고한 형태 그대로다** — *「소스 `grep` 으로 금지어를 세지 마라. 이 저장소는 **주석에 금지어를 적는 것이 설계**다」*(선례 `손피켓` 소스 ↔ 렌더 0).
⚠⚠ **그 선언을 계층 없이 회귀 기준으로 쓰면 다음 게이트가 소스에서 4건을 찾아 «금지 기록 주석을 지우는 것»으로 처방한다 — 그 주석이 바로 이 회귀를 막는 장치다.**
★ **가장 나쁜 형태의 자기모순이 될 뻔했다**: 내가 요청해 만든 방어 장치를, 내가 축을 빠뜨린 선언 때문에, 다음 사람이 지운다.

#### ② 프리렌더 `54px` **4건**도 계층이 없었다

`<script>` 를 지우면 **2건**(= `sizes` 속성 2)이고, 나머지 **2건은 RSC flight 페이로드 안**이다.
★ 이 저장소가 이미 세운 규율(*「Next 산출물을 셀 때는 `<script>` 를 먼저 지운다」*)을 **내가 마감 확인에서 안 지켰다.**

#### ★ 확정 선언 (이것을 회귀 기준으로 써라)

```
sizes 속성      calc(100vw - 54px) 2 · 4.5rem 0
프리렌더(<script> 제거)  calc(100vw - 54px) 2 · 4.5rem 0
소스 주석       54px 1 · 4.5rem 4      ← 전부 «금지 기록». 정당한 출현이다. 지우지 마라
```

#### §76-7. 기록 — **이 라운드 여덟 번째 «축 없는 값»이고, 처음으로 «선언»에서 났다**

앞선 일곱 건은 전부 **측정**에서 났다(대상·계층·시점). 이번은 **측정은 맞았고 «선언문»에 축이 빠졌다** — 값이 틀린 것이 아니라 **그 값이 어느 층의 값인지를 안 적었다.**
★ **마감 문장은 측정만큼 검증되지 않는다**는 것이 이 건의 교훈이다. 측정 블록에는 §5.5 환경표를 꼬박 붙였는데, **«확인 완료» 한 줄에는 아무 축도 안 붙였다.**
