...
#### H-2. ★★ R-1 — `18:30` × 360×640. **선언 판정선 통과. 다만 여유 14.7px**
...
| **실패** | **0** |
| **게시 블로커** | **0** |
| **★ 리더 판단 항목** | **1** — `today` 여유 **14.7px**(H-2). 선언 판정선(360×640)으로는 **통과**이나, **내일이 `today`** 이고 **부제 한 줄이 18.4px 을 가져갔다**. 실기기 주소창을 고려하면 첫 화면에서 `18:30` 이 밀릴 수 있다. **판정선을 그대로 둘지는 리더 소관** |
| **QA 자체 정정** | **5번째** — H-1(전후 해시 대조의 오탐 모드) |
| **미검증(존치)** | U-1 실기기 핀치 · U-2 냉캐시 `srcset` · U-4 실기기 맨눈 판독 · **R-3 `today` 는 프로브**(실물은 **2026-08-28 당일**에만 관측 가능) |
