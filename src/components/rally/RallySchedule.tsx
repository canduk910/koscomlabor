 ...
 *
 * ## ★★ 위 «고치지 마라» 는 «공식 순서 16행» 에 걸린 규율이다 — 두 열의 출처가 다르다
 *
 * | 열 | 출처 | 고칠 수 있나 |
 * |---|---|---|
 * | **공식 순서**(시각 · 내용 · 인명) | **주최측 안내자료 전사** | **아니다.** 한 글자도 안 된다(인명은 소속 병기 필수) |
 * | **코스콤 조합원 일정** | **지부 자체 자료**(사용자 첨부 `4. 조합원 상세일정`) | **자료 주체가 사용자다** — 지시가 오면 고친다 |
 *
 * ★ **2026-08-26 에 코스콤 열 3건을 고쳤다**(사용자 지시 · 검증 §45-7 전부 승인):
 * ① 물품 수령의 `투쟁조끼` → `총파업머리띠` · ② `투쟁용품 반납 및 해산` → **`해산`** ·
 * ③ 그 항목의 `details`(투쟁용품 반납)·`note`(집회장소에서 반납) **삭제**. 근거는 각 줄의 주석에 있다.
 * **주최측 전사에 `투쟁용품`·`조끼`·`반납` 은 0건이고, 공식 16행은 이때도 한 글자도 안 바뀌었다.**
 *
 * ⚠ **이 문단을 지우지 마라.** 안 적으면 다음 사람이 위 변경을 **«금지를 어겼다»** 로 읽거나,
 * 반대로 **«공식 열도 고쳐도 된다»** 로 안다(검증 §45-11 조건 9).
 *
      ...
      items: [
        {
          ...
          details: [
            ...
            /*
             * ★ **`투쟁조끼` → `총파업머리띠`**(사용자 지시 2026-08-26 · 검증 §45-7(a) 승인).
             * **코스콤 열이다** — 파일 상단의 «시각·항목명을 고치지 마라» 는 **공식 16행에 걸린 규율**이고
             * 이 줄은 지부 자체 자료라 사용자가 그 자료의 주체다(1급).
             *
             * ⚠⚠ **`손피켓` 을 지우지 마라** — **출석 QR 이 손피켓에 들어 있다**
             * (블록 2-A `※ 출석 QR은 손피켓에 넣어 배포합니다`). 이 줄이 **조합원이 QR 을 손에 넣는 지점**이라
             * 행동이 갈린다.
             * ⚠ **`등` 을 유지한다** — 빼면 «이 셋이 전부»라는 **우리가 갖고 있지 않은 단정**이 된다.
             * ⚠ **`총파업머리띠` 붙여쓰기를 고치지 마라** — 사용자 지정 문면이고,
             *   *"틀렸는가"* 가 아니라 *"다르게 행동하게 되는가"* 가 기준이다(§5.7).
             */
        ...
        {
          /*
           * ★ **`담당 운영위원 및` 을 뺐다**(사용자 지시 2026-08-27 — *"운영위원 체크를 뺄거야.
           * 별도 검증없이 바로 반영부탁해"*). **두 곳(시작·종료 출석체크) 모두** 같은 값이었다.
           *
           * ★ **정합이 오히려 좋아진다**: 상품권 지급 조건이 **`참석명단 작성(지부천막)` + `금융노조 QR인증(1차·2차)`**
           * 두 가지인데(`QrAttendanceCard`), 이 칸이 `담당 운영위원 및 …` 이면 **운영위원 체크가 별개 절차처럼**
           * 읽혀 조합원이 «셋을 해야 하나» 로 셀 수 있었다. 이제 **출석체크 = QR** 로 한 축이다.
           * ⚠ **되살리려면 상품권 조건 문장과 함께 봐라** — 그 문장이 조건을 «둘» 로 세고 있다.
           */
          time: "18:50~19:00",
          title: "시작 출석체크",
          note: "금융노조 출석QR코드",
  ...
  { time: "20:15~20:20", content: "구호제창 및 파업가 제창", person: null },
  {
    ...
    koscom: {
      ...
      items: [
        ...
        /*
         * ★ **`투쟁용품 반납 및 해산` → `해산`**(사용자 지시 2026-08-26 · 검증 §45-7(b)(c) 승인).
         * **코스콤 열이다**(위 물품 수령 줄과 같은 근거 — 공식 16행은 한 글자도 안 바뀐다).
         * `details: ["투쟁용품 반납 (투쟁조끼)"]` 와 `note: "집회장소에서 반납"` 을 **함께 지웠다.**
         *
         * ★ **«한 낱말만 남는 것»이 성립한다.** 사용자 근거가 논리를 닫는다 —
         * *"투쟁조끼를 안쓸거라 따로 반납할 물품이 없어."* 반납할 물품이 없으면 반납 절차가 없고,
         * 그 시각에 일어나는 일은 **해산뿐**이다. 다른 항목도 전부 명사구라 **형태 불일치도 없다**
         * (`시작 출석체크`·`종료 출석체크`·`결의대회 행사`).
         *
         * ⚠⚠ **칸이 빈약해 보인다고 문장을 짓지 마라** — **빈약해 보이는 것과 사실이 그것뿐인 것은 다르다.**
         * 채우는 순간 창작이다(§5.3).
         * ⚠ **`반납 없음` 같은 형태로 남기지도 마라** — **없는 것을 «없다»로 적는 것도 문장이다.**
         * ⚠ 이 삭제로 **`투쟁조끼`·`반납` 이 «렌더되는 문자»에서 완전히 사라진다. 의도된 것이다**
         * (검증 §45-8 전수 확인 — 프리렌더 HTML 실측 0건). **소스 grep 은 0 이 아니다** — 이 주석들이 잡힌다.
         */
        { time: "20:30~", title: "해산" },
      ],
    },
