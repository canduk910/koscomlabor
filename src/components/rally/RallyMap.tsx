  ...
  return (
    ...
    >
      ...
      */}
      ...
      {/*
        ★ **두 문장 다 «화면에 단서가 없는 조작»이다**(§5.3 판정 기준: *"이 문장이 없으면
        조합원이 다르게 행동하는가"* — 둘 다 예).

        1. 지도를 눌러 위치를 옮기는 것 — 파란 길이 무엇인지도 여기서만 말한다.
        2. **바닥을 눌러 이동하는 것**(사용자 지시 2026-08-24, 기능과 함께 승인). 네이버 화살표는
           보이지만 **탭 이동은 우연히 눌러 보기 전에는 알 수 없다.**

        ⚠ **둘 다 지우지 마라.** 종전에 있던 *"드래그로 둘러보고, 화살표로 길을 따라 걸을 수
        있습니다"* 는 **보편 조작 + 화면에 보이는 것**이라 지웠다(2026-08-22) — 그 기준으로
        지운 문장을 되살리지도 마라.

        ⚠ **손잡이 설명을 덧붙이지 마라**(§5.3). 막대 손잡이는 보편 관용이고,
        보이지 않는 조합원에게는 `role="separator"` + `aria-label` 이 이미 말한다.

        조판: 두 문장을 **각각 `block`** 으로 둔다. 이어 붙이면 390px 에서 첫 문장 꼬리와
        둘째 문장 머리가 한 줄에 섞여 어느 조작 이야기인지 훑어서 갈리지 않는다.
        **굵게 하는 것은 «무엇을 누르는가» 뿐이다** — `파란 길`·`바닥`.
      */}
