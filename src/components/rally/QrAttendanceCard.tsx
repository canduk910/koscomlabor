      ...
      */}
      ...
      {/*
        ...
        *"지정된 시간에만 체크 가능합니다."* 한 줄만 나온다(실측). 이 문장이 없으면
        ...
        ★★ **«손피켓 QR» 이라고 특정한 이유 — 그냥 «QR» 이라고 쓰면 거짓이 된다.**

        위 안내 이미지 안에도 QR 이 있고(빨간 동그라미), **그것은 출석 페이지로 가지 않는다.**
        실측(2026-08-25 · CoreImage 로 디코딩):
          이미지 속 QR → `https://m.site.naver.com/1tgqt` → 308 → `https://qrpage.kr/` → 302 →
          **`https://qrpage.kr/pr/`(웹 제작 업체 «피알페이지» 회사소개 홈페이지)**
        iOS·Android 사용자 환경 각각 확인, 결과 동일.

        **실제 손피켓에 인쇄되는 QR 은 이 이미지의 QR 과 다르다**(사용자 확인 2026-08-25).
        그래서 문장이 가리키는 대상은 **손피켓 QR** 이고, 이미지 속 QR 이 아니다.
        ⚠ **«QR 을 찍으면 나오는 페이지» 로 줄이지 마라** — 바로 위 이미지의 QR 을 가리키게 되어
        조합원을 업체 홈페이지로 보낸다.
        ⚠ 이 사실은 **우리가 잰 것이 아니라 사용자가 확인해 준 것**이다(손피켓 실물이 우리에게 없다).
      */}
      <a
      ...
      >
        ...
        <span className="mt-1 block break-keep text-caption text-white">
          손피켓 QR로 들어가는 출석체크 페이지와 같습니다. 지정된 출석 시간에만 체크됩니다.
        </span>
