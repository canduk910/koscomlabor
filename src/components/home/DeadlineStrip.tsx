import Link from "next/link";
import type { PostListItem } from "@/lib/postView";
import { ROUTES } from "@/lib/routes";
import { daysUntilKst, formatMonthDaySlash } from "@/lib/date";

/**
 * 마감 스트립 (스펙 §16.9.5 — §11.5 의 시각 갱신판. 노출 조건·D-n 규칙·색 의미는 전부 계승).
 * - 노출 조건: deadline이 오늘 이후인 verified 게시물 1건 이상 (판정은 selectUpcomingDeadlines).
 *   0건이면 page.tsx에서 미렌더.
 * - 배경 primary-soft, radius 16px(rounded-card), px-5 py-3.5,
 *   모바일 가로 스크롤(페이지 가로 스크롤 금지 — 스트립 자체만 overflow-x-auto).
 * - 항목: "M/D 제목" 15px/700/#093389 (9.23:1), 각각 게시물 상세 링크.
 * - D-7 이내: 항목 전체 red 필 칩(urgent-strong + 흰 텍스트, 8.46:1) + "D-n" 텍스트 병행
 *   (색만으로 임박을 전달하지 않음). soft 배경 위 빨강 텍스트는 대비 미달로 금지.
 * - **항목 간 1px 세로 구분선 폐기** → gap-4(16px) 여백으로 분리(§16.5 선 의존 제거).
 *   구분선을 되살리지 말 것.
 */
export function DeadlineStrip({ posts }: { posts: PostListItem[] }) {
  if (posts.length === 0) return null;

  return (
    <nav aria-label="마감 예정 일정" className="rounded-card bg-primary-soft px-5 py-3.5">
      <ul className="flex items-center gap-4 overflow-x-auto">
        {posts.map((post) => {
          if (post.deadline === null) return null;
          const days = daysUntilKst(post.deadline);
          const imminent = days !== null && days <= 7;
          const monthDay = formatMonthDaySlash(post.deadline);
          // 분류→상세 경로는 ROUTES.post 매핑 (§15.6R-G) — 삼항 분기를 늘리면 새 분류가
          // 조용히 잘못된 경로(404)로 간다. 항목은 게시물 상세 링크이며 동작 변화 0
          const href = ROUTES.post(post.category, post.id);
          return (
            <li key={post.id} className="flex shrink-0 items-center">
              <Link
                href={href}
                className={`inline-flex min-h-touch items-center gap-1 whitespace-nowrap text-caption font-bold focus-visible:outline-3 focus-visible:outline-primary focus-visible:outline-offset-2 ${
                  imminent
                    ? "rounded-full bg-urgent-strong px-3 py-1 text-white"
                    : "text-primary"
                }`}
              >
                {imminent ? <span>D-{days}</span> : null}
                <span>
                  {monthDay} {post.title}
                </span>
              </Link>
            </li>
          );
        })}
      </ul>
    </nav>
  );
}
