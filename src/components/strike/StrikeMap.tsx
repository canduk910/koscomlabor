...
const GO = "#093389"; // 파랑 11.37 — 경로(무대 2·역 2)
/** ★ 주황 5.18 — **코스콤지부 구역 «하나»**(2026-09-02 사용자 지시 · 원본 사진이 주황 사각형이다).
 *  ⚠ 밝게(`#ea580c` 3.56 — 본문 미달)도, 진하게(7.31↑ — 갈색으로 읽힌다)도 하지 마라. */
const OURS = "#c2410c";
...
const toneColor = (tone: StrikeMapTone): string =>
  tone === "go" ? GO : tone === "ours" ? OURS : REFERENCE;
...
function pillHtml(options: {
  ...
  /* `min-height` 로 24 를 **보증**한다 — padding 만으로는 글꼴 메트릭에 따라 값이 흔들린다(위 `HIT_MIN_PX`).
     ...
     ⚠⚠ **코스콤 pill 만 키우거나 색을 반전시키지 마라** — «강조»는 `tone: "ours"`(주황 · 2026-09-02)와 «유일한 채워진 사각형»
