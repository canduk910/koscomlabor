 ...
 *
 * ## ★ 이 페이지의 «200% 확대 슬롯»은 네 종류다 — 추산하지 마라 (QA 실측 2026-08-26)
 *
 * 360 뷰포트 · 스크롤바 15px · 루트 12px(`globals.css` `html{font-size:75%}`) 기준:
 *
 *   카드 «밖»(컨테이너 `px-4` 만)            281 px   ← QR 출석체크 섹션
 *   카드 «안»(`rounded-panel … p-5`)         201 px   ← 블록 1 · 블록 2
 *   배너(`rounded-card … p-4` + `border`)    215 px   ← PreSurveyBanner
 *   순서표 셀(`table-fixed`)                  46.6 px  ← RallySchedule
 *
 * ⚠ **문면의 폭만 재고 «그 문면이 앉을 슬롯»을 확인하지 않으면 맞는 숫자로 틀린 판정을 한다.**
 *   실제로 그랬다 — 사전 실측표가 카드 «안» 기준이었는데 카드 «밖»에 앉는 문장에 그 수치가 인용됐다.
 * ⚠ **배너 슬롯을 `345 − px-4 − p-4 = 217` 로 추산하지 마라.** `border` 1px 양쪽이 빠져 참값은 215 다.
 * ⚠ 폭을 잴 때는 **`width:0` 요소의 `scrollWidth`**(= 실제 렌더 가능한 최소 폭)를 쓴다.
 *   `width:min-content` 의 rect 는 **`overflow-wrap:break-word` 를 반영하지 않아** 다른 값을 낸다.
 *   (규율 정본: `union-qa-testing` §5.7 · `union-design-system` §0.8)
 */
              ...
              */}
              ...
              {/*
                ...
                **카드가 2026-08-23 에 삭제되며 소멸했다**(`STAGE3_SENTENCE` 계열은 지금 **소비처가 0**이다).
                ...
                ★★ **그리고 검사 비용이 올라갔다**(QA 부록 G-8):
                종전 결속(캡션 ↔ ④)은 **문자열 대조로 자동 검사**됐다. 새 결속은 **대조 대상이
                문서가 아니라 이미지 안의 픽셀**이라 **육안·판독으로만** 검사된다.
                ⚠ **`grep` 으로 캡션을 아무리 훑어도 안 걸린다.** 이미지를 교체하는 사람이
                «캡션만 보면 된다»로 착각하지 않게 하려고 적는다 — **그림을 열어 두 낫표 안 글자를 읽어라.**
                (쌍 2 — 산문 ⑤의 `도로 위`·`길가` ↔ `map` `alt` — 는 여전히 문자열 대조가 가능하다.)
