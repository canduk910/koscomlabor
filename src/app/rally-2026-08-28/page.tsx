          ...
          <section aria-labelledby="gather-heading" className="mt-section md:mt-section-lg">
            ...
            <div className="rounded-panel shadow-card mt-6 bg-bg p-5 md:p-8">
              ...
              */}
              ...
              {/*
                행 순서는 **집결 → 본대회 → 코스콤지부 → 장소**다(검증 요구 62 · §22.13.2).
                시각 3행이 시간순으로 붙고 유일한 비시각 항목인 `장소` 가 맨 아래로 간다 —
                `코스콤지부` 행을 `장소` 아래로 내리면 하루의 시간표로 읽히지 않는다.
              */}
              <dl className="mt-6 grid grid-cols-[auto_1fr] gap-x-4 gap-y-2 md:mt-8">
                ...
                {/*
                  `<dt>` 가 **주어를 진다**(검증 요구 58 · §22.13.1). **`종료` 를 라벨로 쓰지 마라 —
                  주어를 붙여도 반려됐다.** `<dt>` 열만 훑는 조합원(`본대회 / 코스콤지부 / 장소`)에게
                  `종료` 가 보이면 **행사 종료 시각으로 읽는다.** 20:30 은 주최측 안내가 아니라
                  **지부의 참가 계획**이고(사용자 §6.8 — "그 시간까지는 끝낼 계획"),
                  주어가 문장 수사가 아니라 **자료 구조**로 고정돼야 나중에 문안을 다듬어도 빠지지 않는다.

                  `20:30 해산 예정`·`20:30 종료`·`20:30에 끝납니다` 는 **전량 반려된 표현**이다
                  (사용자가 말하지 않은 사실을 추가하거나 단정한다).
                  **8/28 이 지난 뒤에도 `참가 계획입니다` 를 과거형으로 바꾸지 마라**(요구 64) —
                  계획이 실제로 그렇게 실행됐는지 우리는 확인할 수 없다. `RALLY_PAST_NOTE` 가
                  `열렸습니다`·`성황리에` 를 금지한 것과 같은 원칙이다.

                  `<time>` 을 붙이지 않는다(§22.13.5): `19:00 개회` 가 평문이라 한 표에서 한 행만
                  마크업이 다르면 **그 차이 자체가 의미로 읽힌다.** 대형 수치·색 강조도 금지 —
                  `20:30` 은 `19:00 개회` 와 완전히 같은 급이고, 이 페이지의 대형 수치는 `18:30` 하나다.
                */}
                <dt className="text-caption font-semibold text-ink-muted">코스콤지부</dt>
                <dd className="break-keep text-body text-ink">20:30까지 참가 계획</dd>
              ...
              {/*
                ...
                1행: **`종료 시각은 안내되지 않았습니다` 를 빼지 마라.** 빼면 `20:20` 이 행사 종료
                시각으로 굳는다. 식순표는 `20:20~ 폐회선언` 이 마지막이고 **16행 중 유일하게 열린
                표기**다 — 주최측은 종료 시각을 안내한 적이 없다.
              ...
              */}
              <p className="mt-3 max-w-[var(--container-prose)] break-keep text-body text-ink">
                ※ 20:30 은 코스콤지부의 참가 계획입니다. 주최측 식순상 폐회선언은 20:20~ 이며, 종료
                시각은 안내되지 않았습니다.
              </p>
          ...
          <section aria-labelledby="position-heading" className="mt-section md:mt-section-lg">
            ...
            <div className="rounded-panel shadow-card mt-6 bg-bg p-5 md:p-8">
              {/*
                ...
                대오 식별자 표기는 사용자 결정으로 이 줄에서 제거했다(§21.6). 빠진 것은 **위치 정보가 아니라
                주최측 내부 식별자**이고, 도로·방향·거리·현장 행동 지시는 그대로 남아 있어
                "어디로 가는가"에 대한 정보는 하나도 줄지 않았다. 식별자는 **지도 라벨 ④ 와 범례**에
                남아 있다(현장 안내방송이 "대오 2"라 말할 때 연결할 고리). **산문에 되살리지 마라.**
              */}
          ...
          <section aria-labelledby="facility-heading" className="mt-section md:mt-section-lg">
            ...
            <ul className="mt-6 grid gap-4 md:grid-cols-2">
              ...
              <InfoCard title="화장실">
                ...
                여의도공원 화장실 2호(개나리) 이용
                {/*
                  거리는 **반드시 코스콤지부 집결 위치(대오 2) 기준**이다(§21.4.5 · 문안 게이트 38).
                  5번 출구 기준 거리를 쓰지 마라(그쪽은 이 값의 다섯 배쯤 된다) — 큰 숫자를 보면
                  조합원이 "멀어서 못 가겠다"고
                  판단하는데, 정작 조합원이 서 있는 자리에서는 눈앞이다.
                  **기준점을 잘못 잡으면 사실을 다 맞혀도 행동이 틀어진다.**
                  80m 는 밴드 **중심** 기준이며 가까운 쪽 끝은 20m 다 — **더 먼 값을 택했다.**
                  가 보니 예상보다 가까운 것은 문제가 없지만, 예상보다 먼 것은 문제다.
                  대형 수치로 키우지 마라 — 이 페이지의 유일한 대형 수치는 `18:30` 이다.
                */}
