...
export const STRUGGLE_SCHEDULE: readonly ScheduleItem[] = [
   ...
   *    흉내 내면 거짓이다. ★★ **금지는 «더» 강해졌고 근거 수치는 «두 번째로» 낡았다**
   *    (`14:20` → `15:15` → **`17:40`**). ⚠ **어느 수치가 어느 출처인지 함께 적는다:**
   *      **`15:15` = 주최측 식순(폐회선언)** · **`17:40` = 지부 «조합원 일정»(자유투쟁 종료)**.
   *    낡은 `14:20` 을 인용하지 마라
   *  - `· 11시` 금지 — 이 두 줄은 `/bargaining-2026` 카드에서 **연달아** 나온다.
   *    `11시` 가 `10시 30분` 바로 위에 붙으면 *"11시까지 가면 되나"* 로 읽혀 **30분 늦는다**(§52-5)
   *  - `11시~15:15` 류 **범위 금지 — 금지는 «오히려 강해졌다»**. `15:15` 는 **폐회선언 시각**이지
   *    «파업 종료 시각»이 아니다(§51-5 — 이 프로젝트가 반복 금지한 것).
   *    ⚠ **근거가 바뀌었다**[2026-09-01 · v3]: 종전은 *«원문이 파업의 시간 범위를 말하지 않는다»* 였는데
   *      **범위가 «생겼다» — 다만 그것은 «파업»이 아니라 «참가 조합원 일정»의 범위(`08:40~17:40`)이고
   *      출처가 다르다(지부 자체 자료).** 옛 표기 `11시~15:15` 는 이제 **원문과도 어긋난다.**
   */
  {
    date: "2026-09-04",
    title: "총파업",
