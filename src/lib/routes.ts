...
export const EXTERNAL_LINKS = {
  ...
  /**
   * 네이버 길찾기 — **도착지 = KDB산업은행**(사용자 지시 2026-08-22).
   *
   * 바뀐 이유: *"공식집회장소 국회의사당역 5번 출구는 **메인무대 기준**이고,
   * 코스콤지부 기준은 3번 출구에서 여의도공원 쪽 **KDB산업은행 앞**"*.
   * 종전 도착지(5번 출구)는 조합원을 **363m 떨어진 곳**으로 보냈다(3번 출구는 231m).
   *
   * ## 좌표부 인코딩 — 이제 **규칙을 안다**(2026-08-22 역산)
   *
   * `3zfgXq,2AKonz` 는 네이버 내부 인코딩이고 규칙은 다음과 같다:
   *   **base62(알파벳 `0-9a-zA-Z`) 로 `round(좌표 × 1e7) + 2_000_000_000`**
   * 종전 값 `3zf71R,2AKrxU` ↔ `126.9172199, 37.5282738` 로 **양방향 정확히 재현**해 얻었고,
   * 새 값도 역변환하면 `126.9210364, 37.5270565`(OSM `way/682329441` KDB산업은행 본점)로 돌아온다.
   *
   * ⚠ **그래도 소수 좌표 형식(`/p/directions/-/126.92,37.52,…`)으로 바꾸지 마라** —
   * **HTTP 200 을 주면서 마포구 성산동을 보여준다**(검증 10회차 실측). 인코딩 형식을 지켜라.
   * ⚠ **`index.nhn`·`route.nhn` 레거시와 `nmap://` 앱 스킴 금지** — 앱 미설치 조합원에게 죽은 링크다.
   * ⚠ **HTTP 200 을 동작 근거로 삼지 마라.** `map.naver.com` 은 SPA 라 없는 경로에도 200 을 준다.
   *
   * ## ★ 미완 검증 — 반드시 사람이 한 번 눌러 봐야 한다
   *
   * **도착 핀이 KDB산업은행에 꽂히는지 눈으로 확인하지 못했다.**
   * 브라우저 자동화가 `map.naver.com` 을 차단했고, 모바일 UA 로 받은 HTML 은 JS 셸이라
   * 좌표가 들어 있지 않았다(현행 URL 과 응답이 **바이트 단위로 동일**해 대조군으로도 못 썼다).
   * 근거는 **인코딩 왕복 재현뿐**이다. 8/28 전에 실기기로 1회 확인한다(요구 78 정기 점검과 함께).
   */
  naverDirections:
    "https://map.naver.com/p/directions/-/3zfgXq,2AKonz,KDB%EC%82%B0%EC%97%85%EC%9D%80%ED%96%89,,/-/transit?c=3zfgXq,2AKonz,17,0,0,0,dh",
} as const;
