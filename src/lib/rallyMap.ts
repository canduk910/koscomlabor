...
/*
 * ⚠ **`toBandFeature`(대오 밴드 생성기)는 제거됐다** — 검증 12회차로 `RALLY_COLUMNS` 가
 * 통째로 사라졌기 때문이다. 구역 좌표가 확인되면 **새 체계(1·2·3구역)로 다시 설계**하라.
 * 옛 함수를 되살려 3구역에 붙이지 마라 — `calculated` 확신도 스타일이 함께 딸려 오는데
 * **3구역 좌표의 상한은 `estimated` 다**(요구 106).
 */
...
export const MAP_FEATURES: readonly MapFeature[] = [
  ...
  /*
   * ⚠ **대오 밴드(옛 ③④)는 제거됐다**(검증 12회차 요구 98 · 2026-08-21).
   *
   * 근거가 무너졌다: 주최측이 **새 배치도**로 `1·2·3구역` 체계를 안내했고
   * **코스콤지부는 `집회 3구역` 배정 예정**이다. 현행 표시가 가리키던 자리는 새 자료 기준
   * **2구역 — 다른 지부 대오**다. 옛 근거(`9. 기타 토의` · `각 지부별 대오 논의`)는
   * 애초에 확정도가 가장 낮았고 **지부명도 없었다**(`코스콤지부 = 대오 2` 는 사용자 텍스트가 붙인 것).
   *
   * ⚠ **3구역 폴리곤을 추정해 그리지 마라 — 그것이 날조다**(§12-4).
   * 5회차가 대오 2 좌표를 계산할 수 있었던 것은 **확정 지물 두 개**(5번 출구 · 더샵 부지)를
   * 도로에 정사영했기 때문이다. `여의도공원 방향`·`가장 동쪽` 은 **방향어일 뿐 기준점이 아니고**,
   * 그 방식은 4회차 요구 9(`우측 도로` 금지)가 이미 막은 오류다.
   * 원본 이미지를 확보해 좌표가 나오더라도 **`estimated` 등급이 상한**이다(요구 106).
   *
   * **모르면 찍지 않는다** — 6회차가 화장실 좌표에 적용한 원칙이고,
   * 이번은 **조합원 본인이 서는 자리**라 훨씬 무겁다. 표시하지 않는 사실은 **범례 각주가 밝힌다**(요구 102).
   */
  /*
   * ⚠ **⑤ 여의도더샵아일랜드파크 부지는 제거됐다**(검증 16회차 요구 130 · 2026-08-21).
   *
   * **도형은 위치를 주장하지 않아도 "여기가 중요하다"를 말한다.** ④ 가 사라진 뒤 ⑤ 는 지도에 남는
   * **유일한 지상 건물**이 됐고, 그 방향은 새 배치도 기준 **2구역 — 다른 지부 대오**다.
   * **배포본을 이미 본 조합원에게는 옛 자리 기억을 강화한다** — 8회차가 ⑥ 도트에
   * `화장실` 단독 라벨을 금지한 것과 같은 논리다(**기호가 놓이는 것 자체가 주장이다**).
   *
   * 존치 근거(거리 감각)도 실측으로 기각됐다: **축척 바(100m)가 이미 있고** ①⑥ 두 점이 그 역할을 한다.
   * 화장실 카드는 `여의도공원` 만 말하고 이 지물을 쓰지 않는다.
   * §0.4 저촉도 아니다 — ⑤ 는 **참고 지물이지 콘텐츠가 아니었고**, 담던 정보(위치 앵커)는
   * **범례 각주가 이미 밝히고 있다**(요구 102).
   */
...
/**
 * 구역 상태 문안 — **블록 2 산문과 지도 위 상태 패널이 여기 한 곳에서 나온다**(§28.2.4).
 *
 * 요구 88 원칙(단일 출처 파생)을 문안에도 적용한다: 두 자리에 문자열을 따로 두면
 * **구역이 확인됐을 때 한쪽만 고쳐진다.** 그 순간 지도는 "표시하지 않았습니다"라고 말하는데
 * 본문은 위치를 말하는 상태가 된다.
 *
 * ⚠ **`예정` 을 지우지 마라** — 원문 표기가 `3구역 배정 예정` 이고 §6.9 전사는 **원본 대조 전**이다.
 * 구역이 확인되면 이 상수부터 고치고, 그 다음 지도 데이터를 넣어라.
 */
export const ZONE_STATUS = {
  /** 배정 사실. 블록 2 는 앞에 `주최측 안내에 따르면` 을 붙여 출처를 밝힌다 */
  assignment: "코스콤지부는 집회 3구역에 배정될 예정입니다.",
  /** 블록 2 — 우리 상태 + 약속 */
  pending: "구역의 정확한 위치는 확인 중이며, 확인되는 대로 지도에 반영하겠습니다.",
  /** 지도 위 상태 패널 — 같은 사실을 **지도 맥락**으로. 조합원이 지도를 보기 **전에** 읽는다 */
  pendingOnMap: "구역의 정확한 위치는 확인 중이라 지도에 표시하지 않았습니다.",
} as const;
...
export const LEGEND_FOOTNOTE =
  "LED무대는 주최측 안내자료의 “기타 토의” 항목에만 나오고 배치가 논의 중이라 지도에 표시하지 않았습니다. 지부별 집회구역은 주최측이 새 배치도로 안내해 확인 중이며, 확인 전까지 지도에 표시하지 않습니다.";
